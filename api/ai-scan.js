...
Extract every line item. For furniture, capture manufacturer, model number, color/finish when visible. Keep the FULL product description: when a description runs over several lines with options, finish, fabric, dimensions or configuration detail, put every one of those lines in "description" separated by newlines. Midwest reprints that detail on the vendor purchase order, so dropping it makes the PO unusable.`,
