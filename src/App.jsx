  ...
  return <div style={{minHeight:"100vh",background:"#fff",color:"#111",fontFamily:"'Satoshi',sans-serif"}}>
    <div style={{maxWidth:960,margin:"0 auto",padding:"32px 24px"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
        <div style={{display:"flex",alignItems:"center",gap:12}}><img src={MW_LOGO} alt="MW" style={{height:44,width:44,borderRadius:8,objectFit:"contain",flexShrink:0}}/><div><div style={{fontSize:16,fontWeight:700}}>Midwest Educational Furnishings, Inc.</div><div style={{fontSize:11,color:"#888"}}>21191 N Valley Rd, Kildeer, IL 60047 US | (847) 847-1865</div></div></div>
      ...
      <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11,marginBottom:16}}>
        <thead><tr style={{borderBottom:"2px solid #222"}}>{["TAG","MANUF.","MODEL #","DESCRIPTION","COLOR","QTY","SHIPPING EACH","SHIPPING TOTAL","INSTALL EACH","INSTALL TOTAL","YOUR PRICE","EXTENDED"].map((h,i)=><th key={i} style={{padding:"6px 4px",textAlign:i>=5?"right":"left",fontSize:10,fontWeight:700,color:"#2dd4bf",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
        <tbody>{(items||[]).map((item,i)=>{const ship=item.shippingPerUnit||0;const inst=item.installPerUnit||0;const qty=item.qtyOrdered||0;const price=item.unitPrice||0;const ext=(price+ship+inst)*qty;return <tr key={i} style={{borderBottom:"1px solid #e5e5e5"}}><td style={{padding:"6px 4px"}}>{item.tag||""}</td><td style={{padding:"6px 4px"}}>{item.manufacturer||""}</td><td style={{padding:"6px 4px"}}>{item.modelNumber||""}</td><td style={{padding:"6px 4px"}}>{item.description||""}</td><td style={{padding:"6px 4px"}}>{item.color||""}</td><td style={{padding:"6px 4px",textAlign:"right"}}>{qty}</td><td style={{padding:"6px 4px",textAlign:"right"}}>{ship>0?"$"+ship.toFixed(2):""}</td><td style={{padding:"6px 4px",textAlign:"right"}}>{ship>0?"$"+(ship*qty).toFixed(2):""}</td><td style={{padding:"6px 4px",textAlign:"right"}}>{inst>0?"$"+inst.toFixed(2):""}</td><td style={{padding:"6px 4px",textAlign:"right"}}>{inst>0?"$"+(inst*qty).toFixed(2):""}</td><td style={{padding:"6px 4px",textAlign:"right"}}>${price.toFixed(2)}</td><td style={{padding:"6px 4px",textAlign:"right",fontWeight:600}}>${ext.toFixed(2)}</td></tr>})}</tbody>
      </table></div>
...
function Dashboard({jobs,lineItems,reps,vendors,customers,getJobFinancials,getJobItems,setPage,setSelectedJob,dateFilter,setDateFilter}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:6,flexWrap:"wrap"}}><h1 style={{fontSize:28,fontWeight:800,color:"#f0f0f0",letterSpacing:-1.5}}>Command Center</h1><div style={{padding:"3px 8px",background:"#34d399",borderRadius:20,fontSize:11,fontWeight:700,color:"#fff"}}>LIVE</div>
      <div style={{marginLeft:"auto",display:"flex",gap:3,background:"#111",padding:3,borderRadius:8,flexShrink:0}}>{[["all","All"],["ytd","YTD"],["quarter","Q"],["month","Mo"]].map(([v,l])=><button key={v} onClick={()=>setDateFilter(v)} style={{padding:"4px 10px",borderRadius:6,border:"none",cursor:"pointer",background:dateFilter===v?"#2dd4bf":"transparent",color:dateFilter===v?"#000":"#525252",fontSize:11,fontWeight:dateFilter===v?600:400,fontFamily:"inherit",transition:"all 0.15s"}}>{l}</button>)}</div>
    </div>
    <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>{[
      ...
      {label:"Full Export",fn:()=>{notify("Full database exported");const blob=new Blob(["export"],{type:"text/csv"});const a=document.createElement("a");a.href=URL.createObjectURL(blob);a.download="midwest-full-export.csv";a.click()}}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}} className="resp-grid-4">
      <Card style={{padding:16,cursor:"default",transition:"all 0.25s"}} hover><span style={{fontSize:11,color:"#737373",fontWeight:600,textTransform:"uppercase",letterSpacing:1.5}}>Pipeline</span><div style={{fontSize:"clamp(20px,4vw,32px)",fontWeight:800,color:"#f0f0f0",fontFamily:"'JetBrains Mono',monospace",letterSpacing:-1,lineHeight:1,margin:"6px 0 4px"}}><AnimNum value={fmt(totalRev)}/></div><div style={{fontSize:12,color:"#525252"}}>{filtered.length} jobs</div></Card>
      <Card style={{padding:16,cursor:"default",transition:"all 0.25s"}} hover><span style={{fontSize:11,color:"#737373",fontWeight:600,textTransform:"uppercase",letterSpacing:1.5}}>Active</span><div style={{fontSize:"clamp(20px,4vw,32px)",fontWeight:800,color:"#f0f0f0",fontFamily:"'JetBrains Mono',monospace",letterSpacing:-1,lineHeight:1,margin:"6px 0 4px"}}><AnimNum value={activeJobs}/></div><div style={{fontSize:12,color:"#525252"}}>{filtered.filter(j=>j.phase==="Quoting").length} quoting</div></Card>
      <Card style={{padding:16,cursor:"default",transition:"all 0.25s"}} hover><span style={{fontSize:11,color:"#737373",fontWeight:600,textTransform:"uppercase",letterSpacing:1.5}}>Margin</span><div style={{fontSize:"clamp(20px,4vw,32px)",fontWeight:800,color:"#34d399",fontFamily:"'JetBrains Mono',monospace",letterSpacing:-1,lineHeight:1,margin:"6px 0 4px"}}><AnimNum value={pct(avgMargin)}/></div><div style={{fontSize:12,color:"#525252"}}>{fmt(totalRev-totalCost)} profit</div></Card>
      <Card style={{padding:16,cursor:"default",transition:"all 0.25s"}} hover><span style={{fontSize:11,color:"#737373",fontWeight:600,textTransform:"uppercase",letterSpacing:1.5}}>Delivery</span><div style={{fontSize:"clamp(20px,4vw,32px)",fontWeight:800,color:"#fbbf24",fontFamily:"'JetBrains Mono',monospace",letterSpacing:-1,lineHeight:1,margin:"6px 0 4px"}}><AnimNum value={pct(deliveredPct)}/></div><div style={{fontSize:12,color:"#525252"}}>{pendingCount} pending</div></Card>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}} className="resp-grid-2">
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12,display:"flex",alignItems:"center",gap:6}}><I n="alert" s={14}/> Action Required</div>
      ...
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Revenue by Month</div>
        <ResponsiveContainer width="100%" height={160}><BarChart data={monthlyRev}><XAxis dataKey="name" tick={{fill:"#737373",fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>"$"+Math.round(v/1000)+"k"}/><Tooltip contentStyle={darkTip} cursor={{fill:"rgba(45,212,191,0.04)"}}/><RBar dataKey="rev" fill="#2dd4bf" radius={[4,4,0,0]}/></BarChart></ResponsiveContainer>
      </Card>
    </div>

    {/* Row 3: Margin Trend + Vendor Spend */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}} className="resp-grid-2">
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Margin Trend</div>
        <ResponsiveContainer width="100%" height={160}><LineChart data={marginData}><XAxis dataKey="name" tick={{fill:"#737373",fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>v.toFixed(0)+"%"}/><Tooltip contentStyle={darkTip} cursor={{stroke:"rgba(52,211,153,0.2)"}}/><Line type="monotone" dataKey="margin" stroke="#34d399" strokeWidth={2.5} dot={{fill:"#34d399",r:4,strokeWidth:2,stroke:"#000"}}/></LineChart></ResponsiveContainer>
      </Card>
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Vendor Spend</div>
        <ResponsiveContainer width="100%" height={130}><PieChart><Pie data={vendorSpend.map(([name,value])=>({name,value}))} cx="50%" cy="50%" innerRadius={38} outerRadius={58} paddingAngle={2} dataKey="value">{["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"].map((c,i)=><Cell key={i} fill={c}/>)}</Pie><Tooltip contentStyle={darkTip}/></PieChart></ResponsiveContainer>
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:6,justifyContent:"center"}}>{vendorSpend.map(([name],i)=><span key={name} style={{fontSize:10,color:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i],display:"flex",alignItems:"center",gap:4}}><div style={{width:6,height:6,borderRadius:"50%",background:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i]}}/>{name}</span>)}</div>
      </Card>
    </div>

    {/* Row 4: Payment Collection + Pipeline by Phase */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}} className="resp-grid-2">
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Payment Collection</div>
        <ResponsiveContainer width="100%" height={160}><BarChart data={[{name:"Paid",value:paidRev},{name:"Partial",value:partialRev},{name:"Unpaid",value:unpaidRev}]}><XAxis dataKey="name" tick={{fill:"#737373",fontSize:11}} axisLine={false} tickLine={false}/><YAxis tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>"$"+Math.round(v/1000)+"k"}/><Tooltip contentStyle={darkTip} cursor={{fill:"rgba(255,255,255,0.02)"}}/><RBar dataKey="value" radius={[4,4,0,0]}>{[{fill:"#34d399"},{fill:"#fbbf24"},{fill:"#f87171"}].map((c,i)=><Cell key={i} fill={c.fill}/>)}</RBar></BarChart></ResponsiveContainer>
      </Card>
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Pipeline by Phase</div>
        {phaseData.map(p=><div key={p.phase} style={{marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontSize:13,color:"#d4d4d4",fontWeight:500}}>{p.phase}</span><span style={{fontSize:13,color:"#a3a3a3",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(p.rev)}</span></div><Bar value={p.rev} max={totalRev||1} color={statusColor(p.phase)} height={8}/><div style={{fontSize:11,color:"#525252",marginTop:2}}>{p.count} job{p.count!==1?"s":""}</div></div>)}
      </Card>
    </div>

    {/* Row 5: Active Jobs + Top Customers */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}} className="resp-grid-2">
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Active Jobs</div>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>{filtered.map(job=>{const f=getJobFinancials(job.id);return <div key={job.id} onClick={()=>{setSelectedJob(job.id);setPage("jobs")}} style={{padding:"10px 12px",background:"#0a0a0a",borderRadius:10,cursor:"pointer",transition:"all 0.2s",border:"1px solid rgba(255,255,255,0.04)"}} onMouseEnter={e=>{e.currentTarget.style.background="#141414";e.currentTarget.style.borderColor="rgba(45,212,191,0.12)"}} onMouseLeave={e=>{e.currentTarget.style.background="#0a0a0a";e.currentTarget.style.borderColor="rgba(255,255,255,0.04)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13,fontWeight:600,color:"#e5e5e5"}}>{job.name}</span><Badge label={job.phase} color={statusColor(job.phase)}/></div><Bar value={f.totalReceived} max={f.totalOrdered||1} color={statusColor(job.phase)} height={4}/><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:12,color:"#737373"}}>{fmtN(f.totalReceived)}/{fmtN(f.totalOrdered)} units</span><span style={{fontSize:12,color:"#a3a3a3",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(f.totalRevenue)}</span></div></div>})}</div>
      </Card>
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Top Customers</div>
        {topCustomers.map(([name,rev],i)=><div key={name} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<topCustomers.length-1?"1px solid #1a1a1a":"none",transition:"all 0.15s"}}><div style={{width:26,height:26,borderRadius:8,background:"#2dd4bf10",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#2dd4bf",fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{i+1}</div><span style={{fontSize:13,color:"#e5e5e5",flex:1,fontWeight:500}}>{name}</span><span style={{fontSize:13,color:"#a3a3a3",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(rev)}</span></div>)}
      </Card>
    </div>

    {/* Row 6: Team Leaderboard + Top Vendors */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}} className="resp-grid-2">
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Team Leaderboard</div>
        {[...reps].filter(r=>!r.id.includes("SEED_FLAG")).sort((a,b)=>{const ar=jobs.filter(j=>j.salesRep===b.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const br=jobs.filter(j=>j.salesRep===a.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return ar-br}).map((r,i)=>{const rv=jobs.filter(j=>j.salesRep===r.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const jc=jobs.filter(j=>j.salesRep===r.id).length;const comm=rv*(r.commissionRate||0);const pendComm=jobs.filter(j=>j.salesRep===r.id&&j.paymentStatus!=="paid").reduce((s2,j)=>s2+getJobFinancials(j.id).totalRevenue*(r.commissionRate||0),0);return <div key={r.id} style={{padding:"10px 0",borderBottom:i<reps.filter(r2=>!r2.id.includes("SEED_FLAG")).length-1?"1px solid #1a1a1a":"none",transition:"all 0.15s"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:28,height:28,borderRadius:"50%",background:["#2dd4bf","#a78bfa","#fbbf24","#34d399","#f87171","#8b5cf6"][i%6]+"15",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:["#2dd4bf","#a78bfa","#fbbf24","#34d399","#f87171","#8b5cf6"][i%6],fontWeight:700}}>{i+1}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#e5e5e5"}}>{r.name}</div><div style={{fontSize:11,color:"#525252"}}>{r.territory||""} - {jc} job{jc!==1?"s":""} - {((r.commissionRate||0)*100).toFixed(1)}%</div></div><div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(rv)}</div><div style={{fontSize:10,color:"#525252"}}>comm: {fmt(comm)}</div></div></div></div>})}
      </Card>
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Top Vendors by Spend</div>
        {vendorSpend.map(([name,spend],i)=><div key={name} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<vendorSpend.length-1?"1px solid #1a1a1a":"none",transition:"all 0.15s"}}><div style={{width:26,height:26,borderRadius:8,background:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i%6]+"12",display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i%6],fontWeight:700,fontFamily:"'JetBrains Mono',monospace"}}>{i+1}</div><span style={{fontSize:13,color:"#e5e5e5",flex:1,fontWeight:500}}>{name}</span><span style={{fontSize:13,color:"#a3a3a3",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(spend)}</span></div>)}
      </Card>
    </div>

    {/* Row 7: Delivery Health + Commission Summary */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}} className="resp-grid-2">
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:14}}>Delivery Health</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:14}}>
          <div style={{textAlign:"center"}}><div style={{fontSize:28,fontWeight:800,color:"#2dd4bf",fontFamily:"'JetBrains Mono',monospace"}}><AnimNum value={pct(deliveredPct)}/></div><div style={{fontSize:11,color:"#525252"}}>Delivered</div></div>
          <div style={{textAlign:"center"}}><div style={{fontSize:28,fontWeight:800,color:"#a78bfa",fontFamily:"'JetBrains Mono',monospace"}}><AnimNum value={pct(invoicedPct)}/></div><div style={{fontSize:11,color:"#525252"}}>Invoiced</div></div>
          <div style={{textAlign:"center"}}><div style={{fontSize:28,fontWeight:800,color:"#fbbf24",fontFamily:"'JetBrains Mono',monospace"}}>{pendingCount}</div><div style={{fontSize:11,color:"#525252"}}>Pending</div></div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          <div><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12,color:"#a3a3a3"}}>Delivery</span><span style={{fontSize:12,color:"#737373",fontFamily:"'JetBrains Mono',monospace"}}>{fmtN(totalReceived)}/{fmtN(totalOrdered)}</span></div><Bar value={totalReceived} max={totalOrdered||1} color="#2dd4bf" height={6}/></div>
          <div><div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}><span style={{fontSize:12,color:"#a3a3a3"}}>Invoice</span><span style={{fontSize:12,color:"#737373",fontFamily:"'JetBrains Mono',monospace"}}>{fmtN(invoicedTotal)}/{fmtN(totalOrdered)}</span></div><Bar value={invoicedTotal} max={totalOrdered||1} color="#a78bfa" height={6}/></div>
        </div>
      </Card>
      <Card style={{padding:16}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:14}}>Commission Summary</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}>
          <div style={{padding:12,background:"#34d39908",borderRadius:10,textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:"#34d399",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(commEarned)}</div><div style={{fontSize:11,color:"#525252"}}>Earned</div></div>
          <div style={{padding:12,background:"#fbbf2408",borderRadius:10,textAlign:"center"}}><div style={{fontSize:20,fontWeight:800,color:"#fbbf24",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(commPending)}</div><div style={{fontSize:11,color:"#525252"}}>Pending</div></div>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>{reps.filter(r=>!r.id.includes("SEED_FLAG")).map(r=>{const rv=jobs.filter(j=>j.salesRep===r.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const jc=jobs.filter(j=>j.salesRep===r.id).length;const earned=jobs.filter(j=>j.salesRep===r.id&&j.paymentStatus==="paid").reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue*(r.commissionRate||0),0);const pending=jobs.filter(j=>j.salesRep===r.id&&j.paymentStatus!=="paid").reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue*(r.commissionRate||0),0);return <div key={r.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:"1px solid #1a1a1a"}}><span style={{fontSize:12,color:"#d4d4d4",flex:1,fontWeight:500}}>{r.name}</span><span style={{fontSize:11,color:"#525252"}}>{jc} job{jc!==1?"s":""}</span><span style={{fontSize:11,color:"#34d399",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(earned)}</span>{pending>0&&<span style={{fontSize:10,color:"#fbbf24"}}>{fmt(pending)} pending</span>}</div>})}</div>
      </Card>
    </div>

    {/* Row 8: Margin by Job */}
    <Card style={{padding:16,marginBottom:14}} hover><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:12}}>Margin by Job</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:10}} className="resp-grid-3">{filtered.map(j=>{const f=getJobFinancials(j.id);const marginColor=f.margin>=30?"#34d399":f.margin>=20?"#fbbf24":"#f87171";return <div key={j.id} style={{padding:"10px 12px",background:"#0a0a0a",borderRadius:10,border:"1px solid rgba(255,255,255,0.04)",transition:"all 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(45,212,191,0.12)"}} onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(255,255,255,0.04)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><span style={{fontSize:13,fontWeight:600,color:"#e5e5e5"}}>{j.name}</span><span style={{fontSize:14,fontWeight:700,color:marginColor,fontFamily:"'JetBrains Mono',monospace"}}>{pct(f.margin)}</span></div><div style={{display:"flex",justifyContent:"space-between"}}><span style={{fontSize:12,color:"#737373"}}>{fmt(f.totalRevenue)}</span></div><Bar value={f.margin} max={100} color={marginColor} height={4}/></div>})}</div>
    </Card>
  </div>;
}
