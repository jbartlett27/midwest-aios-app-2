      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        @media(max-width:768px){
          ...
          .main-content{padding:16px 14px 80px 14px!important;overflow-x:hidden!important;max-width:100vw!important}
          ...
          .doc-tabs>button{min-width:0!important;padding:10px 8px!important;font-size:12px!important}
          .doc-tabs>button>span:last-child{font-size:14px!important}
          .cal-grid{grid-template-columns:repeat(7,1fr)!important;font-size:10px!important}
        }
        @media(max-width:480px){
          .main-content{padding:10px 10px 80px 10px!important}
...
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,getJobFinancials,updateJob,updateLineItem,notify,qbConfig,setPage,triggerPrint,pendingCommPreview,setPendingCommPreview,...ctx}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices, Commission Statements"/>
    ...
    <div className="doc-tabs" style={{display:"flex",gap:0,marginBottom:24,background:"#111111",borderRadius:12,padding:4,overflow:"hidden"}}>
    ...
    {tab==="preview"&&previewDoc&&<Card style={{border:"1px solid #2dd4bf30"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{fontSize:16,fontWeight:700,color:"#2dd4bf"}}>{previewDoc.type==="quote"?"PROJECT QUOTE":previewDoc.type==="po"?"PURCHASE ORDER":previewDoc.type==="commission"?"COMMISSION STATEMENT":previewDoc.data?.isPartial?"PARTIAL INVOICE":"INVOICE"}</div><StatusBadge docNum={previewDoc.data.docNum}/></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{previewDoc.data.docNum&&<div style={{display:"flex",gap:4}}>{["drafted","sent","approved"].map(s=><button key={s} onClick={()=>setDocStatus(previewDoc.data.docNum,s)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid "+(docStatuses[previewDoc.data.docNum]===s?"#2dd4bf":"#333333"),background:docStatuses[previewDoc.data.docNum]===s?"#2dd4bf20":"transparent",color:docStatuses[previewDoc.data.docNum]===s?"#2dd4bf":"#525252",fontSize:12,fontFamily:"inherit",cursor:"pointer",textTransform:"capitalize"}}>{s}</button>)}</div>}<Btn onClick={()=>handleExportPDF(previewDoc)}><I n="download" s={14}/> Export PDF</Btn><Btn v="ghost" onClick={()=>{const doc=previewDoc;const isQuote=doc.type==="quote";const isPO=doc.type==="po";const isComm=doc.type==="commission";const recipient=isQuote?doc.data.customer?.email:isPO?doc.data.vendor?.email:isComm?doc.data.rep?.email:doc.data.customer?.email;setEmailTo(recipient||"");setEmailSubject((isQuote?"Project Quote":isPO?"Purchase Order":isComm?"Commission Statement":"Invoice")+" - "+doc.job.name+" - "+(doc.data.docNum||""));setEmailModal(doc)}}><I n="send" s={14}/> Email</Btn>{previewDoc.type!=="quote"&&previewDoc.type!=="commission"&&<Btn v={qbConfig.connected?"primary":"secondary"} onClick={()=>qbConfig.connected?pushToQB(previewDoc):setPage("qbsetup")} style={pushing?{opacity:0.6,pointerEvents:"none"}:{}}><I n="send" s={14}/> {pushing?"Pushing...":qbConfig.connected?"Push to QB":"Connect QB"}</Btn>}<Btn v="secondary" onClick={()=>setTab(previewDoc.type==="commission"?"commissions":previewDoc.type==="quote"?"quotes":previewDoc.type==="po"?"pos":"invoices")}>&larr; Back</Btn>{previewDoc.type==="invoice"&&previewDoc.data.items.length>0&&<Btn v="ghost" style={{fontSize:12,color:"#34d399",border:"1px solid #05966930"}} onClick={()=>{previewDoc.data.items.forEach(item=>{if(item.qtyReceived>item.qtyInvoiced){ctx.updateLineItem(item.id,{qtyInvoiced:item.qtyReceived,invoiceDate:new Date().toISOString().split("T")[0]})}});notify("All "+previewDoc.data.items.length+" items marked as invoiced")}}>Mark All Invoiced</Btn>}</div></div>
      ...
      <div style={{background:"#fff",color:"#111",borderRadius:12,padding:40,fontFamily:"'Satoshi',sans-serif",maxWidth:800,margin:"0 auto"}}>
        ...
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
          <thead><tr>{(previewDoc.type==="quote"?["Tag","Manuf.","Model #","Description","Color","Qty","Shipping/Ea","Shipping Ext","Install/Ea","Install Ext","Your Price","Extended"]:previewDoc.type==="commission"?["Job","Customer","Revenue","Status","Commission"]:["Description","Qty","Rate","Amount"]).map((h,i)=><th key={i} style={{padding:"8px 6px",textAlign:i<5&&previewDoc.type==="quote"?"left":"right",fontSize:10,fontWeight:600,color:"#888",textTransform:"uppercase",letterSpacing:0.5,borderBottom:"2px solid #e5e5e5"}}>{h}</th>)}</tr></thead>
...
// SALES PORTAL
// ===============================================================
function SalesPortalPage({jobs,reps,customers,lineItems,getJobFinancials,getJobItems,vendors,setPage,setSelectedJob,updateJob,notify}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Sales Portal" sub="Pipeline management, CRM, and team performance"/>
    ...
    {crmTab==="tasks"&&<>
      <Card style={{marginBottom:16,border:"1px solid rgba(45,212,191,0.12)"}}>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          ...
          <input type="date" value={taskDue} onChange={e=>setTaskDue(e.target.value)} style={{...inputStyle,width:150}}/>
      ...
      <Card><div style={{fontSize:15,fontWeight:700,color:"#f0f0f0",marginBottom:14}}>Open Tasks ({allTasks.filter(t=>!t.done).length})</div>
        {allTasks.filter(t=>!t.done).length===0?<div style={{fontSize:13,color:"#737373"}}>No open tasks. Add one above.</div>:
        allTasks.filter(t=>!t.done).map((t,i)=><div key={i} style={{display:"flex",gap:12,alignItems:"center",padding:"10px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
          <div style={{width:8,height:8,borderRadius:"50%",background:"#fbbf24",flexShrink:0}}/>
          <div style={{flex:1}}><div style={{fontSize:13,color:"#e5e5e5"}}>{t.text}</div><div style={{fontSize:11,color:"#737373"}}>{t.jobName}</div></div>
        </div>)}
      </Card>
    </>}
    ...
    {crmTab==="notes"&&<>
      <Card style={{marginBottom:16,border:"1px solid rgba(167,139,250,0.12)"}}>
        ...
        <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
          <textarea value={noteText} onChange={e=>setNoteText(e.target.value)} placeholder="Meeting notes, follow-up items, observations..." rows={2} style={{...inputStyle,flex:1,minWidth:200,resize:"vertical"}}/>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            <select id="noteJob" style={{...inputStyle,width:180}}>{rj.map(j=><option key={j.id} value={j.id}>{j.name}</option>)}</select>
            <Btn v="ghost" onClick={()=>{const sel=document.getElementById("noteJob");if(sel)addNote(sel.value)}}>Save Note</Btn>
          </div>
        </div>
      </Card>
      <Card><div style={{fontSize:15,fontWeight:700,color:"#f0f0f0",marginBottom:14}}>Recent Notes</div>
        {allNotes.length===0?<div style={{fontSize:13,color:"#737373"}}>No notes yet. Notes persist to the database with each job.</div>:
        allNotes.map((n,i)=><div key={i} style={{padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
          <div style={{fontSize:13,color:"#e5e5e5",lineHeight:1.6}}>{n.text}</div>
          <div style={{fontSize:11,color:"#9a9a9a",marginTop:4}}>{n.jobName}{n.time?" -- "+n.time:""}</div>
        </div>)}
      </Card>
    </>}

    {/* TEAM */}
    {crmTab==="team"&&isOverview&&<Card>
      <div style={{fontSize:15,fontWeight:700,color:"#f0f0f0",marginBottom:14}}>Team Performance</div>
      {reps.filter(r=>!r.id.includes("SEED_FLAG")).map(r=>{const rJobs=jobs.filter(j=>j.salesRep===r.id);const rv=rJobs.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const pRev=rJobs.filter(j=>j.paymentStatus==="paid").reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const comm=rv*(r.commissionRate||0);return <div key={r.id} onClick={()=>setActiveRep(r.id)} className="hover-lift" style={{display:"flex",alignItems:"center",gap:14,padding:"16px",background:"#0a0a0a",borderRadius:12,marginBottom:10,cursor:"pointer",border:"1px solid rgba(255,255,255,0.04)",transition:"all 0.2s"}}>
        <div style={{width:44,height:44,borderRadius:10,background:"linear-gradient(135deg,rgba(45,212,191,0.12),rgba(167,139,250,0.12))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:800,color:"#2dd4bf"}}>{r.name.split(" ").map(n=>n[0]).join("")}</div>
        <div style={{flex:1}}><div style={{fontSize:15,fontWeight:600,color:"#f0f0f0"}}>{r.name}</div><div style={{fontSize:12,color:"#9a9a9a"}}>{r.territory} -- {r.tier} -- {rJobs.length} jobs</div></div>
        <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
          ...
          {r.commissionRate>0&&<div style={{textAlign:"center"}}><div style={{fontSize:16,fontWeight:700,color:"#fbbf24",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(comm)}</div><div style={{fontSize:10,color:"#737373"}}>Commission</div></div>}
        </div>
      </div>})}
    </Card>}
    {crmTab==="team"&&!isOverview&&<Card><div style={{fontSize:13,color:"#9a9a9a"}}>Switch to Overview to see all team members.</div></Card>}
  </div>;
}
...
function PlaybookPage({jobs,reps,vendors,customers,lineItems,getJobFinancials,setPage,customSops,addSop,deleteSop,notify}){
  ...
  if(activeDoc){const doc=allSops.find(s=>s.id===activeDoc);if(!doc)return null;return <RichSOP doc={doc}/>}
