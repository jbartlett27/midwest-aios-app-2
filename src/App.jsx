...
function SalesPortalPage({jobs,reps,customers,lineItems,getJobFinancials,vendors,addJob,notify,setSelectedJob,setPage,showNewJob,setShowNewJob,updateJob,getJobItems,addLineItem,updateLineItem,deleteLineItem,getItemStatus,getJobPOStatus,getJobInvStatus,addCustomer}){
  ...
  const [newJobSP,setNewJobSP]=useState({name:"",customer:customers[0]?.id||"",salesRep:"",dueDate:"",startDate:"",notes:"",terms:"Net 30",poNumber:"",shipTo:"",shipVia:""});
  ...
  const handleCreateJobSP=()=>{if(!newJobSP.name)return;const id="JOB-2026-"+String(jobs.length+1).padStart(3,"0");addJob({id,...newJobSP,salesRep:newJobSP.salesRep||(isOverview?"":activeRep),phase:"Quoting",createdDate:new Date().toISOString().split("T")[0],startDate:newJobSP.startDate||"",endDate:"",paymentStatus:"unpaid",terms:newJobSP.terms||"Net 30",poNumber:newJobSP.poNumber||"",shipTo:newJobSP.shipTo||"",shipVia:newJobSP.shipVia||"",billTo:"",orderNotes:"",docStatuses:{},activities:[],auditTrail:[]});setShowNewJobSP(false);setNewJobSP({name:"",customer:customers[0]?.id||"",salesRep:"",dueDate:"",startDate:"",notes:"",terms:"Net 30",poNumber:"",shipTo:"",shipVia:""});notify("Job "+id+" created")};
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Sales Portal" sub={isOverview?"Company overview and per-rep dashboards":rep.name+" -- "+rep.territory} action={<Btn onClick={()=>{setNewJobSP({...newJobSP,salesRep:isOverview?"":activeRep});setShowNewJobSP(true)}}><I n="plus" s={14}/> New Job</Btn>}/>
    ...
    {showNewJobSP&&<Card style={{marginBottom:20,border:"1px solid #2dd4bf30"}}><div style={{fontSize:14,fontWeight:700,marginBottom:16,color:"#2dd4bf"}}>Create New Job{!isOverview?" for "+rep.name:""}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:4}}>Job Name</label><input value={newJobSP.name} onChange={e=>setNewJobSP({...newJobSP,name:e.target.value})} placeholder="e.g., Lincoln USD Media Center" style={inputStyle}/></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:4}}>Customer</label><select value={newJobSP.customer} onChange={e=>setNewJobSP({...newJobSP,customer:e.target.value})} style={inputStyle}>{customers.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:4}}>Sales Rep</label><select value={newJobSP.salesRep||(isOverview?"":activeRep)} onChange={e=>setNewJobSP({...newJobSP,salesRep:e.target.value})} style={inputStyle}>{reps.map(r=><option key={r.id} value={r.id}>{r.name}</option>)}</select></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 2fr",gap:12,marginBottom:12}}><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:4}}>Start Date</label><input type="date" value={newJobSP.startDate} onChange={e=>setNewJobSP({...newJobSP,startDate:e.target.value})} style={inputStyle}/></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:4}}>Due Date</label><input type="date" value={newJobSP.dueDate} onChange={e=>setNewJobSP({...newJobSP,dueDate:e.target.value})} style={inputStyle}/></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:4}}>PO Notes</label><input value={newJobSP.notes} onChange={e=>setNewJobSP({...newJobSP,notes:e.target.value})} placeholder="Notes for purchase orders..." style={inputStyle}/></div></div><div style={{display:"flex",gap:8}}><Btn onClick={handleCreateJobSP}>Create Job</Btn><Btn v="secondary" onClick={()=>setShowNewJobSP(false)}>Cancel</Btn></div></Card>}
