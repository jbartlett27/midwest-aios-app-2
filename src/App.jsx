  ...
  return <div style={{minHeight:"100vh",background:"#fff",color:"#111",fontFamily:"'Satoshi',sans-serif"}}>
    <div style={{maxWidth:960,margin:"0 auto",padding:"32px 24px"}}>
      ...
      <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11,marginBottom:16}}>
        ...
        <tbody>{(items||[]).map((item,i)=>{const hc=quoteData.hiddenCols||{manuf:true,model:true,shippingEach:true,installEach:true};const ship=item.shippingPerUnit||0;const inst=item.installPerUnit||0;const qty=item.qtyOrdered||0;const price=item.unitPrice||0;const ext=(price+ship+inst)*qty;const cells=[{key:"tag",v:item.tag||""},{key:"manuf",v:item.manufacturer||""},{key:"model",v:item.modelNumber||""},{key:"description",v:item.description||""},{key:"color",v:item.color||""},{key:"qty",v:qty,a:"right"},{key:"shippingEach",v:ship>0?"$"+ship.toFixed(2):"",a:"right"},{key:"shippingTotal",v:ship>0?"$"+(ship*qty).toFixed(2):"",a:"right"},{key:"installEach",v:inst>0?"$"+inst.toFixed(2):"",a:"right"},{key:"installTotal",v:inst>0?"$"+(inst*qty).toFixed(2):"",a:"right"},{key:"unitPrice",v:"$"+price.toFixed(2),a:"right"},{key:"lineTotal",v:"$"+ext.toFixed(2),a:"right",b:true}];return <tr key={i} style={{borderBottom:"1px solid #e5e5e5"}}>{cells.filter(c=>!hc[c.key]).map(c=><td key={c.key} style={{padding:"6px 4px",textAlign:c.a||"left",fontWeight:c.b?600:400}}>{c.v}</td>)}</tr>})}</tbody>
      </table></div>
