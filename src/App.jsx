  ...
  return <div style={{minHeight:"100vh",background:"#fff",color:"#111",fontFamily:"'Satoshi',sans-serif",padding:"40px 20px"}}>
    <div style={{maxWidth:800,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:32}}>
        <div><div style={{fontSize:18,fontWeight:700}}>Midwest Educational Furnishings, Inc.</div><div style={{fontSize:12,color:"#888",lineHeight:1.6}}>21191 N Valley Rd<br/>Kildeer, IL 60047 US<br/>(847) 847-1865</div></div>
        <div style={{textAlign:"right"}}><div style={{fontSize:28,fontWeight:300,color:"#888"}}>Quote</div><div style={{fontSize:14,fontWeight:600,marginTop:4}}>{docNum}</div><div style={{fontSize:12,color:"#888",marginTop:8}}>Date: {new Date().toLocaleDateString()}</div></div>
      </div>
      <div style={{textAlign:"center",marginBottom:20,padding:"8px 0",borderBottom:"2px solid #2dd4bf"}}><div style={{fontSize:12,color:"#888"}}>Designing Spaces | Building Futures | WBE Certified Enterprise</div></div>
      <div style={{display:"flex",gap:40,marginBottom:24}}>
        <div style={{flex:1}}><div style={{fontSize:10,fontWeight:700,color:"#888",textTransform:"uppercase",letterSpacing:1.5,marginBottom:6}}>PREPARED FOR</div><div style={{fontSize:14,fontWeight:600}}>{customer?.name}</div>{customer?.contact&&<div style={{fontSize:12,color:"#888"}}>Attn: {customer.contact}</div>}{customer?.address&&<div style={{fontSize:12,color:"#888"}}>{customer.address}</div>}</div>
        <div style={{textAlign:"right"}}><div style={{fontSize:12,color:"#888"}}>Project: {job?.name}</div><div style={{fontSize:12,color:"#888"}}>Terms: {terms||"Net 30"}</div></div>
      </div>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginBottom:24}}>
        <thead><tr style={{borderBottom:"2px solid #e5e5e5"}}>{["Description","Qty","Unit Price","Shipping","Install","Line Total"].map((h,i)=><th key={i} style={{padding:"8px 6px",textAlign:i>0?"right":"left",fontSize:10,fontWeight:600,color:"#888",textTransform:"uppercase"}}>{h}</th>)}</tr></thead>
        <tbody>{(items||[]).map((item,i)=>{const ship=item.shippingPerUnit||0;const inst=item.installPerUnit||0;const each=(item.unitPrice||0)+ship+inst;return <tr key={i} style={{borderBottom:"1px solid #f0f0f0"}}><td style={{padding:"8px 6px"}}><div style={{fontWeight:500}}>{item.description}</div>{item.modelNumber&&<div style={{fontSize:11,color:"#888"}}>{item.modelNumber}</div>}{item.color&&<div style={{fontSize:11,color:"#888"}}>{item.color}</div>}</td><td style={{padding:"8px 6px",textAlign:"right"}}>{item.qtyOrdered}</td><td style={{padding:"8px 6px",textAlign:"right"}}>{fmt(item.unitPrice||0)}</td><td style={{padding:"8px 6px",textAlign:"right",color:"#888"}}>{ship>0?fmt(ship):""}</td><td style={{padding:"8px 6px",textAlign:"right",color:"#888"}}>{inst>0?fmt(inst):""}</td><td style={{padding:"8px 6px",textAlign:"right",fontWeight:600}}>{fmt(each*(item.qtyOrdered||0))}</td></tr>})}</tbody>
        <tfoot><tr><td colSpan={5} style={{padding:"12px 6px",textAlign:"right",fontWeight:600,fontSize:14,borderTop:"2px solid #e5e5e5"}}>TOTAL</td><td style={{padding:"12px 6px",textAlign:"right",fontWeight:700,fontSize:18,borderTop:"2px solid #e5e5e5"}}>{fmt(total||0)}</td></tr></tfoot>
      </table>
      <div style={{padding:16,background:"#fafafa",borderRadius:8,fontSize:12,color:"#888",marginBottom:24}}>Quote valid for 30 days. Prices subject to manufacturer availability. Payment terms: {terms||"Net 30"}.</div>
      {onApprove&&<div style={{textAlign:"center",padding:"24px 0"}}><button onClick={onApprove} style={{padding:"14px 40px",borderRadius:10,border:"none",background:"#2dd4bf",color:"#000",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(45,212,191,0.3)"}} onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="none"}}>Approve Quote</button><div style={{fontSize:12,color:"#888",marginTop:8}}>By clicking Approve, you authorize Midwest Educational Furnishings to proceed with this order.</div></div>}
    </div>
  </div>;
}
...
function Dashboard({jobs,lineItems,reps,vendors,customers,getJobFinancials,getJobItems,setPage,setSelectedJob,dateFilter,setDateFilter}){
  ...
  const blob=new Blob([wb],{type:"text/csv"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="midwest-full-export-"+new Date().toISOString().split("T")[0]+".csv";a.click();URL.revokeObjectURL(url);notify("Full database exported -- 5 tables")}}
    ...
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12,marginBottom:20}}>
