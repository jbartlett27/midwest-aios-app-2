...
function ShareQuotePortal({quoteData,onApprove}){
  const [approved,setApproved]=useState(quoteData?.approved||false);
  if(!quoteData)return null;
  const {job,customer,items,total,docNum,terms}=quoteData;
  return <div style={{minHeight:"100vh",background:"#fff",color:"#111",fontFamily:"'Satoshi',sans-serif",padding:"40px 20px"}}>
    <div style={{maxWidth:900,margin:"0 auto"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:24,flexWrap:"wrap",gap:16}}>
        <div><div style={{fontSize:18,fontWeight:700}}>Midwest Educational Furnishings, Inc.</div><div style={{fontSize:12,color:"#888",lineHeight:1.6}}>21191 N Valley Rd, Kildeer, IL 60047 US<br/>(847) 847-1865</div></div>
        <div style={{textAlign:"right"}}><div style={{fontSize:28,fontWeight:300,color:"#888"}}>Quote</div>{docNum&&<div style={{fontSize:13,fontWeight:600,marginTop:4}}>{docNum}</div>}<div style={{fontSize:12,color:"#888",marginTop:4}}>Date: {new Date().toLocaleDateString()}</div></div>
      </div>
      <div style={{textAlign:"center",marginBottom:16,padding:"6px 0",borderBottom:"2px solid #2dd4bf",fontSize:12,color:"#888"}}>Designing Spaces | Building Futures | WBE Certified Enterprise</div>
      <div style={{display:"flex",gap:40,marginBottom:20,flexWrap:"wrap"}}>
        <div style={{flex:1,minWidth:200}}><div style={{fontSize:10,fontWeight:700,color:"#888",textTransform:"uppercase",letterSpacing:1.5,marginBottom:4}}>PREPARED FOR</div><div style={{fontSize:14,fontWeight:600}}>{customer?.name||""}</div>{customer?.contact&&<div style={{fontSize:12,color:"#888"}}>Attn: {customer.contact}</div>}</div>
        <div style={{textAlign:"right"}}><div style={{fontSize:12,color:"#888"}}>Project: {job?.name||""}</div><div style={{fontSize:12,color:"#888"}}>Terms: {terms||job?.terms||"Net 30"}</div>{job?.shipTo&&<div style={{fontSize:12,color:"#888"}}>Ship To: {job.shipTo}</div>}</div>
      </div>
      <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginBottom:20}}>
        <thead><tr style={{borderBottom:"2px solid #e5e5e5"}}>{["Description","Mfr","Model #","Color","Qty","Your Price","Shipping Each","Shipping Total","Install Each","Install Total","Line Total"].map((h,i)=><th key={i} style={{padding:"6px 4px",textAlign:i>=4?"right":"left",fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
        <tbody>{(items||[]).map((item,i)=>{const ship=item.shippingPerUnit||0;const inst=item.installPerUnit||0;const qty=item.qtyOrdered||0;const price=item.unitPrice||0;const lineTotal=(price+ship+inst)*qty;return <tr key={i} style={{borderBottom:"1px solid #f0f0f0"}}><td style={{padding:"6px 4px",maxWidth:200}}>{item.description||""}</td><td style={{padding:"6px 4px",color:"#888"}}>{item.manufacturer||""}</td><td style={{padding:"6px 4px",color:"#888"}}>{item.modelNumber||""}</td><td style={{padding:"6px 4px",color:"#888"}}>{item.color||""}</td><td style={{padding:"6px 4px",textAlign:"right"}}>{qty}</td><td style={{padding:"6px 4px",textAlign:"right"}}>${price.toFixed(2)}</td><td style={{padding:"6px 4px",textAlign:"right",color:"#888"}}>{ship>0?"$"+ship.toFixed(2):""}</td><td style={{padding:"6px 4px",textAlign:"right",color:"#888"}}>{ship>0?"$"+(ship*qty).toFixed(2):""}</td><td style={{padding:"6px 4px",textAlign:"right",color:"#888"}}>{inst>0?"$"+inst.toFixed(2):""}</td><td style={{padding:"6px 4px",textAlign:"right",color:"#888"}}>{inst>0?"$"+(inst*qty).toFixed(2):""}</td><td style={{padding:"6px 4px",textAlign:"right",fontWeight:600}}>${lineTotal.toFixed(2)}</td></tr>})}</tbody>
        <tfoot><tr><td colSpan={10} style={{padding:"10px 4px",textAlign:"right",fontWeight:600,fontSize:14,borderTop:"2px solid #e5e5e5"}}>TOTAL</td><td style={{padding:"10px 4px",textAlign:"right",fontWeight:700,fontSize:18,borderTop:"2px solid #e5e5e5"}}>${(total||0).toFixed(2)}</td></tr></tfoot>
      </table></div>
      <div style={{padding:12,background:"#fafafa",borderRadius:8,fontSize:12,color:"#888",marginBottom:24}}>Quote valid for 30 days. Prices subject to manufacturer availability. Payment terms: {terms||job?.terms||"Net 30"}.</div>
      {!approved?<div style={{textAlign:"center",padding:"20px 0"}}><button onClick={()=>{setApproved(true);if(onApprove)onApprove();try{localStorage.setItem('quote_approved_'+docNum,Date.now().toString())}catch{}}} style={{padding:"14px 40px",borderRadius:10,border:"none",background:"#2dd4bf",color:"#000",fontSize:16,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 24px rgba(45,212,191,0.3)"}} onMouseLeave={e=>{e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="none"}}>Approve Quote</button><div style={{fontSize:12,color:"#888",marginTop:8}}>By clicking Approve, you authorize Midwest Educational Furnishings to proceed.</div></div>
      :<div style={{textAlign:"center",padding:"24px 0"}}><div style={{display:"inline-flex",alignItems:"center",gap:8,padding:"12px 32px",background:"#d1fae5",borderRadius:10,color:"#065f46",fontSize:16,fontWeight:700}}>&#10003; Quote Approved</div><div style={{fontSize:12,color:"#888",marginTop:8}}>Thank you. Midwest Educational Furnishings has been notified.</div></div>}
    </div>
  </div>;
}
...
export default function MidwestAIOS() {
  ...
  const addSop = (sop) => { setCustomSops(p => [...p, sop]); db.saveSop(sop); };
  ...
  return (
    <div style={{display:"flex",height:"100vh",width:"100vw",fontFamily:"'Satoshi','Satoshi',sans-serif",background:"#000000",color:"#d4d4d4",overflow:"hidden"}}>
      ...
      {/* Mobile hamburger */}
      {!mobileMenuOpen&&<div className="mobile-menu-btn" style={{display:"none",position:"fixed",bottom:16,left:"50%",transform:"translateX(-50%)",zIndex:10001,background:"rgba(18,18,18,0.72)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:28,padding:"8px 6px",backdropFilter:"blur(40px) saturate(180%)",WebkitBackdropFilter:"blur(40px) saturate(180%)",boxShadow:"0 8px 40px rgba(0,0,0,0.45), inset 0 0.5px 0 rgba(255,255,255,0.06)"}}><div style={{display:"flex",gap:2}}>{[{id:"dashboard",icon:"dashboard",l:"Home"},{id:"jobs",icon:"briefcase",l:"Jobs"},{id:"documents",icon:"file",l:"Docs"},{id:"deliveries",icon:"truck",l:"Track"},{id:"brain",icon:"brain",l:"AI"}].map(t=><button key={t.id} onClick={()=>{setPage(t.id);setSelectedJob(null)}} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"6px 14px",borderRadius:20,border:"none",background:page===t.id?"rgba(45,212,191,0.15)":"transparent",color:page===t.id?"#2dd4bf":"#525252",cursor:"pointer",fontFamily:"'Satoshi',sans-serif",fontSize:9,fontWeight:page===t.id?600:400,transition:"all 0.15s"}}><I n={t.icon} s={18}/>{t.l}</button>)}<button onClick={()=>setMobileMenuOpen(true)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"6px 14px",borderRadius:20,border:"none",background:"transparent",color:"#525252",cursor:"pointer",fontFamily:"'Satoshi',sans-serif",fontSize:9,fontWeight:400}}><I n="settings" s={18}/>More</button></div></div>}
...
function Dashboard({jobs,lineItems,reps,vendors,customers,getJobFinancials,getJobItems,setPage,setSelectedJob,dateFilter,setDateFilter}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><div style={{marginBottom:36}}><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8,flexWrap:"wrap"}}><h1 style={{fontSize:32,fontWeight:800,color:"#f0f0f0",letterSpacing:-1.5}}>Command Center</h1><div style={{padding:"4px 10px",background:"#34d399",borderRadius:20,fontSize:12,fontWeight:700,color:"#fff",letterSpacing:0.5}}>LIVE</div>
    <div style={{marginLeft:"auto",display:"flex",gap:4,background:"#111111",padding:3,borderRadius:8,flexShrink:0}}>{[["all","All Time"],["ytd","YTD"],["quarter","Quarter"],["month","Month"]].map(([v,l])=> <button key={v} onClick={()=>setDateFilter(v)} style={{padding:"5px 12px",borderRadius:6,border:"none",cursor:"pointer",background:dateFilter===v?"#2dd4bf":"transparent",color:dateFilter===v?"#000000":"#525252",fontSize:12,fontWeight:dateFilter===v?600:400,fontFamily:"inherit"}}>{l}</button>)}</div>
  </div><div className="dash-exports" style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap"}}>{[
  ...
  const blob=new Blob([wb],{type:"text/csv"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="midwest-full-export-"+new Date().toISOString().split("T")[0]+".csv";a.click();URL.revokeObjectURL(url);notify("Full database exported -- 5 tables")}}
    ...
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12,marginBottom:20,alignItems:"start"}}>
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:10,display:"flex",alignItems:"center",gap:8}}><I n="alert" s={14}/> Action Required</div><div style={{display:"flex",flexDirection:"column",gap:10}}>
    ...
    {/* Financial Charts */}
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:16,marginBottom:24,alignItems:"start"}}>
      <Card style={{padding:18}}><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:16}}>Revenue by Month</div>
        <ResponsiveContainer width="100%" height={180}><BarChart data={(()=>{const m={};filtered.forEach(j=>{const d=new Date(j.createdDate);if(!isNaN(d.getTime())){const key=d.toLocaleString("default",{month:"short"});m[key]=(m[key]||0)+getJobFinancials(j.id).totalRevenue}});return Object.entries(m).map(([name,rev])=>({name,rev}))})()}>
          <XAxis dataKey="name" tick={{fill:"#525252",fontSize:11}} axisLine={false} tickLine={false}/>
          <YAxis tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>"$"+Math.round(v/1000)+"k"}/>
          <Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,fontSize:12}} labelStyle={{color:"#737373"}} formatter={v=>fmt(v)}/>
          <RBar dataKey="rev" fill="#2dd4bf" radius={[4,4,0,0]}/>
        </BarChart></ResponsiveContainer>
      </Card>
      <Card style={{padding:18}}><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:16}}>Margin Trend</div>
        <ResponsiveContainer width="100%" height={180}><LineChart data={(()=>{const m={};filtered.forEach(j=>{const d=new Date(j.createdDate);if(!isNaN(d.getTime())){const key=d.toLocaleString("default",{month:"short"});if(!m[key])m[key]={sum:0,count:0};m[key].sum+=getJobFinancials(j.id).margin;m[key].count++}});return Object.entries(m).map(([name,d])=>({name,margin:d.sum/d.count}))})()}>
          <XAxis dataKey="name" tick={{fill:"#525252",fontSize:11}} axisLine={false} tickLine={false}/>
          <YAxis tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>v.toFixed(0)+"%"}/>
          <Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,fontSize:12}} formatter={v=>pct(v)}/>
          <Line type="monotone" dataKey="margin" stroke="#34d399" strokeWidth={2} dot={{fill:"#34d399",r:3}}/>
        </LineChart></ResponsiveContainer>
      </Card>
      <Card style={{padding:18}}><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:16}}>Vendor Spend</div>
        <ResponsiveContainer width="100%" height={180}><PieChart><Pie data={(()=>{const m={};fLineItems.forEach(i=>{const v=vendors.find(v=>v.id===i.vendor);const name=v?.name||"Other";m[name]=(m[name]||0)+i.unitCost*i.qtyOrdered});return Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([name,value])=>({name,value}))})()}
          cx="50%" cy="50%" innerRadius={45} outerRadius={70} paddingAngle={2} dataKey="value">
          {["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"].map((c,i)=><Cell key={i} fill={c}/>)}
        </Pie><Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,fontSize:12}} formatter={v=>fmt(v)}/></PieChart></ResponsiveContainer>
        <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:8,justifyContent:"center"}}>{(()=>{const m={};fLineItems.forEach(i=>{const v=vendors.find(v=>v.id===i.vendor);const name=v?.name||"Other";m[name]=(m[name]||0)+i.unitCost*i.qtyOrdered});return Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([name],i)=><span key={name} style={{fontSize:10,color:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i],display:"flex",alignItems:"center",gap:3}}><div style={{width:6,height:6,borderRadius:"50%",background:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i]}}/>{name}</span>)})()}</div>
      </Card>
      <Card style={{padding:18}}><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:16}}>Payment Collection</div>
        <ResponsiveContainer width="100%" height={180}><BarChart data={(()=>{const paid=filtered.filter(j=>j.paymentStatus==="paid");const partial=filtered.filter(j=>j.paymentStatus==="partial");const unpaid=filtered.filter(j=>j.paymentStatus==="unpaid");return[{name:"Paid",value:paid.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0),fill:"#34d399"},{name:"Partial",value:partial.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0),fill:"#fbbf24"},{name:"Unpaid",value:unpaid.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0),fill:"#f87171"}]})()}>
          <XAxis dataKey="name" tick={{fill:"#525252",fontSize:11}} axisLine={false} tickLine={false}/>
          <YAxis tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>"$"+Math.round(v/1000)+"k"}/>
          <Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid rgba(255,255,255,0.08)",borderRadius:8,fontSize:12}} formatter={v=>fmt(v)}/>
          <RBar dataKey="value" radius={[4,4,0,0]}>{[{fill:"#34d399"},{fill:"#fbbf24"},{fill:"#f87171"}].map((c,i)=><Cell key={i} fill={c.fill}/>)}</RBar>
        </BarChart></ResponsiveContainer>
      </Card>
    </div>

    <Card><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:10,display:"flex",alignItems:"center",gap:8}}><I n="briefcase" s={14}/> Active Jobs</div>{jobs.length===0&&<div style={{textAlign:"center",padding:20,color:"#a3a3a3",fontSize:13}}>No jobs yet. Go to Job Records to create your first project.</div>}<div style={{display:"flex",flexDirection:"column",gap:8}}>{jobs.map(job=>{const f=getJobFinancials(job.id);return <div key={job.id} onClick={()=>{setSelectedJob(job.id);setPage("jobs")}} className="slide-row hover-lift" style={{padding:"10px 12px",background:"#1a1a1a",borderRadius:8,cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={e=>e.currentTarget.style.background="#222222"} onMouseLeave={e=>e.currentTarget.style.background="#1a1a1a"}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13,fontWeight:600,color:"#d4d4d4"}}>{job.name}</span><Badge label={job.phase} color={statusColor(job.phase)}/></div><Bar value={f.totalReceived} max={f.totalOrdered||1} color={statusColor(job.phase)} height={4}/><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:12,color:"#a3a3a3"}}>{fmtN(f.totalReceived)}/{fmtN(f.totalOrdered)} units</span><span style={{fontSize:12,color:"#a3a3a3"}}>{fmt(f.totalRevenue)}</span></div></div>})}</div></Card>
    </div>
    {/* --- PIPELINE DONUT + REVENUE BREAKDOWN ---------------- */}
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:24,marginBottom:32}}>
  ...
  <text x={cx} y={cy+10} textAnchor="middle" fill="#555" fontSize={9} letterSpacing={2}>PIPELINE</text>
    ...
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:24}}>
    ...
    {tab==="preview"&&previewDoc&&<Card style={{border:"1px solid #2dd4bf30"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{fontSize:16,fontWeight:700,color:"#2dd4bf"}}>{previewDoc.type==="quote"?"PROJECT QUOTE":previewDoc.type==="po"?"PURCHASE ORDER":previewDoc.type==="commission"?"COMMISSION STATEMENT":previewDoc.data?.isPartial?"PARTIAL INVOICE":"INVOICE"}</div><StatusBadge docNum={previewDoc.data.docNum}/></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{previewDoc.data.docNum&&<div style={{display:"flex",gap:4}}>{["drafted","sent","approved"].map(s=><button key={s} onClick={()=>setDocStatus(previewDoc.data.docNum,s)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid "+(docStatuses[previewDoc.data.docNum]===s?"#2dd4bf":"#333333"),background:docStatuses[previewDoc.data.docNum]===s?"#2dd4bf20":"transparent",color:docStatuses[previewDoc.data.docNum]===s?"#2dd4bf":"#525252",fontSize:12,fontFamily:"inherit",cursor:"pointer",textTransform:"capitalize"}}>{s}</button>)}</div>}<Btn onClick={()=>handleExportPDF(previewDoc)}><I n="download" s={14}/> Export PDF</Btn>{previewDoc.type==="quote"&&<Btn v="ghost" onClick={()=>{const data=btoa(JSON.stringify({job:{name:previewDoc.job.name,terms:previewDoc.job.terms,shipTo:previewDoc.job.shipTo},customer:previewDoc.data.customer,items:previewDoc.data.items.map(it=>({description:it.description,manufacturer:it.manufacturer||"",modelNumber:it.modelNumber||"",color:it.color||"",qtyOrdered:it.qtyOrdered,unitPrice:it.unitPrice,shippingPerUnit:it.shippingPerUnit||0,installPerUnit:it.installPerUnit||0})),total:previewDoc.data.total,docNum:previewDoc.data.docNum}));const url=window.location.origin+window.location.pathname+"#quote="+data;window.open(url,"_blank");navigator.clipboard.writeText(url).catch(()=>{})}}><I n="send" s={14}/> Share Link</Btn>}<Btn v="ghost" onClick={()=>{const doc=previewDoc;const isQuote=doc.type==="quote";const isPO=doc.type==="po";const isComm=doc.type==="commission";const recipient=isQuote?doc.data.customer?.email:isPO?doc.data.vendor?.email:isComm?doc.data.rep?.email:doc.data.customer?.email;setEmailTo(recipient||"");setEmailSubject((isQuote?"Project Quote":isPO?"Purchase Order":isComm?"Commission Statement":"Invoice")+" - "+doc.job.name+" - "+(doc.data.docNum||""));setEmailModal(doc)}}><I n="send" s={14}/> Email</Btn>{previewDoc.type!=="quote"&&previewDoc.type!=="commission"&&<Btn v={qbConfig.connected?"primary":"secondary"} onClick={()=>qbConfig.connected?pushToQB(previewDoc):setPage("qbsetup")} style={pushing?{opacity:0.6,pointerEvents:"none"}:{}}><I n="send" s={14}/> {pushing?"Pushing...":qbConfig.connected?"Push to QB":"Connect QB"}</Btn>}<Btn v="secondary" onClick={()=>setTab(previewDoc.type==="commission"?"commissions":previewDoc.type==="quote"?"quotes":previewDoc.type==="po"?"pos":"invoices")}>&larr; Back</Btn>{previewDoc.type==="invoice"&&previewDoc.data.items.length>0&&<Btn v="ghost" style={{fontSize:12,color:"#34d399",border:"1px solid #05966930"}} onClick={()=>{previewDoc.data.items.forEach(item=>{if(item.qtyReceived>item.qtyInvoiced){ctx.updateLineItem(item.id,{qtyInvoiced:item.qtyReceived,invoiceDate:new Date().toISOString().split("T")[0]})}});notify("All "+previewDoc.data.items.length+" items marked as invoiced")}}>Mark All Invoiced</Btn>}</div></div>
      ...
      <div style={{background:"#fff",color:"#111",borderRadius:12,padding:40,fontFamily:"'Satoshi',sans-serif",maxWidth:800,margin:"0 auto"}}>
        ...
        <div style={{overflowX:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
          <thead><tr>{(previewDoc.type==="quote"?["Tag","Manuf.","Model #","Description","Color","Qty","Shipping Each","Shipping Total","Install Each","Install Total","Unit Price","Line Total"]:previewDoc.type==="commission"?["Job","Customer","Revenue","Status","Commission"]:["Description","Qty","Rate","Amount"]).map((h,i)=><th key={i} style={{padding:"8px 6px",textAlign:i<5&&previewDoc.type==="quote"?"left":"right",fontSize:10,fontWeight:600,color:"#888",textTransform:"uppercase",letterSpacing:0.5,borderBottom:"2px solid #e5e5e5"}}>{h}</th>)}</tr></thead>
...
function EditTaskOverlay({task,setEditTask,jobs,reps,saveTask,deleteSop,notify,inputStyle}){
  ...
  return <div onClick={()=>{if(t.text?.trim()){saveTask(t,t.isNew?null:t.sopId)};setEditTask(null)}} style={{position:"fixed",inset:0,zIndex:9999,background:"rgba(10,10,10,0.75)",backdropFilter:"blur(40px) saturate(180%)",WebkitBackdropFilter:"blur(40px) saturate(180%)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
