...
function Dashboard({jobs,lineItems,reps,vendors,customers,getJobFinancials,getJobItems,setPage,setSelectedJob,dateFilter,setDateFilter}){
  ...
  const blob=new Blob([wb],{type:"text/csv"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="midwest-full-export-"+new Date().toISOString().split("T")[0]+".csv";a.click();URL.revokeObjectURL(url);notify("Full database exported -- 5 tables")}}
    ...
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12,marginBottom:16,alignItems:"start"}}>
    ...
    {/* Financial Charts */}
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:12,marginBottom:20}}>
      <Card style={{padding:16}}><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:16}}>Revenue by Month</div>
        <ResponsiveContainer width="100%" height={160}><BarChart data={(()=>{const m={};filtered.forEach(j=>{const d=new Date(j.createdDate);if(!isNaN(d.getTime())){const key=d.toLocaleString("default",{month:"short"});m[key]=(m[key]||0)+getJobFinancials(j.id).totalRevenue}});return Object.entries(m).map(([name,rev])=>({name,rev}))})()}>
      ...
      <Card style={{padding:18}}><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:16}}>Vendor Spend</div>
        <ResponsiveContainer width="100%" height={160}><PieChart><Pie data={(()=>{const m={};fLineItems.forEach(i=>{const v=vendors.find(v=>v.id===i.vendor);const name=v?.name||"Other";m[name]=(m[name]||0)+i.unitCost*i.qtyOrdered});return Object.entries(m).sort((a,b)=>b[1]-a[1]).slice(0,6).map(([name,value])=>({name,value}))})()}
      ...
      <Card style={{padding:18}}><div style={{fontSize:14,fontWeight:700,color:"#f0f0f0",marginBottom:16}}>Payment Collection</div>
        <ResponsiveContainer width="100%" height={160}><BarChart data={(()=>{const paid=filtered.filter(j=>j.paymentStatus==="paid");const partial=filtered.filter(j=>j.paymentStatus==="partial");const unpaid=filtered.filter(j=>j.paymentStatus==="unpaid");return[{name:"Paid",value:paid.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0),fill:"#34d399"},{name:"Partial",value:partial.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0),fill:"#fbbf24"},{name:"Unpaid",value:unpaid.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0),fill:"#f87171"}]})()}>
          <XAxis dataKey="name" tick={{fill:"#525252",fontSize:11}} axisLine={false} tickLine={false}/>
          <YAxis tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false} tickFormatter={v=>"$"+Math.round(v/1000)+"k"}/>
