...
export default function MidwestAIOS() {
  ...
  return (
    <div style={{display:"flex",height:"100vh",width:"100vw",fontFamily:"'Satoshi','Satoshi',sans-serif",background:"#000000",color:"#d4d4d4",overflow:"hidden"}}>
      ...
      {/* Mobile hamburger */}
      {!mobileMenuOpen&&<div className="mobile-menu-btn" style={{display:"none",position:"fixed",bottom:16,left:"50%",transform:"translateX(-50%)",zIndex:10001,background:"rgba(12,12,12,0.35)",border:"1px solid rgba(255,255,255,0.18)",borderRadius:28,padding:"8px 6px",backdropFilter:"blur(16px) saturate(220%) brightness(1.15) contrast(1.05)",WebkitBackdropFilter:"blur(16px) saturate(220%) brightness(1.15) contrast(1.05)",boxShadow:"0 8px 40px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.1), inset 0 -1px 0 rgba(255,255,255,0.03)"}}><div style={{display:"flex",gap:2}}>{[{id:"dashboard",icon:"dashboard",l:"Home"},{id:"jobs",icon:"briefcase",l:"Jobs"},{id:"documents",icon:"file",l:"Docs"},{id:"deliveries",icon:"truck",l:"Track"},{id:"brain",icon:"brain",l:"AI"}].map(t=><button key={t.id} onClick={()=>{setPage(t.id);setSelectedJob(null)}} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"6px 14px",borderRadius:20,border:"none",background:page===t.id?"rgba(45,212,191,0.22)":"transparent",color:page===t.id?"#2dd4bf":"#636363",cursor:"pointer",fontFamily:"'Satoshi',sans-serif",fontSize:page===t.id?10:9,fontWeight:page===t.id?700:400,transition:"all 0.4s cubic-bezier(0.34,1.56,0.64,1)",transform:page===t.id?"scale(1.18) translateY(-3px)":"scale(0.95) translateY(0)",transformOrigin:"center center",boxShadow:page===t.id?"0 6px 20px rgba(45,212,191,0.25)":"none"}}><I n={t.icon} s={page===t.id?22:17}/>{t.l}</button>)}<button onClick={()=>setMobileMenuOpen(true)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"6px 14px",borderRadius:20,border:"none",background:"transparent",color:"#525252",cursor:"pointer",fontFamily:"'Satoshi',sans-serif",fontSize:9,fontWeight:400}}><I n="settings" s={18}/>More</button></div></div>}
...
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,getJobFinancials,updateJob,updateLineItem,notify,qbConfig,setPage,triggerPrint,pendingCommPreview,setPendingCommPreview,...ctx}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices, Commission Statements"/>
    ...
    {tab==="quotes"&&<div>{jobs.map(job=>{const q=genQuote(job);const customer=customers.find(c=>c.id===job.customer);return <Card key={job.id} style={{marginBottom:10,padding:14}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,flexWrap:"wrap"}}><div style={{flex:1,minWidth:180}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}><span style={{fontSize:14,fontWeight:700,color:"#e5e5e5"}}>{job.name}</span><Badge label={job.phase} color={statusColor(job.phase)}/></div><div style={{fontSize:12,color:"#a3a3a3"}}>{customer?.name} - {q.items.length} items - {fmt(q.total)}</div></div><div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}><StatusBadge docNum={q.docNum}/>{docStatuses[q.docNum]==="approved"&&<span style={{fontSize:11,color:"#34d399",padding:"2px 8px",background:"#34d39910",borderRadius:6}}>POs unlocked</span>}<Btn style={{fontSize:12,padding:"6px 12px"}} onClick={()=>{setPreviewDoc({type:"quote",data:q,job});setTab("preview")}}><I n="file" s={12}/> Quote</Btn></div></div></Card>})}</div>}
