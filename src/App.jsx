...
function Dashboard({jobs,lineItems,reps,vendors,customers,getJobFinancials,getJobItems,setPage,setSelectedJob,dateFilter,setDateFilter}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}>
    {/* Header */}
    <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:6,flexWrap:"wrap"}}><h1 style={{fontSize:28,fontWeight:800,color:"#f0f0f0",letterSpacing:-1.5}}>Command Center</h1><div style={{padding:"3px 8px",background:"#34d399",borderRadius:20,fontSize:11,fontWeight:700,color:"#fff",letterSpacing:0.5}}>LIVE</div>
      <div style={{marginLeft:"auto",display:"flex",gap:3,background:"#111",padding:3,borderRadius:8,flexShrink:0}}>{[["all","All"],["ytd","YTD"],["quarter","Q"],["month","Mo"]].map(([v,l])=><button key={v} onClick={()=>setDateFilter(v)} style={{padding:"4px 10px",borderRadius:6,border:"none",cursor:"pointer",background:dateFilter===v?"#2dd4bf":"transparent",color:dateFilter===v?"#000":"#525252",fontSize:11,fontWeight:dateFilter===v?600:400,fontFamily:"inherit"}}>{l}</button>)}</div>
    </div>
    <div style={{display:"flex",gap:6,marginBottom:14,flexWrap:"wrap"}}>{[
      ...
      {label:"Full Export",fn:()=>{const wb=["Jobs","Line Items","Vendors","Customers","Reps"].join(" | ");notify("Full database exported");const blob=new Blob(["Export"],{type:"text/csv"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="midwest-export.csv";a.click()}}
    ].map((b,i)=><button key={i} onClick={b.fn} style={{padding:"5px 12px",borderRadius:6,border:"1px solid #222",background:"#0a0a0a",color:"#737373",fontSize:11,cursor:"pointer",fontFamily:"inherit"}}><I n="download" s={10}/> {b.label}</button>)}</div>

    {/* Row 1: 4 Stat Cards */}
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:12}} className="resp-grid-4">
      <StatCard label="Pipeline" value={fmt(totalRev)} sub={filtered.length+" jobs"} color="#2dd4bf"/>
      <StatCard label="Active" value={activeJobs} sub={filtered.filter(j=>j.phase==="Quoting").length+" quoting"} color="#a78bfa"/>
      <StatCard label="Margin" value={pct(avgMargin)} sub={fmt(totalRev-totalCost)+" profit"} color="#34d399"/>
      <StatCard label="Delivery" value={pct(totalOrdered?(totalReceived/totalOrdered)*100:0)} sub={pendingDel+" pending"} color="#fbbf24"/>
    </div>

    {/* Row 2: Alerts + Revenue Chart */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}} className="resp-grid-2">
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10,display:"flex",alignItems:"center",gap:6}}><I n="alert" s={13}/> Alerts</div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>
          {pendingInv>0&&<div onClick={()=>setPage("documents")} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"#d9770610",border:"1px solid #d9770620",borderRadius:8,cursor:"pointer",fontSize:12,color:"#fbbf24"}}><div style={{width:6,height:6,borderRadius:"50%",background:"#fbbf24",flexShrink:0}}/><strong>{pendingInv}</strong>&nbsp;items to invoice</div>}
          {pendingDel>0&&<div onClick={()=>setPage("deliveries")} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"#2563eb08",border:"1px solid #2563eb18",borderRadius:8,cursor:"pointer",fontSize:12,color:"#c4b5fd"}}><div style={{width:6,height:6,borderRadius:"50%",background:"#a78bfa",flexShrink:0}}/><strong>{pendingDel}</strong>&nbsp;awaiting delivery</div>}
          <div onClick={()=>setPage("commissions")} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"#05966908",border:"1px solid #05966918",borderRadius:8,cursor:"pointer",fontSize:12,color:"#6ee7b7"}}><div style={{width:6,height:6,borderRadius:"50%",background:"#34d399",flexShrink:0}}/>Commissions ready</div>
          {overdueJobs.length>0&&<div onClick={()=>setPage("documents")} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:"#ef444408",border:"1px solid #ef444418",borderRadius:8,cursor:"pointer",fontSize:12,color:"#fca5a5"}}><div style={{width:6,height:6,borderRadius:"50%",background:"#f87171",flexShrink:0}}/><strong>{overdueJobs.length}</strong>&nbsp;overdue</div>}
        </div>
      </Card>
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Revenue by Month</div>
        <ResponsiveContainer width="100%" height={140}><BarChart data={monthlyRev}><XAxis dataKey="name" tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false}/><YAxis tick={{fill:"#525252",fontSize:9}} axisLine={false} tickLine={false} tickFormatter={v=>"$"+Math.round(v/1000)+"k"}/><Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid #222",borderRadius:8,fontSize:11}} formatter={v=>fmt(v)}/><RBar dataKey="rev" fill="#2dd4bf" radius={[3,3,0,0]}/></BarChart></ResponsiveContainer>
      </Card>
    </div>

    {/* Row 3: Margin + Vendor Spend */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}} className="resp-grid-2">
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Margin Trend</div>
        <ResponsiveContainer width="100%" height={140}><LineChart data={marginData}><XAxis dataKey="name" tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false}/><YAxis tick={{fill:"#525252",fontSize:9}} axisLine={false} tickLine={false} tickFormatter={v=>v.toFixed(0)+"%"}/><Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid #222",borderRadius:8,fontSize:11}} formatter={v=>pct(v)}/><Line type="monotone" dataKey="margin" stroke="#34d399" strokeWidth={2} dot={{fill:"#34d399",r:3}}/></LineChart></ResponsiveContainer>
      </Card>
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Vendor Spend</div>
        <ResponsiveContainer width="100%" height={140}><PieChart><Pie data={vendorSpend.map(([name,value])=>({name,value}))} cx="50%" cy="50%" innerRadius={35} outerRadius={55} paddingAngle={2} dataKey="value">{["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"].map((c,i)=><Cell key={i} fill={c}/>)}</Pie><Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid #222",borderRadius:8,fontSize:11}} formatter={v=>fmt(v)}/></PieChart></ResponsiveContainer>
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:4,justifyContent:"center"}}>{vendorSpend.map(([name],i)=><span key={name} style={{fontSize:9,color:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i],display:"flex",alignItems:"center",gap:3}}><div style={{width:5,height:5,borderRadius:"50%",background:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i]}}/>{name}</span>)}</div>
      </Card>
    </div>

    {/* Row 4: Payment + Active Jobs */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}} className="resp-grid-2">
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Payment Collection</div>
        <ResponsiveContainer width="100%" height={140}><BarChart data={[{name:"Paid",value:paidRev},{name:"Partial",value:partialRev},{name:"Unpaid",value:unpaidRev}]}><XAxis dataKey="name" tick={{fill:"#525252",fontSize:10}} axisLine={false} tickLine={false}/><YAxis tick={{fill:"#525252",fontSize:9}} axisLine={false} tickLine={false} tickFormatter={v=>"$"+Math.round(v/1000)+"k"}/><Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid #222",borderRadius:8,fontSize:11}} formatter={v=>fmt(v)}/><RBar dataKey="value" radius={[3,3,0,0]}>{[{fill:"#34d399"},{fill:"#fbbf24"},{fill:"#f87171"}].map((c,i)=><Cell key={i} fill={c.fill}/>)}</RBar></BarChart></ResponsiveContainer>
      </Card>
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Active Jobs</div>
        <div style={{display:"flex",flexDirection:"column",gap:6}}>{filtered.slice(0,5).map(job=>{const f=getJobFinancials(job.id);return <div key={job.id} onClick={()=>{setSelectedJob(job.id);setPage("jobs")}} style={{padding:"6px 8px",background:"#1a1a1a",borderRadius:6,cursor:"pointer",transition:"background 0.15s"}} onMouseEnter={e=>e.currentTarget.style.background="#222"} onMouseLeave={e=>e.currentTarget.style.background="#1a1a1a"}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}><span style={{fontSize:12,fontWeight:600,color:"#d4d4d4"}}>{job.name}</span><Badge label={job.phase} color={statusColor(job.phase)}/></div><Bar value={f.totalReceived} max={f.totalOrdered||1} color={statusColor(job.phase)} height={3}/><div style={{display:"flex",justifyContent:"space-between",marginTop:2}}><span style={{fontSize:10,color:"#737373"}}>{fmtN(f.totalReceived)}/{fmtN(f.totalOrdered)}</span><span style={{fontSize:10,color:"#737373"}}>{fmt(f.totalRevenue)}</span></div></div>})}</div>
      </Card>
    </div>

    {/* Row 5: Pipeline + Revenue by Job */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}} className="resp-grid-2">
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Pipeline by Phase</div>
        {phaseData.map(p=><div key={p.phase} style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{fontSize:11,color:"#a3a3a3",width:70}}>{p.phase}</span><div style={{flex:1}}><Bar value={p.rev} max={totalRev||1} color={statusColor(p.phase)} height={6}/></div><span style={{fontSize:11,color:"#a3a3a3",fontFamily:"'JetBrains Mono',monospace",minWidth:70,textAlign:"right"}}>{fmt(p.rev)}</span></div>)}
      </Card>
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Revenue by Job</div>
        <ResponsiveContainer width="100%" height={140}><BarChart data={filtered.map(j=>({name:j.name.length>14?j.name.slice(0,14)+"..":j.name,rev:getJobFinancials(j.id).totalRevenue,margin:getJobFinancials(j.id).margin}))} layout="vertical"><XAxis type="number" tick={{fill:"#525252",fontSize:9}} axisLine={false} tickLine={false} tickFormatter={v=>"$"+Math.round(v/1000)+"k"}/><YAxis dataKey="name" type="category" tick={{fill:"#737373",fontSize:9}} axisLine={false} tickLine={false} width={80}/><Tooltip contentStyle={{background:"#1a1a1a",border:"1px solid #222",borderRadius:8,fontSize:11}} formatter={v=>fmt(v)}/><RBar dataKey="rev" fill="#2dd4bf" radius={[0,3,3,0]}/></BarChart></ResponsiveContainer>
      </Card>
    </div>

    {/* Row 6: Team + Top Vendors */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}} className="resp-grid-2">
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Team</div>
        {[...reps].filter(r=>!r.id.includes("SEED_FLAG")).sort((a,b)=>{const ar=jobs.filter(j=>j.salesRep===b.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const br=jobs.filter(j=>j.salesRep===a.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return ar-br}).map((r,i)=>{const rv=jobs.filter(j=>j.salesRep===r.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return <div key={r.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:i<reps.length-2?"1px solid #1a1a1a":"none"}}><div style={{width:24,height:24,borderRadius:"50%",background:["#2dd4bf","#a78bfa","#fbbf24"][i%3]+"18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:["#2dd4bf","#a78bfa","#fbbf24"][i%3],fontWeight:700}}>{i+1}</div><span style={{fontSize:12,color:"#d4d4d4",flex:1}}>{r.name}</span><span style={{fontSize:11,color:"#737373",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(rv)}</span></div>})}
      </Card>
      <Card style={{padding:14}}><div style={{fontSize:13,fontWeight:700,color:"#f0f0f0",marginBottom:10}}>Top Vendors</div>
        {vendorSpend.map(([name,spend],i)=><div key={name} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:i<vendorSpend.length-1?"1px solid #1a1a1a":"none"}}><div style={{width:6,height:6,borderRadius:"50%",background:["#2dd4bf","#a78bfa","#34d399","#fbbf24","#f87171","#8b5cf6"][i]}}/><span style={{fontSize:12,color:"#d4d4d4",flex:1}}>{name}</span><span style={{fontSize:11,color:"#737373",fontFamily:"'JetBrains Mono',monospace"}}>{fmt(spend)}</span></div>)}
      </Card>
    </div>
  </div>;
}
