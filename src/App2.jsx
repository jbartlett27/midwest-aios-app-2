  ...
  return <div style={{animation:"fadeUp 0.4s"}}>
    ...
    {editing&&<Card style={{marginBottom:20,border:"1px solid #2dd4bf30"}}><div style={{fontSize:15,fontWeight:700,marginBottom:14,color:"#2dd4bf"}}>Edit Customer</div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12,marginBottom:14}}>
        <div><label style={{fontSize:12,color:"#9a9a9a",display:"block",marginBottom:4}}>Type</label><select value={editForm.type||""} onChange={e=>setEditForm({...editForm,type:e.target.value})} style={inputStyle}>{customerTypeOptions(editForm.type).map(t=><option key={t}>{t}</option>)}</select></div>
...
function BrainPage({jobs,reps,lineItems,vendors,customers,getJobFinancials,getJobItems,_commissionFor,_bankTxnHash,brainQuery,setBrainQuery,customSops,addSop,deleteSop,brainLoading,setBrainLoading,brainHistory,setBrainHistory,updateJob,addJob,updateLineItem,addLineItem,deleteLineItem,updateRep,addRep,addCustomer,updateCustomer,addVendor,updateVendor,notify,setPage,deleteJob,pendingBrainFile,setPendingBrainFile,pendingBrainEmail,setPendingBrainEmail,currentUser}){
  ...
  const brainTools=[
    ...
    {name:"create_standalone_bill",description:"Create a standalone vendor bill not tied to a PO. ONLY for charges with no purchase order behind them (one-off vendor charges, freight that arrived without a PO, rebill scenarios). IMPORTANT: standalone bills ADD cost on top of the job's line items. If the job has a PO for this vendor the call is BLOCKED and you must use record_bill_against_po instead (second/third invoices from the same vendor belong on the PO bill, not as standalone). Pass force_standalone: true only for a genuine extra charge with no PO behind it.",input_schema:{type:"object",properties:{vendor_name:{type:"string"},job_id:{type:"string"},amount:{type:"number"},bill_date:{type:"string",description:"YYYY-MM-DD (defaults to today)"},ref_number:{type:"string",description:"Vendor invoice number"},memo:{type:"string"},file_url:{type:"string",description:"Optional URL of the bill PDF to attach"},force_standalone:{type:"boolean",description:"Set true ONLY after confirming this is a true extra charge not covered by any PO on the job -- overrides the duplicate-cost guard"}},required:["vendor_name","job_id","amount"]}},
  ...
  const executeTool=async(toolName,input)=>{
    try{
      ...
      if (toolName === 'create_vendor_credit' || toolName === 'create_standalone_bill') {
        ...
        // be -- line items on that PO, so a standalone entry double-counts the cost.
        // This is the recurring McHenry-style problem Maureen reported Jul 23 2026.
        if (toolName === 'create_standalone_bill' && input.force_standalone !== true) {
          const _dupPO = _genPOsForJob(job).find(p => p.vendor && p.vendor.id === v.id && (p.items||[]).reduce((s2,i2)=>s2+((i2.unitCost||0)*(Number(i2.qtyOrdered)||0)),0) > 0);
          if (_dupPO) return {error: 'BLOCKED to prevent a double-count: '+job.name+' already has a PO for '+v.name+' ('+_dupPO.docNum+'). If this invoice covers items already on the job, record it against the PO instead: record_bill_against_po (add the invoice number via vendor_invoice_num), then mark_bill_paid when paid. If the invoice is for NEW items not on the job yet, add them as line items first so they flow through the PO. Only if this is a true extra charge with no PO behind it, retry with force_standalone: true.'};
        }
  ...
  const handleCsv=async(e)=>{
    ...
    try{
      ...
      const ci={name:col(['name','full name']),title:col(['title','job title']),company:col(['company','organization']),email:col(['email']),
        ...
        // Generic fallback: a column named just "Phone" / "Telephone" / "Cell" matched
        // NONE of the specific patterns above, so reformatted spreadsheets silently
        // lost every phone number. Reported by Maureen Aug 1 2026.
        anyPhone:col(['phone','telephone','cell']),
        linkedin:col(['linkedin']),compLinkedin:col(['company linkedin']),website:col(['website']),twitter:col(['twitter']),
        emp:col(['employee','# employees']),city:col(['city']),state:col(['state']),address:col(['address','street']),
      ...
      // Key -> existing prospect map, so duplicate rows can ENRICH the record they
      // matched instead of being thrown away. This is the "can they be added in
      ...
      for(let i=1;i<lines.length;i++){
        ...
        const phone=g(row,ci.mobile)||g(row,ci.directPhone)||g(row,ci.corpPhone)||g(row,ci.compPhone)||g(row,ci.otherPhone)||g(row,ci.anyPhone);
        const candidate={name,company,email:g(row,ci.email),linkedin:g(row,ci.linkedin),phone};
        const rowData={rank:parseInt(g(row,ci.rank))||i,name,title:g(row,ci.title),company,email:g(row,ci.email),phone,
          ...
          status:g(row,ci.status)||'New',notes:g(row,ci.notes)};
        const dupKey=_checkProspectDup(candidate,existingKeys);
        if(dupKey){
          const ex=_keyToProspect[dupKey];
          if(ex){
            const patch={};
            ['title','company','email','phone','linkedin','companyLinkedin','website','twitter','employees','city','state','address'].forEach(f2=>{if(!ex[f2]&&rowData[f2])patch[f2]=rowData[f2]});
            if(!ex.assignedRep&&csvRep)patch.assignedRep=csvRep;
            if(Object.keys(patch).length>0){const{id:_pid,...exData}=ex;const merged={id:_pid,...exData,...patch};save(_pid,{...exData,...patch});_prospectKeys(merged).forEach(k2=>{_keyToProspect[k2]=merged});enriched++;continue;}
          }
          skipped++;const prefix=dupKey.split(':')[0];if(skipReasons[prefix]!==undefined)skipReasons[prefix]++;continue}
        save('PROS-'+Date.now()+'-'+Math.random().toString(36).slice(2,7),{...rowData,assignedRep:csvRep||'',addedAt:new Date().toISOString()});added++;
      }
      ...
      notify(added+' new prospect'+(added!==1?'s':'')+(_csvRepName?' -- assigned to '+_csvRepName:'')+(enriched>0?', '+enriched+' existing updated with new info':'')+(skipped>0?', '+skipped+' unchanged duplicates skipped':''));
            ...
            {col.slice(0,30).map(p=>{
              ...
              return <div key={p.id} style={{padding:'10px 12px',background:'#0a0a0a',borderRadius:8,border:'1px solid #111',transition:'all 0.15s',cursor:'pointer'}} onClick={()=>{setViewMode('table');setExpandedId(p.id);setStatusFilter('all');setPageNum(0)}} onMouseEnter={e=>{e.currentTarget.style.borderColor='#2dd4bf30';e.currentTarget.style.transform='translateY(-1px)'}} onMouseLeave={e=>{e.currentTarget.style.borderColor='#111';e.currentTarget.style.transform='translateY(0)'}}>
                ...
                <div style={{display:'flex',gap:4,marginTop:6,alignItems:'center'}}>
                  ...
                  <button onClick={e=>{e.stopPropagation();setSearch(p.name||'');setStatusFilter('all');setStateFilter('all');setRepFilter('all');setPageNum(0);setViewMode('table');setExpandedId(p.id);setEditing(p.id);setEditForm({...p})}} title="Edit this prospect" style={{padding:'3px 8px',borderRadius:4,background:'rgba(45,212,191,0.08)',border:'1px solid rgba(45,212,191,0.25)',color:'#2dd4bf',fontSize:9,fontWeight:700,cursor:'pointer',fontFamily:'inherit',letterSpacing:0.5}}>EDIT</button>
                </div>
        ...
        <tbody>{paged.map(p=>{
          ...
          <tr style={{borderBottom:'1px solid #0a0a0a',transition:'background 0.1s',background:selected.has(p.id)?'rgba(45,212,191,0.03)':'transparent',cursor:'pointer'}} onClick={()=>setExpandedId(isExp?null:p.id)} onMouseEnter={e=>{if(!selected.has(p.id))e.currentTarget.style.background='#050505'}} onMouseLeave={e=>{e.currentTarget.style.background=selected.has(p.id)?'rgba(45,212,191,0.03)':'transparent'}}>
            ...
            <td style={{padding:'8px 6px',whiteSpace:'nowrap'}} onClick={e=>e.stopPropagation()}>
              ...
              <button onClick={()=>{setEditing(p.id);setEditForm({...p});setExpandedId(p.id)}} title="Edit this prospect" style={{background:'none',border:'none',color:'#737373',cursor:'pointer',fontSize:12,fontFamily:"'JetBrains Mono',monospace",padding:'4px 6px'}} onMouseEnter={e=>{e.currentTarget.style.color='#2dd4bf'}} onMouseLeave={e=>{e.currentTarget.style.color='#525252'}}>EDIT</button>
          ...
          {isExp&&<tr style={{background:'#050505'}}><td colSpan={12} style={{padding:'12px 16px 16px 42px',borderBottom:'1px solid #1a1a1a'}}>
            ...
            <><div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12,flexWrap:'wrap'}}>
              <Btn style={{fontSize:11,padding:'5px 14px'}} onClick={()=>{setEditing(p.id);setEditForm({...p})}}><I n="edit" s={12}/> Edit Prospect</Btn>
              <span style={{fontSize:10.5,color:'#525252'}}>Change the contact, company, status, rep or any other field, then Save.</span>
            </div>
...
const [tab,setTab]=useState("vendors");
  ...
  const field = (k, l, type) => <div key={k}><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:4}}>{l}</label>{type==="select-tier"?<select value={form[k]||""} onChange={e=>setForm(prev=>({...prev,[k]:e.target.value}))} style={inputStyle}>{["Associate","Mid-Level","Senior"].map(t=><option key={t}>{t}</option>)}</select>:type==="select-ctype"?<select value={form[k]||""} onChange={e=>setForm(prev=>({...prev,[k]:e.target.value}))} style={inputStyle}>{customerTypeOptions(form[k]).map(t=><option key={t}>{t}</option>)}</select>:<input type={type||"text"} value={form[k]==null?"":form[k]} onChange={e=>setForm(prev=>({...prev,[k]:e.target.value}))} style={inputStyle}/>}</div>;
  ...
  const handlePdfUpload=async(file)=>{
    ...
    try{
      ...
      const fieldList=tab==='vendors'?'name (company), contact (person), email, phone, category (e.g. Furniture, Seating, AV), address':tab==='customers'?'name (organization), contact (person), email, phone, type (one of: '+CUSTOMER_TYPES.join(', ')+'), address':'name, email, territory, tier (Associate, Mid-Level, Senior), commissionRate (decimal, e.g. 0.05 for 5%)';
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Directory" sub="Manage all vendors, customers, and team members -- edit, add, sort" action={<div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
    ...
    {selected.size>0&&<div style={{display:"flex",alignItems:"center",gap:12,padding:"10px 16px",background:"rgba(45,212,191,0.06)",border:"1px solid rgba(45,212,191,0.15)",borderRadius:10,marginBottom:16,flexWrap:"wrap"}}>
      ...
      {bulkAction==="type"&&<div style={{display:"flex",gap:8,alignItems:"center"}}><select onChange={e=>{if(e.target.value)handleBulkEdit("type",e.target.value)}} style={{...inputStyle,width:160,padding:"5px 10px"}}><option value="">Choose type...</option>{CUSTOMER_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>}
        ...
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:csvPreview.tab==='reps'?500:700}}>
          ...
          <tbody>{csvPreview.rows.map((r,idx)=><tr key={idx} style={{borderBottom:"1px solid rgba(255,255,255,0.02)",opacity:r._include?1:0.35,transition:"opacity 0.15s",background:r._action==='enrich'?"rgba(251,191,36,0.04)":"transparent"}}>
            ...
            {csvPreview.tab==='customers'&&<>
              ...
              <td style={{padding:"4px 6px"}}><input value={r.email} onChange={e=>csvPreviewUpdate(idx,'email',e.target.value)} style={{background:"transparent",border:"1px solid transparent",borderRadius:4,padding:"2px 4px",color:"#2dd4bf",fontSize:11,width:"100%",fontFamily:"inherit",outline:"none"}} onFocus={e=>e.target.style.borderColor='rgba(45,212,191,0.3)'} onBlur={e=>e.target.style.borderColor='transparent'}/></td>
              <td style={{padding:"4px 6px"}}><input value={r.phone} onChange={e=>csvPreviewUpdate(idx,'phone',e.target.value)} style={{background:"transparent",border:"1px solid transparent",borderRadius:4,padding:"2px 4px",color:"#9a9a9a",fontSize:11,width:"100%",fontFamily:"inherit",outline:"none"}} onFocus={e=>e.target.style.borderColor='rgba(45,212,191,0.3)'} onBlur={e=>e.target.style.borderColor='transparent'}/></td>
              <td style={{padding:"4px 6px"}}><select value={r.type||'K-12 District'} onChange={e=>csvPreviewUpdate(idx,'type',e.target.value)} style={{background:"transparent",border:"1px solid transparent",borderRadius:4,padding:"2px 4px",color:"#a78bfa",fontSize:11,fontFamily:"inherit",outline:"none",cursor:"pointer"}}>{customerTypeOptions(r.type).map(t=><option key={t} style={{background:"#111"}} value={t}>{t}</option>)}</select></td>
              <td style={{padding:"4px 6px"}}><input value={r.address} onChange={e=>csvPreviewUpdate(idx,'address',e.target.value)} style={{background:"transparent",border:"1px solid transparent",borderRadius:4,padding:"2px 4px",color:"#737373",fontSize:11,width:"100%",fontFamily:"inherit",outline:"none"}} onFocus={e=>e.target.style.borderColor='rgba(45,212,191,0.3)'} onBlur={e=>e.target.style.borderColor='transparent'}/></td>
            </>}
            {csvPreview.tab==='reps'&&<>
              <td style={{padding:"4px 6px"}}><input value={r.name} onChange={e=>csvPreviewUpdate(idx,'name',e.target.value)} style={{background:"transparent",border:"1px solid transparent",borderRadius:4,padding:"2px 4px",color:"#e5e5e5",fontSize:11,width:"100%",fontFamily:"inherit",outline:"none"}} onFocus={e=>e.target.style.borderColor='rgba(45,212,191,0.3)'} onBlur={e=>e.target.style.borderColor='transparent'}/></td>
