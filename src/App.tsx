...
// MIDWEST EDUCATIONAL FURNISHINGS â AI OPERATING SYSTEM v2
// Full Mutable State Â· PDF Export Â· QB Integration Guide
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
const INIT_JOBS = [
  ...
  { id: "JOB-2026-002", name: "Midwest Tech STEM Lab Build-Out", customer: "C002", salesRep: "S001", phase: "In Progress", createdDate: "2026-02-03", startDate: "2026-02-10", endDate: "", dueDate: "2026-07-15", notes: "New STEM lab + library media center. Multi-vendor job â partial shipments expected.", paymentStatus: "partial", docStatuses: {}, activities: [] },
  ...
  { id: "JOB-2026-004", name: "Heritage Academy New Classroom Wing", customer: "C004", salesRep: "S003", phase: "Quoting", createdDate: "2026-03-05", startDate: "", endDate: "", dueDate: "2026-09-01", notes: "Brand new wing â 4 classrooms, 1 media room. Awaiting final budget approval from board.", paymentStatus: "unpaid", docStatuses: {}, activities: [] },
];
...
// âââ PDF EXPORT: zero dependencies, works everywhere âââââââââ
function usePrintOverlay() {
  const triggerPrint = (title, html) => {
    ...
    w.document.write('<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + title + '</title><style>' +
      ...
      '<span style="color:#999;font-size:12px;line-height:40px;margin-left:12px">Click Save as PDF â choose "Save as PDF" as the printer</span>' +
      '</div>' +
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// MAIN APP â Supabase-backed with Auth
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
...
export default function MidwestAIOS() {
  ...
  const searchResults = searchQuery.length < 2 ? [] : [
    ...jobs.filter(j => j.name.toLowerCase().includes(searchQuery.toLowerCase()) || j.id.toLowerCase().includes(searchQuery.toLowerCase())).map(j => ({ type: "Job", name: j.name, sub: j.id + " Â· " + j.phase, id: j.id, action: () => { setSelectedJob(j.id); setPage("jobs"); setSearchOpen(false); } })),
    ...vendors.filter(v => v.name.toLowerCase().includes(searchQuery.toLowerCase())).map(v => ({ type: "Vendor", name: v.name, sub: v.category + " Â· " + (v.discountRate * 100).toFixed(0) + "% discount", id: v.id, action: () => { setPage("directory"); setSearchOpen(false); } })),
    ...
    ...reps.filter(r => r.name.toLowerCase().includes(searchQuery.toLowerCase()) && !r.id.includes("SEED_FLAG")).map(r => ({ type: "Rep", name: r.name, sub: r.territory + " Â· " + r.tier, id: r.id, action: () => { setPage("commissions"); setSearchOpen(false); } })),
    ...lineItems.filter(i => i.description.toLowerCase().includes(searchQuery.toLowerCase())).slice(0, 8).map(i => { const j = jobs.find(j => j.id === i.jobId); return { type: "Item", name: i.description, sub: (j?.name || "") + " Â· " + fmtN(i.qtyOrdered) + " units", id: i.id, action: () => { if (j) { setSelectedJob(j.id); setPage("jobs"); } setSearchOpen(false); } }; }),
  ].slice(0, 12);
  ...
  // âââ LOAD FROM SUPABASE ON MOUNT âââââââââââââââââââââââââââ
  useEffect(() => {
    const loadFromDb = async () => {
      try {
        ...
        if (hasData) {
          // Database has data â load it. Never overwrite.
          ...
          // Database is completely empty (first-time setup only) â seed with sample data
  ...
  // Save QB config to localStorage (not DB â contains secrets)
  useEffect(() => { try { localStorage.setItem('mw_qbConfig', JSON.stringify(qbConfig)); } catch {} }, [qbConfig]);

  // âââ DERIVED COMPUTATIONS ââââââââââââââââââââââââââââââââââ
  ...
  // âââ MUTATORS (update state + save to Supabase) ââââââââââââ
...
function Dashboard({jobs,lineItems,reps,vendors,customers,getJobFinancials,getJobItems,setPage,setSelectedJob,dateFilter,setDateFilter}){
  ...
  const blob=new Blob([wb],{type:"text/csv"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="midwest-full-export-"+new Date().toISOString().split("T")[0]+".csv";a.click();URL.revokeObjectURL(url);notify("Full database exported â 5 tables")}}
  ].map((btn,i)=><button key={i} onClick={btn.fn} style={{display:"flex",alignItems:"center",gap:6,padding:"8px 14px",borderRadius:8,border:"1px solid #222222",background:"linear-gradient(135deg,#0a0a0a,#111111)",color:"#737373",fontSize:11,fontWeight:500,fontFamily:"inherit",cursor:"pointer",transition:"all 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="#2dd4bf44";e.currentTarget.style.color="#2dd4bf"}} onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(255,255,255,0.04)";e.currentTarget.style.color="#737373"}}><I n={btn.icon} s={12}/>{btn.label}</button>)}</div><p style={{fontSize:13,color:"#525252",marginBottom:4}}>Midwest Educational Furnishings â {dateFilter==="all"?"All time":dateFilter==="ytd"?"Year to date":dateFilter==="quarter"?"This quarter":"This month"} Â· {filtered.length} jobs</p></div>
    ...
    {/* âââ PIPELINE DONUT + REVENUE BREAKDOWN ââââââââââââââââ */}
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// JOB RECORDS â Full CRUD + Line Items
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function JobsPage(ctx){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}>
    <Header title="Job Records" sub="Central hub â single source of truth for every project" action={<div style={{display:"flex",gap:8}}><Btn onClick={()=>setNewCust(true)} v="secondary"><I n="plus" s={14}/> New Customer</Btn><Btn onClick={()=>setShowNewJob(true)}><I n="plus" s={14}/> New Job</Btn></div>}/>
    ...
    {viewMode==="table"&&<Tbl columns={[
      ...
      {header:"Created",render:r=><span style={{fontSize:11,color:"#525252"}}>{r.createdDate||"â"}</span>},
      {header:"Start",render:r=><span style={{fontSize:11,color:r.startDate?"#a3a3a3":"#404040"}}>{r.startDate||"â"}</span>},
      {header:"Due",render:r=><span style={{fontSize:11,color:r.dueDate&&new Date(r.dueDate)<new Date()?"#f87171":"#a3a3a3"}}>{r.dueDate||"â"}</span>},
...
// âââ JOB DETAIL ââââââââââââââââââââââââââââââââââââââââââââââ
function JobDetail({job,ctx}){
  ...
  const duplicateJob=()=>{const newId='JOB-'+new Date().getFullYear()+'-'+String(Math.floor(Math.random()*900)+100);const newJob={...job,id:newId,name:job.name+' (Copy)',phase:'Quoting',paymentStatus:'unpaid',createdDate:new Date().toISOString().split('T')[0],startDate:'',endDate:''};addJob(newJob);items.forEach(item=>{addLineItem({...item,id:'LI-'+Math.random().toString(36).slice(2,8),jobId:newId,qtyReceived:0,qtyInvoiced:0,deliveryDate:'',invoiceDate:''})});setSelectedJob(newId);notify('Job duplicated â '+newId+' created with all line items')};

  const saveJob=()=>{updateJob(job.id,editJob);setEditing(false);notify("Job updated â changes propagated everywhere")};
  ...
  return <div style={{animation:"fadeUp 0.3s"}}>
    <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(0,0,0,0.92)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",margin:"-32px -40px 0",padding:"16px 40px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
        <div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:2}}><h2 style={{fontSize:22,fontWeight:800,color:"#e5e5e5",margin:0}}>{job.name}</h2><Badge label={job.phase} color={statusColor(job.phase)}/><Badge label={job.paymentStatus} color={statusColor(job.paymentStatus)}/></div><div style={{fontSize:12,color:"#525252"}}>{job.id} Â· {customer?.name} Â· {rep?.name} Â· {fmt(f.totalRevenue)} rev Â· {pct(f.margin)} margin</div></div>
  ...
  triggerPrint('Job Packet - '+job.name,coverHtml+'<div style="page-break-before:always"></div>'+quoteHtml+'<div style="page-break-before:always"></div><h2 style="color:#2563eb;border-bottom:2px solid #2563eb;padding-bottom:8px">PURCHASE ORDERS</h2>'+poSections+'<div style="page-break-before:always"></div>'+deliveryHtml+summaryHtml+sigHtml)}} style={{fontSize:11,padding:"6px 10px"}}><I n="download" s={12}/> Job Packet</Btn></div>
    ...
    }{/* TIMELINE */}<><div style={{marginTop:32}}></div>{/* âââ JOB TIMELINE ââââââââââââââââââââââââââââââââââââââââ */}
    <Card style={{marginTop:24}}>
      ...
      {job.paymentStatus!=="paid"&&<div style={{marginTop:12,display:"flex",gap:8}}><Btn v="ghost" style={{fontSize:11}} onClick={()=>{updateJob(job.id,{paymentStatus:"paid",endDate:new Date().toISOString().split("T")[0]});addActivity("Payment received â marked as paid");notify("Job marked as paid")}}>Mark as Paid</Btn>{job.paymentStatus==="unpaid"&&<Btn v="ghost" style={{fontSize:11}} onClick={()=>{updateJob(job.id,{paymentStatus:"partial"});addActivity("Partial payment received");notify("Payment status: partial")}}>Mark Partial Payment</Btn>}</div>}
    </Card>

    </>
    }{/* ACTIVITY */}<><div style={{marginTop:32}}></div>{/* âââ ACTIVITY FEED ââââââââââââââââââââââââââââââââââââââââ */}
...
function DeliveryCalendar({jobs,lineItems,vendors,customers,getJobItems,setPage,setSelectedJob,notify}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Delivery Calendar" sub={monthName+" "+year+" â expected deliveries for active jobs"} action={<div style={{display:"flex",gap:8}}><Btn v="secondary" onClick={()=>setMonthOffset(p=>p-1)}>&larr; Prev</Btn><Btn v="secondary" onClick={()=>setMonthOffset(0)}>Today</Btn><Btn v="secondary" onClick={()=>setMonthOffset(p=>p+1)}>Next &rarr;</Btn></div>}/>
      ...
      return <Card key={job.id} style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            ...
            <div style={{fontSize:12,color:"#525252",marginTop:2}}>{job.id} Â· {items.length} items pending Â· {fmtN(totalOut)} units outstanding</div>
          </div>
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// DOCUMENTS â Quotes, POs, Invoices, Commission Statements
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,getJobFinancials,updateJob,updateLineItem,notify,qbConfig,setPage,triggerPrint,...ctx}){
  ...
  // Doc statuses stored IN each job record in Supabase â persists across browsers, deploys, cache clears
  ...
  const setDocStatus = (docNum, status) => {
    ...
    if (jobId) {
      ...
      // Commission statements â store on first job or use localStorage fallback
  ...
  // Stable doc numbers â same job+vendor always = same docNum, so statuses persist when you navigate away and back
  ...
  const handleExportPDF=(doc)=>{
    ...
    const docTitle=isQuote?"PROJECT QUOTE":isPO?"PURCHASE ORDER":isComm?"COMMISSION STATEMENT":"INVOICE";
    const name=isQuote?doc.data.customer?.name:isPO?doc.data.vendor?.name:isComm?doc.data.rep?.name:doc.data.customer?.name;
    const addr=isQuote?doc.data.customer?.address:isPO?doc.data.vendor?.address:isComm?"":doc.data.customer?.address;
    const rows=doc.data.items.map(i=>{
      const q=i.displayQty!==undefined?i.displayQty:i.qtyOrdered;
      const p=i.displayPrice!==undefined?i.displayPrice:i.unitPrice;
    ...
    triggerPrint(docTitle+" - "+doc.job.name,
      '<div class="hdr"><div><img src="'+MW_LOGO+'" style="height:36px;margin-bottom:8px"/><div class="co">MIDWEST EDUCATIONAL FURNISHINGS</div><div>Designing Spaces | Building Futures</div><div style="margin-top:4px">(847) 847-1865 Â· info@mwfurnishings.com Â· www.mwfurnishings.com</div></div><div style="text-align:right"><div class="gold" style="font-weight:bold;font-size:14px">'+docTitle+'</div><div style="font-weight:bold;font-size:13px">'+(doc.data.docNum||'')+'</div><div>Date: '+new Date().toLocaleDateString()+'</div><div>Job: '+doc.job.id+'</div>'+(isQuote?'<div>Valid for 30 days</div>':'')+'</div></div>'
      +'<h2>'+(isQuote?"Prepared For":isPO?"Vendor":isComm?"Sales Rep":"Bill To")+': '+name+'</h2>'
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices, Commission Statements"/>
    <div style={{display:"flex",gap:0,alignItems:"center",marginBottom:16,padding:"10px 16px",background:"#111111",borderRadius:8,fontSize:11,color:"#525252"}}><span style={{color:"#2dd4bf",fontWeight:600}}>Workflow:</span><span style={{margin:"0 8px"}}>Quote</span><span style={{color:"#404040"}}>â</span><span style={{margin:"0 8px",color:"#2dd4bf"}}>Approve/Send</span><span style={{color:"#404040"}}>â</span><span style={{margin:"0 8px"}}>Purchase Orders</span><span style={{color:"#404040"}}>â</span><span style={{margin:"0 8px"}}>Draft/Send</span><span style={{color:"#404040"}}>â</span><span style={{margin:"0 8px"}}>Invoices</span><span style={{color:"#404040"}}>â</span><span style={{margin:"0 8px",color:"#34d399"}}>Mark Paid</span></div>
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// COMMISSIONS â Editable Reps + PDF Export
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function CommissionsPage({jobs,reps,customers,updateRep,addRep,deleteRep,getJobFinancials,notify,triggerPrint}){
  ...
  const saveEdit=()=>{updateRep(editingRep,{name:editForm.name,email:editForm.email,territory:editForm.territory,commissionRate:parseFloat(editForm.commissionRate)||0,tier:editForm.tier});setEditingRep(null);notify("Sales rep updated â commissions recalculated everywhere")};
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Commission Engine" sub="Editable reps, customizable rates â auto-calculated on every job" action={<Btn onClick={()=>setAddingRep(true)}><I n="plus" s={14}/> Add Sales Rep</Btn>}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:24}}>{reps.map(rep=>{const rj=jobs.filter(j=>j.salesRep===rep.id);const totalRev=rj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const paidRev=rj.filter(j=>j.paymentStatus==="paid").reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const isEd=editingRep===rep.id;
      ...
      return <Card key={rep.id} style={{border:isEd?"1px solid #2dd4bf44":undefined}}>
        ...
        :<><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div><div style={{fontSize:15,fontWeight:700,color:"#e5e5e5"}}>{rep.name}</div><div style={{fontSize:12,color:"#525252"}}>{rep.territory} Â· {rep.tier}</div></div><div style={{display:"flex",gap:6}}><Badge label={`${(rep.commissionRate*100).toFixed(1)}%`} color="#2dd4bf"/><button onClick={()=>startEdit(rep)} style={{background:"none",border:"none",cursor:"pointer",color:"#525252",padding:2}}><I n="edit" s={14}/></button></div></div>
        ...
        <div style={{marginTop:12}}><Bar value={paidRev} max={totalRev||1} color="#34d399" height={4}/><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:11,color:"#525252"}}>{rj.length} jobs Â· {fmt(totalRev)} revenue</span><Btn v="ghost" style={{fontSize:10,padding:"3px 8px"}} onClick={()=>{setSelectedRep(rep.id===selectedRep?null:rep.id)}}>Statement</Btn></div></div>
...
function BrainPage({jobs,reps,lineItems,vendors,customers,getJobFinancials,brainQuery,setBrainQuery,brainLoading,setBrainLoading}){
  const [history,setHistory]=useState([{role:"system",content:"Welcome to the Midwest Brain. I have access to all your live data â vendors, jobs, deliveries, financials. Ask me anything."}]);
  ...
  const buildAnswer = (q) => {
    ...
    // âââ VENDOR QUERIES ââââââââââââââââââââââââââââââââââââ
    ...
    if (matchedVendor || ql.includes("vendor")) {
      if (matchedVendor) {
        ...
        return `**${matchedVendor.name}**\n\nContact: ${matchedVendor.contact}\nEmail: ${matchedVendor.email}\nPhone: ${matchedVendor.phone}\nCategory: ${matchedVendor.category}\nAddress: ${matchedVendor.address || 'Not set'}\n\n**History from your data:**\nâ¢ Total spend: ${fmt(totalSpend)}\nâ¢ ${vendorItems.length} line items across ${jobIds.length} jobs\nâ¢ Average unit cost: ${fmt(avgCost)}\nâ¢ Jobs: ${jobIds.map(id => jobs.find(j => j.id === id)?.name || id).join(', ')}`;
      }
      return `**All Vendors (${vendors.length}):**\n\n${vendors.map(v => { const spend = lineItems.filter(li => li.vendor === v.id).reduce((s, i) => s + i.unitCost * i.qtyOrdered, 0); return `â¢ **${v.name}** (${v.category}) â ${fmt(spend)} total spend â ${v.contact}`; }).join('\n')}`;
    }
    
    // âââ CUSTOMER QUERIES ââââââââââââââââââââââââââââââââââ
    ...
    if (matchedCust || ql.includes("customer") || ql.includes("school") || ql.includes("district")) {
      if (matchedCust) {
        ...
        return `**${matchedCust.name}** (${matchedCust.type})\n\nContact: ${matchedCust.contact}\nEmail: ${matchedCust.email}\nPhone: ${matchedCust.phone}\nAddress: ${matchedCust.address || 'Not set'}\n\n**History:**\nâ¢ ${custJobs.length} jobs â ${fmt(totalRev)} total revenue\nâ¢ Average margin: ${pct(avgMargin)}\nâ¢ Jobs: ${custJobs.map(j => `${j.name} (${j.phase})`).join(', ')}`;
      }
      return `**All Customers (${customers.length}):**\n\n${customers.map(c => { const rev = jobs.filter(j => j.customer === c.id).reduce((s, j) => s + getJobFinancials(j.id).totalRevenue, 0); return `â¢ **${c.name}** (${c.type}) â ${fmt(rev)} revenue â ${c.contact}`; }).join('\n')}`;
    }
    
    // âââ COMMISSION QUERIES ââââââââââââââââââââââââââââââââ
    if (ql.includes("commission") || ql.includes("sales rep") || ql.includes("rep")) {
      return `**Commission Structure (Live Data):**\n\n${reps.map(rep => { const rv = jobs.filter(j => j.salesRep === rep.id).reduce((s, j) => s + getJobFinancials(j.id).totalRevenue, 0); const paidRv = jobs.filter(j => j.salesRep === rep.id && j.paymentStatus === 'paid').reduce((s, j) => s + getJobFinancials(j.id).totalRevenue, 0); return `â¢ **${rep.name}** (${rep.tier}) â ${(rep.commissionRate * 100).toFixed(1)}% rate\n  Pipeline: ${fmt(rv)} â Earned: ${fmt(paidRv * rep.commissionRate)} â Projected: ${fmt(rv * rep.commissionRate)}`; }).join('\n\n')}\n\n**Total pipeline:** ${fmt(jobs.reduce((s, j) => s + getJobFinancials(j.id).totalRevenue, 0))}`;
    }
    
    // âââ JOB QUERIES âââââââââââââââââââââââââââââââââââââââ
    if (ql.includes("job") || ql.includes("status") || ql.includes("active") || ql.includes("project")) {
      ...
      return `**Jobs (${jobs.length} total):**\n\n${Object.entries(byPhase).map(([phase, count]) => `â¢ ${phase}: ${count}`).join('\n')}\n\n${jobs.map(j => { const f = getJobFinancials(j.id); return `â¢ **${j.name}** (${j.id})\n  ${j.phase} â ${fmt(f.totalRevenue)} â ${fmtN(f.totalReceived)}/${fmtN(f.totalOrdered)} delivered â ${j.paymentStatus}`; }).join('\n\n')}`;
    }
    
    // âââ MARGIN QUERIES ââââââââââââââââââââââââââââââââââââ
    if (ql.includes("margin") || ql.includes("profit") || ql.includes("profitab")) {
      ...
      return `**Margin Analysis (Live Data):**\n\nâ¢ Average margin across all jobs: **${pct(avgMargin)}**\nâ¢ Best margin: **${best?.name}** at ${pct(best?.margin || 0)}\nâ¢ Lowest margin: **${worst?.name}** at ${pct(worst?.margin || 0)}\nâ¢ Total profit: ${fmt(margins.reduce((s, m) => s + m.totalRevenue - m.totalCost, 0))}\n\n${margins.sort((a, b) => b.margin - a.margin).map(m => `â¢ ${m.name}: ${pct(m.margin)} margin â ${fmt(m.totalRevenue)} revenue`).join('\n')}`;
    }
    
    // âââ DELIVERY QUERIES ââââââââââââââââââââââââââââââââââ
    if (ql.includes("delivery") || ql.includes("partial") || ql.includes("shipment") || ql.includes("outstanding")) {
      ...
      return `**Delivery Status (Live Data):**\n\nâ¢ Overall: ${fmtN(totalRcv)} / ${fmtN(totalOrd)} units received (${pct(totalOrd ? totalRcv / totalOrd * 100 : 0)})\nâ¢ ${pending.length} line items still pending\n\n**Outstanding items:**\n${pending.map(i => { const job = jobs.find(j => j.id === i.jobId); const vendor = vendors.find(v => v.id === i.vendor); return `â¢ ${i.description} â ${fmtN(i.qtyOrdered - i.qtyReceived)} outstanding â ${vendor?.name || 'Unknown'} â ${job?.name || 'Unknown job'}`; }).join('\n')}`;
    }
    
    // âââ INVOICE QUERIES âââââââââââââââââââââââââââââââââââ
    if (ql.includes("invoice") || ql.includes("billing") || ql.includes("receivable")) {
      ...
      return `**Invoice Status (Live Data):**\n\nâ¢ ${uninvoiced.length} line items received but not yet invoiced\nâ¢ Uninvoiced value: **${fmt(uninvTotal)}**\n\n**Ready to invoice:**\n${uninvoiced.map(i => { const job = jobs.find(j => j.id === i.jobId); return `â¢ ${i.description} â ${fmtN(i.qtyReceived - i.qtyInvoiced)} units Ã ${fmt(i.unitPrice)} = ${fmt(i.unitPrice * (i.qtyReceived - i.qtyInvoiced))} â ${job?.name || ''}`; }).join('\n')}`;
    }
    
    // âââ REVENUE / FINANCIAL QUERIES âââââââââââââââââââââââ
    if (ql.includes("revenue") || ql.includes("financial") || ql.includes("money") || ql.includes("total")) {
      ...
      return `**Financial Overview (Live Data):**\n\nâ¢ Total pipeline revenue: **${fmt(totalRev)}**\nâ¢ Total cost: ${fmt(totalCost)}\nâ¢ Total profit: **${fmt(totalRev - totalCost)}**\nâ¢ Average margin: ${pct(jobs.reduce((s, j) => s + getJobFinancials(j.id).margin, 0) / jobs.length)}\n\n**Revenue by Rep:**\n${byRep.map(r => `â¢ ${r.name}: ${fmt(r.rev)}`).join('\n')}\n\n**By Payment Status:**\nâ¢ Paid: ${fmt(jobs.filter(j => j.paymentStatus === 'paid').reduce((s, j) => s + getJobFinancials(j.id).totalRevenue, 0))}\nâ¢ Partial: ${fmt(jobs.filter(j => j.paymentStatus === 'partial').reduce((s, j) => s + getJobFinancials(j.id).totalRevenue, 0))}\nâ¢ Unpaid: ${fmt(jobs.filter(j => j.paymentStatus === 'unpaid').reduce((s, j) => s + getJobFinancials(j.id).totalRevenue, 0))}`;
    }
    
    // âââ DEFAULT âââââââââââââââââââââââââââââââââââââââââââ
    return `I have access to your full database with **${jobs.length} jobs**, **${vendors.length} vendors**, **${customers.length} customers**, **${reps.length} reps**, and **${lineItems.length} line items**.\n\nTry asking:\nâ¢ "Show me all vendors" or "Tell me about Virco"\nâ¢ "What are our margins?"\nâ¢ "Commission breakdown"\nâ¢ "What's outstanding for delivery?"\nâ¢ "Revenue overview"\nâ¢ "Uninvoiced items"\nâ¢ "Tell me about Lincoln" (any customer name)\nâ¢ "Active job status"`;
  };
...
function DirectoryPage({vendors,customers,reps,updateVendor,addVendor,deleteVendor,updateCustomer,addCustomer,deleteCustomer,updateRep,addRep,deleteRep,notify}){
  ...
  const save=()=>{
    ...
    if(adding){
      ...
      notify("Updated â changes propagated");
    }
  ...
  // render helper (NOT a React component â prevents remount/focus loss on each keystroke)
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Directory" sub="Manage all vendors, customers, and sales reps â edit, add, sort" action={<Btn onClick={startAdd}><I n="plus" s={14}/> Add {tab==="vendors"?"Vendor":tab==="customers"?"Customer":"Sales Rep"}</Btn>}/>
...
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// QUICKBOOKS SETUP â Integration Guide + OAuth Flow Docs
// âââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
function QBSetupPage({notify,qbConfig,setQbConfig}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="QuickBooks Integration" sub="Enter your API credentials to connect â invoices and POs push directly"/>
    ...
    <Card style={{marginBottom:20}}>
      ...
      <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        <div>
          ...
          <div style={{fontSize:11,color:"#404040",marginTop:4}}>From developer.intuit.com â My Apps â Keys & credentials</div>
        </div>
        <div>
    ...
    <Card style={{marginBottom:16}}><div style={{fontSize:16,fontWeight:700,color:"#e5e5e5",marginBottom:12}}>How to Get Your Credentials</div>
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div><div style={{fontSize:13,fontWeight:700,color:"#2dd4bf",marginBottom:4}}>Step 2: Get Your Realm ID</div><div style={{fontSize:13,color:"#a3a3a3",lineHeight:1.7}}>Log into QuickBooks Online. Look at the URL â it contains your Realm ID (Company ID) as a number like <span style={{fontFamily:"'JetBrains Mono',monospace",color:"#2dd4bf"}}>https://app.qbo.intuit.com/app/homepage?companyId=<strong>1234567890</strong></span></div></div>
    ...
    <Card><div style={{fontSize:14,fontWeight:700,color:"#e5e5e5",marginBottom:12}}>API Endpoints Used</div><div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:12,color:"#a3a3a3",background:"#000000",padding:16,borderRadius:8,lineHeight:2}}>
      <div><span style={{color:"#34d399"}}>POST</span> <span style={{color:"#2dd4bf"}}>/v3/company/{"{realmId}"}/invoice</span> â Create invoice</div>
      <div><span style={{color:"#34d399"}}>POST</span> <span style={{color:"#2dd4bf"}}>/v3/company/{"{realmId}"}/purchaseorder</span> â Create PO</div>
      <div><span style={{color:"#a78bfa"}}>GET</span>  <span style={{color:"#2dd4bf"}}>/v3/company/{"{realmId}"}/companyinfo/{"{realmId}"}</span> â Test connection</div>
      <div><span style={{color:"#fbbf24"}}>POST</span> <span style={{color:"#2dd4bf"}}>/oauth2/v1/tokens/bearer</span> â Token refresh</div>
    </div></Card>

    <Card style={{marginTop:16,borderLeft:"3px solid #2dd4bf"}}><div style={{fontSize:13,color:"#a3a3a3",lineHeight:1.7}}>
      <strong style={{color:"#2dd4bf"}}>How it works:</strong> When you push a PO or Invoice from the Documents page, the system sends the document data to the QuickBooks API using your access token. QuickBooks creates the record and returns a confirmation. The Job Record stays the source of truth â QB receives the finalized document. One entry, one record, everything flows.
    </div></Card>
  </div>;
}
