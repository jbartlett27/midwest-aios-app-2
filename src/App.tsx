  ...
  triggerPrint('Job Packet - '+job.name,coverHtml+'<div style="page-break-before:always"></div>'+quoteHtml+'<div style="page-break-before:always"></div>'+hd+poSections+'<div style="page-break-before:always"></div>'+hd+deliveryHtml+summaryHtml+sigHtml)}} style={{fontSize:12,padding:"6px 10px"}}><I n="download" s={12}/> Job Packet</Btn></div>
    ...
    <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",borderRadius:10,border:"1px solid #222222"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:1500}}><thead><tr style={{background:"#111111"}}>{["Tag","Manuf.","Disc%","Model #","Description","Color","List","Net Each","Your Price","Ship/Unit","Install/Unit","Qty","Recd","Status","Total",""].map((h,i)=><th key={i} style={{padding:"6px 8px",textAlign:i>=5?"right":"left",fontWeight:600,color:"#a3a3a3",fontSize:11,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid #222222",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{(()=>{
      ...
      return <tr key={item.id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
        ...
        <td style={{padding:"6px 8px",textAlign:"right",minWidth:45}}>{(()=>{const v=vendors.find(v=>v.id===item.vendor);const dr=v?.discountRate||0;return isE?<input type="number" value={dr?Math.round(dr*100):""} onChange={e=>{const newDr=(parseFloat(e.target.value)||0)/100;updateVendor(v?.id||"",{discountRate:newDr});if(item.listPrice)updateLineItem(item.id,{unitCost:Math.round((item.listPrice||0)*(1-newDr)*100)/100})}} style={{...eS,width:40,textAlign:"right"}} placeholder="0"/>:dr>0?<span style={{fontFamily:"'JetBrains Mono',monospace",color:"#34d399",fontSize:11}}>{Math.round(dr*100)}%</span>:<span style={{color:"#555"}}>--</span>})()}</td>
