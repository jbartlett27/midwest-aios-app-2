...
function JobsPage(ctx){
  ...
  const [newJob,setNewJob]=useState({name:"",customer:customers[0]?.id||"",salesRep:reps[0]?.id||"",dueDate:"",notes:""});
  const [newCust,setNewCust]=useState(false);
  const [custForm,setCustForm]=useState({name:"",contact:"",email:"",phone:"",type:"K-12 District"});

  const handleCreateJob=()=>{if(!newJob.name)return;const id=`JOB-2026-${String(jobs.length+1).padStart(3,"0")}`;addJob({id,...newJob,phase:"Quoting",createdDate:new Date().toISOString().split("T")[0],paymentStatus:"unpaid"});setShowNewJob(false);setNewJob({name:"",customer:customers[0]?.id||"",salesRep:reps[0]?.id||"",dueDate:"",notes:""});notify(`Job ${id} created — saved to database`)};
  ...
  return <div style={{animation:"fadeUp 0.4s"}}>
    ...
    <Tbl columns={[
      ...
      {header:"Sales Rep",render:r=>reps.find(rep=>rep.id===r.salesRep)?.name},
