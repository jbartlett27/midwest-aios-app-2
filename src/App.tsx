...
function JobsPage(ctx){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}>
    ...
    {viewMode==="table"&&<Tbl columns={[
      ...
      {header:"Created",render:r=><span style={{fontSize:11,color:"#525252"}}>{r.createdDate||"--"}</span>},
      {header:"Start",render:r=><span style={{fontSize:11,color:r.startDate?"#a3a3a3":"#404040"}}>{r.startDate||"--"}</span>},
      {header:"Due",render:r=><span style={{fontSize:11,color:r.dueDate&&new Date(r.dueDate)<new Date()?"#f87171":"#a3a3a3"}}>{r.dueDate||"--"}</span>},
...
function JobDetail({job,ctx}){
  ...
  const [newItem,setNewItem]=useState({description:"",vendor:vendors[0]?.id||"",tag:"",manufacturer:"",modelNumber:"",color:"",listPrice:0,unitCost:0,unitPrice:0,qtyOrdered:0,qtyReceived:0,qtyInvoiced:0});
  ...
  const saveNewItem=()=>{if(!newItem.description)return;addLineItem({...newItem,jobId:job.id,tag:newItem.tag||"",manufacturer:newItem.manufacturer||"",modelNumber:newItem.modelNumber||"",color:newItem.color||"",listPrice:parseFloat(newItem.listPrice)||0,unitCost:parseFloat(newItem.unitCost)||0,unitPrice:parseFloat(newItem.unitPrice)||0,qtyOrdered:parseInt(newItem.qtyOrdered)||0,qtyReceived:parseInt(newItem.qtyReceived)||0,qtyInvoiced:parseInt(newItem.qtyInvoiced)||0});setAddingItem(false);setNewItem({description:"",vendor:vendors[0]?.id||"",tag:"",manufacturer:"",modelNumber:"",color:"",listPrice:0,unitCost:0,unitPrice:0,qtyOrdered:0,qtyReceived:0,qtyInvoiced:0});notify("Line item added -- financials updated")};
  ...
  return <div style={{animation:"fadeUp 0.3s"}}>
    <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(0,0,0,0.92)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",margin:"-32px -40px 0",padding:"16px 40px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
        <div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:2}}><h2 style={{fontSize:22,fontWeight:800,color:"#e5e5e5",margin:0}}>{job.name}</h2><Badge label={job.phase} color={statusColor(job.phase)}/><Badge label={job.paymentStatus} color={statusColor(job.paymentStatus)}/></div><div style={{fontSize:12,color:"#525252"}}>{job.id} - {customer?.name} - {rep?.name} - {fmt(f.totalRevenue)} rev - {pct(f.margin)} margin</div></div>
  ...
  triggerPrint('Job Packet - '+job.name,coverHtml+'<div style="page-break-before:always"></div>'+quoteHtml+'<div style="page-break-before:always"></div><h2 style="color:#2563eb;border-bottom:2px solid #2563eb;padding-bottom:8px">PURCHASE ORDERS</h2>'+poSections+'<div style="page-break-before:always"></div>'+deliveryHtml+summaryHtml+sigHtml)}} style={{fontSize:11,padding:"6px 10px"}}><I n="download" s={12}/> Job Packet</Btn></div>
    ...
    {addingItem&&<Card style={{marginBottom:20,border:"1px solid #05966930"}}><div style={{fontSize:14,fontWeight:700,marginBottom:12,color:"#34d399"}}>Add Line Item</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:8}}><div><label style={{fontSize:10,color:"#525252",display:"block",marginBottom:3}}>Tag/Room</label><input value={newItem.tag} onChange={e=>setNewItem({...newItem,tag:e.target.value})} placeholder="e.g. 100A" style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:10,color:"#525252",display:"block",marginBottom:3}}>Manufacturer</label><input value={newItem.manufacturer} onChange={e=>setNewItem({...newItem,manufacturer:e.target.value})} placeholder="e.g. Smith System" style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:10,color:"#525252",display:"block",marginBottom:3}}>Model #</label><input value={newItem.modelNumber} onChange={e=>setNewItem({...newItem,modelNumber:e.target.value})} placeholder="e.g. 714" style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:10,color:"#525252",display:"block",marginBottom:3}}>Color/Finish</label><input value={newItem.color} onChange={e=>setNewItem({...newItem,color:e.target.value})} placeholder="e.g. Black/Chrome" style={{...inputStyle,fontSize:12}}/></div></div><div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr 1fr",gap:8,marginBottom:12}}><div><label style={{fontSize:10,color:"#525252",display:"block",marginBottom:3}}>Description</label><input value={newItem.description} onChange={e=>setNewItem({...newItem,description:e.target.value})} style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:10,color:"#525252",display:"block",marginBottom:3}}>Vendor</label><select value={newItem.vendor} onChange={e=>{const vid=e.target.value;const calc=autoCalcFromList(parseFloat(newItem.listPrice)||0,vid);setNewItem({...newItem,vendor:vid,...calc})}} style={{...inputStyle,fontSize:12}}>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}{v.discountRate?" ("+((v.discountRate*100).toFixed(0))+"% off)":""}</option>)}</select></div><div><label style={{fontSize:10,color:"#525252",display:"block",marginBottom:3}}>List Price</label><input type="number" value={newItem.listPrice} onChange={e=>{const lp=parseFloat(e.target.value)||0;const calc=autoCalcFromList(lp,newItem.vendor);setNewItem({...newItem,listPrice:lp,...calc})}} style={{...inputStyle,fontSize:12}} placeholder="List price"/></div><div><label style={{fontSize:10,color:"#34d399",display:"block",marginBottom:3}}>Cost (auto)</label><input type="number" value={newItem.unitCost} onChange={e=>setNewItem({...newItem,unitCost:parseFloat(e.target.value)||0})} style={{...inputStyle,fontSize:12,borderColor:"#05966930"}}/></div><div><label style={{fontSize:10,color:"#2dd4bf",display:"block",marginBottom:3}}>Sell Price</label><input type="number" value={newItem.unitPrice} onChange={e=>setNewItem({...newItem,unitPrice:parseFloat(e.target.value)||0})} style={{...inputStyle,fontSize:12,borderColor:"#2dd4bf30"}}/></div><div><label style={{fontSize:10,color:"#525252",display:"block",marginBottom:3}}>Qty</label><input type="number" value={newItem.qtyOrdered} onChange={e=>setNewItem({...newItem,qtyOrdered:e.target.value})} style={{...inputStyle,fontSize:12}}/></div></div>{newItem.listPrice>0&&<div style={{fontSize:11,color:"#34d399",marginBottom:8}}>Vendor discount: {((vendors.find(v=>v.id===newItem.vendor)?.discountRate||0)*100).toFixed(0)}% off list -- Cost: {fmt(newItem.unitCost)} -- Sell: {fmt(newItem.unitPrice)} -- Margin: {newItem.unitPrice>0?((1-newItem.unitCost/newItem.unitPrice)*100).toFixed(1):0}%</div>}<div style={{display:"flex",gap:8}}><Btn onClick={saveNewItem}>Add Item</Btn><Btn v="secondary" onClick={()=>setAddingItem(false)}>Cancel</Btn></div></Card>}

    </>
    }{detailTab==="items"&&<><Header title="Line Items" sub="Click any cell value to edit inline -- changes propagate across all views"/>
    ...
    }{detailTab==="timeline"&&<>{/* --- JOB TIMELINE ---------------------------------------- */}
    <Card style={{marginTop:24}}>
      ...
      {job.paymentStatus!=="paid"&&<div style={{marginTop:12,display:"flex",gap:8}}><Btn v="ghost" style={{fontSize:11}} onClick={()=>{updateJob(job.id,{paymentStatus:"paid",endDate:new Date().toISOString().split("T")[0]});addActivity("Payment received -- marked as paid");notify("Job marked as paid")}}>Mark as Paid</Btn>{job.paymentStatus==="unpaid"&&<Btn v="ghost" style={{fontSize:11}} onClick={()=>{updateJob(job.id,{paymentStatus:"partial"});addActivity("Partial payment received");notify("Payment status: partial")}}>Mark Partial Payment</Btn>}</div>}
    </Card>

    </>
    }{detailTab==="activity"&&<>{/* --- ACTIVITY FEED ---------------------------------------- */}
      ...
      return <Card key={job.id} style={{marginBottom:16}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div>
            ...
            <div style={{fontSize:12,color:"#525252",marginTop:2}}>{job.id} - {items.length} items pending - {fmtN(totalOut)} units outstanding</div>
          </div>
...
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,getJobFinancials,updateJob,updateLineItem,notify,qbConfig,setPage,triggerPrint,...ctx}){
  ...
  const handleExportPDF=(doc)=>{
    ...
    if(isPO){
      ...
      html=mwHdr+'<h1 style="font-size:28px;font-weight:300;color:#888;margin:24px 0 16px">Purchase Order</h1><div style="display:flex;justify-content:space-between;margin-bottom:12px"><div style="flex:1"><div style="font-weight:700;font-size:12px;text-transform:uppercase;margin-bottom:4px">VENDOR</div><div style="font-size:13px;line-height:1.6">'+(vendObj.name||'')+'<br>'+(vendObj.address||'').replace(/\n/g,'<br>')+'</div></div><div style="flex:1"><div style="font-weight:700;font-size:12px;text-transform:uppercase;margin-bottom:4px">SHIP TO</div><div style="font-size:13px;line-height:1.6">'+(job.shipTo||customer2.name||'')+'<br>'+(customer2.address||'').replace(/\n/g,'<br>')+'</div></div><div style="text-align:right"><div style="font-size:13px"><strong>P.O. NO.</strong> '+(doc.data.docNum||'')+'</div><div style="font-size:13px"><strong>DATE</strong> '+todayStr+'</div></div></div><hr style="border:none;border-top:1px solid #ccc;margin:12px 0">'+(job.shipVia?'<div style="margin-bottom:12px"><div style="font-weight:700;font-size:12px;text-transform:uppercase;margin-bottom:4px">SHIP VIA</div><div style="font-size:13px">'+job.shipVia+'</div></div>':'')+'<table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr style="border-bottom:2px solid #222"><th style="text-align:left;padding:8px;font-weight:600;color:#2dd4bf">DESCRIPTION</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:60px">QTY</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:90px">RATE</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:100px">AMOUNT</th></tr></thead><tbody>'+rows+'</tbody></table><div style="margin-top:24px;display:flex;justify-content:space-between"><div style="font-size:12px;color:#666;max-width:50%;white-space:pre-wrap">'+(job.notes||'')+'</div><div style="text-align:right"><div style="font-size:13px;margin-bottom:4px">SUBTOTAL<span style="margin-left:40px">'+fmt(total)+'</span></div><div style="font-size:18px;font-weight:700">TOTAL<span style="margin-left:40px">'+fmt(total)+'</span></div></div></div>';
    } else if(isInvoice){
      const rows=items.map(i=>'<tr><td style="padding:10px 8px;border-bottom:1px solid #eee;vertical-align:top;white-space:pre-wrap">'+(buildDesc(i))+'</td><td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;vertical-align:top">'+(i.displayQty||i.qtyOrdered)+'</td><td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;vertical-align:top">'+fmt(i.displayPrice||i.unitPrice)+'</td><td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;vertical-align:top">'+fmt((i.displayQty||i.qtyOrdered)*(i.displayPrice||i.unitPrice))+'</td></tr>').join('');
      html=mwHdr+'<div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px"><div style="display:flex;gap:40px"><div><div style="font-weight:700;font-size:12px;text-transform:uppercase;margin-bottom:4px">BILL TO</div><div style="font-size:13px;line-height:1.6">'+(customer2.name||'')+'<br>'+(customer2.contact?'Attn: '+customer2.contact+'<br>':'')+(customer2.address||'').replace(/\n/g,'<br>')+'</div></div><div><div style="font-weight:700;font-size:12px;text-transform:uppercase;margin-bottom:4px">SHIP TO</div><div style="font-size:13px;line-height:1.6">'+(job.shipTo||customer2.name||'')+'</div></div></div><div style="background:#111;color:#fff;padding:12px 20px;text-align:right;min-width:220px"><div style="font-size:20px;font-weight:700;margin-bottom:8px">INVOICE '+(doc.data.docNum||'')+'</div><div style="font-size:12px"><strong>DATE</strong> '+todayStr+'  <strong>TERMS</strong> '+terms+'</div><div style="font-size:12px;margin-top:4px;color:#2dd4bf"><strong>DUE DATE</strong> '+dueDateStr+'</div></div></div>'+(job.poNumber?'<div style="margin:12px 0"><div style="font-weight:700;font-size:12px">P.O. NUMBER</div><div style="font-size:13px">'+job.poNumber+'</div></div>':'')+'<table style="width:100%;border-collapse:collapse;font-size:13px;margin-top:16px"><thead><tr style="border-bottom:2px solid #222"><th style="text-align:left;padding:8px;font-weight:600;color:#2dd4bf">DESCRIPTION</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:60px">QTY</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:90px">RATE</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:100px">AMOUNT</th></tr></thead><tbody>'+rows+'</tbody></table><div style="margin-top:24px;text-align:right"><div style="font-size:13px;color:#666;margin-bottom:8px">Thank you for your business!</div><div style="font-size:13px;margin-bottom:4px">SUBTOTAL<span style="margin-left:40px">'+fmt(total)+'</span></div><div style="font-size:18px;font-weight:700;background:#111;color:#fff;padding:8px 16px;display:inline-block">TOTAL DUE<span style="margin-left:40px;color:#2dd4bf">'+fmt(total)+'</span></div></div>';
    } else if(isQuote){
      const rows=items.map(i=>'<tr><td style="padding:10px 8px;border-bottom:1px solid #eee;vertical-align:top">'+(i.tag||'')+'</td><td style="padding:10px 8px;border-bottom:1px solid #eee;vertical-align:top">'+(i.manufacturer||vendors.find(v=>v.id===i.vendor)?.name||'')+'</td><td style="padding:10px 8px;border-bottom:1px solid #eee;vertical-align:top;white-space:pre-wrap">'+(i.modelNumber?i.modelNumber+"\n":"")+(i.description||'')+(i.color?"\n"+i.color:"")+'</td><td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;vertical-align:top">'+(i.displayQty||i.qtyOrdered)+'</td><td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;vertical-align:top">'+fmt(i.displayPrice||i.unitPrice)+'</td><td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;vertical-align:top">'+fmt((i.displayQty||i.qtyOrdered)*(i.displayPrice||i.unitPrice))+'</td></tr>').join('');
      html=mwHdr+'<div style="text-align:center;margin:16px 0 24px"><div style="font-size:12px;color:#666">DESIGNING SPACES | BUILDING FUTURES</div><div style="font-size:12px;color:#666">\"WBE\" Certified Enterprise</div></div><div style="display:flex;justify-content:space-between;margin-bottom:16px"><div><div style="font-weight:700;font-size:12px;text-transform:uppercase;margin-bottom:4px">PREPARED FOR</div><div style="font-size:13px;line-height:1.6">'+(customer2.name||'')+'<br>'+(customer2.contact?'Attn: '+customer2.contact+'<br>':'')+(customer2.address||'').replace(/\n/g,'<br>')+'</div></div><div style="text-align:right"><div style="font-size:22px;font-weight:700;margin-bottom:4px">QUOTE</div><div style="font-size:13px">'+(doc.data.docNum||'')+'</div><div style="font-size:13px;margin-top:4px">Date: '+todayStr+'</div><div style="font-size:13px">Project: '+job.name+'</div></div></div><table style="width:100%;border-collapse:collapse;font-size:12px;margin-top:16px"><thead><tr style="border-bottom:2px solid #222"><th style="text-align:left;padding:8px;font-weight:600;color:#2dd4bf;width:60px">TAG</th><th style="text-align:left;padding:8px;font-weight:600;color:#2dd4bf;width:80px">MANUF.</th><th style="text-align:left;padding:8px;font-weight:600;color:#2dd4bf">DESCRIPTION</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:50px">QTY</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:80px">YOUR PRICE</th><th style="text-align:right;padding:8px;font-weight:600;color:#2dd4bf;width:100px">EXTENDED</th></tr></thead><tbody>'+rows+'</tbody></table><div style="margin-top:16px;text-align:right;font-size:16px;font-weight:700">TOTAL: '+fmt(total)+'</div><div style="margin-top:24px;padding:16px;background:#f9f8f6;border-radius:8px;font-size:12px;color:#666"><strong>Terms:</strong> Quote valid for 30 days. Prices subject to manufacturer availability. Delivery and installation included unless noted. Payment terms: '+terms+'.</div>';
    } else {
      // Commission statement - keep simple
      const rows=items.map(i=>'<tr><td style="padding:8px;border-bottom:1px solid #eee">'+(i.description||'')+'</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">'+(i.displayQty||1)+'</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">'+fmt(i.displayPrice||0)+'</td><td style="padding:8px;border-bottom:1px solid #eee;text-align:right">'+fmt((i.displayQty||1)*(i.displayPrice||0))+'</td></tr>').join('');
      html=mwHdr+'<h2>Commission Statement</h2><table style="width:100%;border-collapse:collapse;font-size:13px"><thead><tr style="border-bottom:2px solid #222"><th style="text-align:left;padding:8px">Description</th><th style="text-align:right;padding:8px;width:60px">Qty</th><th style="text-align:right;padding:8px;width:90px">Rate</th><th style="text-align:right;padding:8px;width:100px">Amount</th></tr></thead><tbody>'+rows+'</tbody></table><div style="margin-top:16px;text-align:right;font-size:16px;font-weight:700">TOTAL: '+fmt(total)+'</div>';
    }
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices, Commission Statements"/>
    <div style={{display:"flex",gap:0,alignItems:"center",marginBottom:16,padding:"10px 16px",background:"#111111",borderRadius:8,fontSize:11,color:"#525252"}}><span style={{color:"#2dd4bf",fontWeight:600}}>Workflow:</span><span style={{margin:"0 8px"}}>Quote</span><span style={{color:"#404040"}}>&rarr;</span><span style={{margin:"0 8px",color:"#2dd4bf"}}>Approve/Send</span><span style={{color:"#404040"}}>&rarr;</span><span style={{margin:"0 8px"}}>Purchase Orders</span><span style={{color:"#404040"}}>&rarr;</span><span style={{margin:"0 8px"}}>Draft/Send</span><span style={{color:"#404040"}}>&rarr;</span><span style={{margin:"0 8px"}}>Invoices</span><span style={{color:"#404040"}}>&rarr;</span><span style={{margin:"0 8px",color:"#34d399"}}>Mark Paid</span></div>
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Commission Engine" sub="Editable reps, customizable rates -- auto-calculated on every job" action={<Btn onClick={()=>setAddingRep(true)}><I n="plus" s={14}/> Add Sales Rep</Btn>}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:24}}>{reps.map(rep=>{const rj=jobs.filter(j=>j.salesRep===rep.id);const totalRev=rj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const paidRev=rj.filter(j=>j.paymentStatus==="paid").reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const isEd=editingRep===rep.id;
      ...
      return <Card key={rep.id} style={{border:isEd?"1px solid #2dd4bf44":undefined}}>
        ...
        :<><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div><div style={{fontSize:15,fontWeight:700,color:"#e5e5e5"}}>{rep.name}</div><div style={{fontSize:12,color:"#525252"}}>{rep.territory} - {rep.tier}</div></div><div style={{display:"flex",gap:6}}><Badge label={`${(rep.commissionRate*100).toFixed(1)}%`} color="#2dd4bf"/><button onClick={()=>startEdit(rep)} style={{background:"none",border:"none",cursor:"pointer",color:"#525252",padding:2}}><I n="edit" s={14}/></button></div></div>
        ...
        <div style={{marginTop:12}}><Bar value={paidRev} max={totalRev||1} color="#34d399" height={4}/><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:11,color:"#525252"}}>{rj.length} jobs - {fmt(totalRev)} revenue</span><Btn v="ghost" style={{fontSize:10,padding:"3px 8px"}} onClick={()=>{setSelectedRep(rep.id===selectedRep?null:rep.id)}}>Statement</Btn></div></div>
    ...
    <Card style={{marginBottom:20}}>
      ...
      <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
        <div>
          ...
          <div style={{fontSize:11,color:"#404040",marginTop:4}}>From developer.intuit.com &rarr; My Apps &rarr; Keys &amp; credentials</div>
        </div>
        <div>
