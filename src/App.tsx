  ...
  triggerPrint('Job Packet - '+job.name,coverHtml+'<div style="page-break-before:always"></div>'+quoteHtml+'<div style="page-break-before:always"></div>'+hd+poSections+'<div style="page-break-before:always"></div>'+hd+deliveryHtml+summaryHtml+sigHtml)}} style={{fontSize:12,padding:"6px 10px"}}><I n="download" s={12}/> Job Packet</Btn></div>
    ...
    <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",borderRadius:10,border:"1px solid #222222"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:1400}}><thead><tr style={{background:"#111111"}}>{["Tag","Manuf.","Model #","Description","Color","List","Net Each","Your Price","Ship/Unit","Install/Unit","Qty","Recd","Status","Total",""].map((h,i)=><th key={i} style={{padding:"6px 8px",textAlign:i>=5?"right":"left",fontWeight:600,color:"#a3a3a3",fontSize:11,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid #222222",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{(()=>{
      ...
      return <tr key={item.id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
        ...
        <td style={{padding:"6px 8px"}}><div style={{display:"flex",gap:3}}>{editingItem!=="ALL"&&<Btn v={isE?"primary":"ghost"} style={{fontSize:11,padding:"2px 6px"}} onClick={()=>setEditingItem(isE?null:item.id)}>{isE?"Done":"Edit"}</Btn>}<Btn v="secondary" style={{fontSize:11,padding:"2px 6px"}} onClick={()=>{addLineItem({...item,id:"LI-"+Math.random().toString(36).slice(2,8),qtyReceived:0,qtyInvoiced:0});notify("Line item duplicated")}}>Dup</Btn>{isE&&<><Btn v="ghost" style={{fontSize:11,padding:"2px 6px",color:"#a78bfa"}} onClick={()=>{const g=prompt("Group name:",item.group||"");if(g!==null)updateLineItem(item.id,{group:g})}}>Grp</Btn><Btn v="danger" style={{fontSize:11,padding:"2px 6px"}} onClick={()=>{deleteLineItem(item.id);if(editingItem===item.id)setEditingItem(null);notify("Line item deleted")}}>Del</Btn></>}</div></td>
      </tr>})}{grp&&<tr style={{borderTop:"1px solid #a78bfa30"}}><td colSpan={13} style={{padding:"6px 8px",textAlign:"right",fontSize:12,color:"#a78bfa",fontWeight:600}}>{grp} Subtotal</td><td style={{padding:"6px 8px",textAlign:"right",fontFamily:"'JetBrains Mono',monospace",fontWeight:700,color:"#a78bfa"}}>{fmt(grpTotal)}</td><td></td></tr>}</React.Fragment>})})()}</tbody></table></div>
