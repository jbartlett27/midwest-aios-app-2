...
// ═══════════════════════════════════════════════════════════════
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,getJobFinancials,notify,qbConfig,setPage,triggerPrint}){
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      const resp=await fetch('/api/send-email',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:emailTo,subject:emailSubject,html})});
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices, Commission Statements"/>
    ...
    <div className="resp-grid-4" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:20}}>
      <Card style={{padding:14,cursor:"pointer",border:tab==="quotes"?"1px solid #c8a25c44":"1px solid #1e2130"}} onClick={()=>setTab("quotes")}><div style={{fontSize:10,color:"#6b7280",textTransform:"uppercase",letterSpacing:1}}>Quotes</div><div style={{fontSize:22,fontWeight:700,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{totalQuotes}</div></Card>
      <Card style={{padding:14,cursor:"pointer",border:tab==="pos"?"1px solid #c8a25c44":"1px solid #1e2130"}} onClick={()=>setTab("pos")}><div style={{fontSize:10,color:"#6b7280",textTransform:"uppercase",letterSpacing:1}}>Purchase Orders</div><div style={{fontSize:22,fontWeight:700,color:"#2563eb",fontFamily:"'DM Mono',monospace"}}>{totalPOs}</div></Card>
      <Card style={{padding:14,cursor:"pointer",border:tab==="invoices"?"1px solid #c8a25c44":"1px solid #1e2130"}} onClick={()=>setTab("invoices")}><div style={{fontSize:10,color:"#6b7280",textTransform:"uppercase",letterSpacing:1}}>Invoices</div><div style={{fontSize:22,fontWeight:700,fontFamily:"'DM Mono',monospace"}}><span style={{color:"#d97706"}}>{pendingInvJobs.length}</span><span style={{color:"#4b5563"}}> / </span><span style={{color:"#059669"}}>{fullyInvoicedJobs.length}</span></div><div style={{fontSize:10,color:"#6b7280"}}>pending / complete</div></Card>
      <Card style={{padding:14,cursor:"pointer",border:tab==="commissions"?"1px solid #c8a25c44":"1px solid #1e2130"}} onClick={()=>setTab("commissions")}><div style={{fontSize:10,color:"#6b7280",textTransform:"uppercase",letterSpacing:1}}>Commission Statements</div><div style={{fontSize:22,fontWeight:700,color:"#059669",fontFamily:"'DM Mono',monospace"}}>{reps.filter(r=>r.commissionRate>0&&!r.id.includes("SEED_FLAG")).length}</div></Card>
    </div>

    {tab==="quotes"&&<div><div style={{fontSize:14,fontWeight:600,color:"#f5f5f4",marginBottom:12}}>Project Quotes</div>{jobs.map(job=>{const q=genQuote(job);const customer=customers.find(c=>c.id===job.customer);return <Card key={job.id} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{job.name}</span><Badge label={job.phase} color={statusColor(job.phase)}/></div><div style={{fontSize:12,color:"#6b7280"}}>{customer?.name} · {q.items.length} items · {fmt(q.total)}</div></div><div style={{display:"flex",gap:8,alignItems:"center"}}><Btn onClick={()=>{setPreviewDoc({type:"quote",data:q,job});setTab("preview")}}><I n="file" s={14}/> Generate</Btn></div></div></Card>})}</div>}

    {tab==="pos"&&<div><div style={{fontSize:14,fontWeight:600,color:"#f5f5f4",marginBottom:12}}>Purchase Orders by Job</div>{jobs.map(job=>{const pos=genPOs(job);if(pos.length===0)return null;return <Card key={job.id} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{job.name}</span><span style={{fontSize:12,color:"#6b7280",marginLeft:8}}>{pos.length} vendor POs · {fmt(pos.reduce((s,p)=>s+p.total,0))}</span></div></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))",gap:10}}>{pos.map((po,i)=><div key={i} onClick={()=>{setPreviewDoc({type:"po",data:po,job});setTab("preview")}} style={{padding:14,background:"#1a1d27",borderRadius:8,border:"1px solid #1e2130",cursor:"pointer",transition:"border 0.15s"}} onMouseEnter={e=>e.currentTarget.style.borderColor="#c8a25c44"} onMouseLeave={e=>e.currentTarget.style.borderColor="#1e2130"}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13,fontWeight:600,color:"#e8e6e3"}}>{po.vendor?.name}</span><StatusBadge docNum={po.docNum}/></div><div style={{fontSize:12,color:"#6b7280"}}>{po.items.length} items</div><div style={{fontSize:16,fontWeight:700,color:"#c8a25c",fontFamily:"'DM Mono',monospace",marginTop:4}}>{fmt(po.total)}</div></div>)}</div></Card>})}</div>}

    {tab==="invoices"&&<div><div style={{fontSize:14,fontWeight:600,color:"#f5f5f4",marginBottom:12}}>Invoices — All Jobs</div>{[...jobs].sort((a,b)=>{const aHas=getJobItems(a.id).some(i=>i.qtyReceived>i.qtyInvoiced)?0:1;const bHas=getJobItems(b.id).some(i=>i.qtyReceived>i.qtyInvoiced)?0:1;return aHas-bHas}).map(job=>{const inv=genInvoice(job);const items=getJobItems(job.id);const invoicedTotal=items.reduce((s,i)=>s+i.unitPrice*i.qtyInvoiced,0);const fullTotal=items.reduce((s,i)=>s+i.unitPrice*i.qtyOrdered,0);const isComplete=items.length>0&&items.every(i=>i.qtyInvoiced>=i.qtyOrdered);const hasPending=inv.items.length>0;const customer=customers.find(c=>c.id===job.customer);return <Card key={job.id} style={{marginBottom:12,opacity:isComplete&&!hasPending?0.7:1,border:hasPending?"1px solid #d9770630":"1px solid #1e2130"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{job.name}</span>{isComplete?<Badge label="fully invoiced" color="#059669"/>:hasPending?<Badge label="pending" color="#d97706"/>:<Badge label="not started" color="#6b7280"/>}</div><div style={{fontSize:12,color:"#6b7280"}}>{customer?.name} · {fmt(invoicedTotal)} of {fmt(fullTotal)} invoiced</div></div><div style={{display:"flex",gap:8}}>{hasPending&&<Btn onClick={()=>{setPreviewDoc({type:"invoice",data:inv,job});setTab("preview")}}><I n="receipt" s={14}/> Draft Invoice ({inv.items.length} items)</Btn>}{isComplete&&<span style={{fontSize:11,color:"#059669",padding:"8px 12px"}}>Complete</span>}</div></div><Bar value={invoicedTotal} max={fullTotal||1} color={isComplete?"#059669":"#d97706"} height={4}/></Card>})}</div>}

    {tab==="commissions"&&<div><div style={{fontSize:14,fontWeight:600,color:"#f5f5f4",marginBottom:12}}>Commission Statements — All Team Members</div>{reps.filter(r=>!r.id.includes("SEED_FLAG")).map(rep=>{const rj=jobs.filter(j=>j.salesRep===rep.id);const totalRev=rj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const comm=totalRev*rep.commissionRate;const paidRev=rj.filter(j=>j.paymentStatus==="paid").reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return <Card key={rep.id} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{rep.name}</span><Badge label={rep.tier} color="#c8a25c"/>{rep.commissionRate>0&&<Badge label={pct(rep.commissionRate*100)} color="#059669"/>}</div><div style={{fontSize:12,color:"#6b7280"}}>{rep.territory} · {rj.length} jobs · Pipeline: {fmt(totalRev)}{rep.commissionRate>0?" · Commission: "+fmt(comm):""}</div></div>{rep.commissionRate>0&&rj.length>0?<Btn onClick={()=>{const items=rj.map(j=>({description:j.name+' ('+((customers.find(c=>c.id===j.customer))?.name||'')+')' ,displayQty:1,displayPrice:getJobFinancials(j.id).totalRevenue*rep.commissionRate}));setPreviewDoc({type:"commission",data:{rep,items,total:comm,docNum:genDocNum('COMM-')},job:{id:'ALL',name:rep.name+' Commission Statement',notes:'Period: '+new Date().toLocaleDateString()}});setTab("preview")}}><I n="file" s={14}/> Generate Statement</Btn>:<span style={{fontSize:11,color:"#4b5563",padding:"8px 12px"}}>{rj.length===0?"No jobs":"N/A"}</span>}</div>{rj.length>0&&<div style={{marginTop:8}}><Bar value={paidRev} max={totalRev||1} color="#059669" height={3}/><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:10,color:"#6b7280"}}>Paid: {fmt(paidRev)}</span><span style={{fontSize:10,color:"#6b7280"}}>Total: {fmt(totalRev)}</span></div></div>}</Card>})}</div>}
    ...
    {tab==="preview"&&previewDoc&&<Card style={{border:"1px solid #c8a25c30"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{fontSize:16,fontWeight:700,color:"#c8a25c"}}>{previewDoc.type==="quote"?"PROJECT QUOTE":previewDoc.type==="po"?"PURCHASE ORDER":previewDoc.type==="commission"?"COMMISSION STATEMENT":"INVOICE"}</div><StatusBadge docNum={previewDoc.data.docNum}/></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{previewDoc.data.docNum&&<div style={{display:"flex",gap:4}}>{["drafted","sent","approved"].map(s=><button key={s} onClick={()=>setDocStatus(previewDoc.data.docNum,s)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid "+(docStatuses[previewDoc.data.docNum]===s?"#c8a25c":"#2a2d37"),background:docStatuses[previewDoc.data.docNum]===s?"#c8a25c20":"transparent",color:docStatuses[previewDoc.data.docNum]===s?"#c8a25c":"#6b7280",fontSize:10,fontFamily:"inherit",cursor:"pointer",textTransform:"capitalize"}}>{s}</button>)}</div>}<Btn onClick={()=>handleExportPDF(previewDoc)}><I n="download" s={14}/> Export PDF</Btn><Btn v="ghost" onClick={()=>{const doc=previewDoc;const isQuote=doc.type==="quote";const isPO=doc.type==="po";const isComm=doc.type==="commission";const recipient=isQuote?doc.data.customer?.email:isPO?doc.data.vendor?.email:isComm?doc.data.rep?.email:doc.data.customer?.email;setEmailTo(recipient||"");setEmailSubject((isQuote?"Project Quote":isPO?"Purchase Order":isComm?"Commission Statement":"Invoice")+" - "+doc.job.name+" - "+(doc.data.docNum||""));setEmailModal(doc)}}><I n="send" s={14}/> Email</Btn>{previewDoc.type!=="quote"&&previewDoc.type!=="commission"&&<Btn v={qbConfig.connected?"primary":"secondary"} onClick={()=>qbConfig.connected?pushToQB(previewDoc):setPage("qbsetup")} style={pushing?{opacity:0.6,pointerEvents:"none"}:{}}><I n="send" s={14}/> {pushing?"Pushing...":qbConfig.connected?"Push to QB":"Connect QB"}</Btn>}<Btn v="secondary" onClick={()=>setTab(previewDoc.type==="commission"?"commissions":previewDoc.type==="quote"?"quotes":previewDoc.type==="po"?"pos":"invoices")}>&larr; Back</Btn></div></div>
      {emailModal&&<div style={{background:"#1a1d27",border:"1px solid #c8a25c30",borderRadius:8,padding:20,marginBottom:16}}><div style={{fontSize:14,fontWeight:700,color:"#c8a25c",marginBottom:12}}>Email Document</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}><div><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:4}}>To (email address)</label><input value={emailTo} onChange={e=>setEmailTo(e.target.value)} placeholder="recipient@email.com" style={inputStyle}/></div><div><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:4}}>Subject</label><input value={emailSubject} onChange={e=>setEmailSubject(e.target.value)} style={inputStyle}/></div></div><div style={{display:"flex",gap:8}}><Btn onClick={sendEmail} style={emailSending?{opacity:0.6,pointerEvents:"none"}:{}}>{emailSending?"Sending...":"Send Email"}</Btn><Btn v="secondary" onClick={()=>setEmailModal(null)}>Cancel</Btn></div><div style={{fontSize:10,color:"#4b5563",marginTop:8}}>Sends an HTML email with the full document. Requires Resend API key in Vercel environment variables.</div></div>}
