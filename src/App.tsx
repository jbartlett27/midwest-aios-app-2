...
export default function MidwestAIOS() {
  ...
  const updateLineItem = (id, u) => {
    setLineItems(p => { const old=p.find(li=>li.id===id);const updated = p.map(li => li.id===id ? {...li,...u} : li); const item = updated.find(li=>li.id===id); if(item){db.saveLineItem(item);const changes=Object.keys(u).filter(k=>JSON.stringify(old[k])!==JSON.stringify(u[k]));if(changes.length>0){const log={time:new Date().toISOString(),type:"edit",entity:"lineItem",entityId:id,fields:changes.map(k=>({field:k,from:old[k],to:u[k]}))};setAuditLog(prev=>[log,...prev].slice(0,500))}} return updated; });
  };
  ...
  const updateJob = (id, u) => {
    setJobs(p => { const old=p.find(j=>j.id===id);const updated = p.map(j => j.id===id ? {...j,...u} : j); const job = updated.find(j=>j.id===id); if(job){db.saveJob(job);const skip=new Set(["activities","docStatuses"]);const changes=Object.keys(u).filter(k=>!skip.has(k)&&JSON.stringify(old[k])!==JSON.stringify(u[k]));if(changes.length>0){const log={time:new Date().toISOString(),type:"edit",entity:"job",entityId:id,fields:changes.map(k=>({field:k,from:old[k],to:u[k]}))};setAuditLog(prev=>[log,...prev].slice(0,500))}} return updated; });
  };
  const addJob = (job) => {
    setJobs(p => [...p, job]);
    db.saveJob(job);
