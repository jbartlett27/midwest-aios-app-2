...
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,getJobFinancials,updateJob,updateLineItem,notify,qbConfig,setPage,triggerPrint,...ctx}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices, Commission Statements"/>
    ...
    {tab==="invoices"&&<><div style={{marginBottom:8,padding:"10px 14px",background:"#12141b",borderRadius:8,fontSize:12,color:"#6b7280"}}>Invoices require POs to be drafted first. Quote must be approved before POs.</div><div>{[...jobs].sort((a,b)=>{const aHas=getJobItems(a.id).some(i=>i.qtyReceived>i.qtyInvoiced)?0:1;const bHas=getJobItems(b.id).some(i=>i.qtyReceived>i.qtyInvoiced)?0:1;return aHas-bHas}).map(job=>{const inv=genInvoice(job);const items=getJobItems(job.id);const invoicedTotal=items.reduce((s,i)=>s+i.unitPrice*i.qtyInvoiced,0);const fullTotal=items.reduce((s,i)=>s+i.unitPrice*i.qtyOrdered,0);const isComplete=items.length>0&&items.every(i=>i.qtyInvoiced>=i.qtyOrdered);const hasPending=inv.items.length>0;const customer=customers.find(c=>c.id===job.customer);return <Card key={job.id} style={{marginBottom:12,opacity:isComplete?0.65:1,border:hasPending?"1px solid #d9770630":"1px solid #1e2130"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{job.name}</span>{isComplete?<Badge label="fully invoiced" color="#059669"/>:hasPending?<Badge label="pending" color="#d97706"/>:<Badge label="not started" color="#6b7280"/>}<StatusBadge docNum={inv.docNum}/></div><div style={{fontSize:12,color:"#6b7280"}}>{customer?.name} · {fmt(invoicedTotal)} of {fmt(fullTotal)} invoiced</div></div>{(()=>{const jobPOs=genPOs(job);const anyPODrafted=jobPOs.some(po=>docStatuses[po.docNum]&&docStatuses[po.docNum]!=="new");return hasPending?anyPODrafted?<Btn onClick={()=>{setPreviewDoc({type:"invoice",data:inv,job});setTab("preview")}}><I n="receipt" s={14}/> Draft Invoice ({inv.items.length})</Btn>:<span style={{fontSize:11,color:"#d97706",padding:"6px 10px"}}>Draft POs first</span>:null})()}{job.paymentStatus!=="paid"&&<Btn v="ghost" style={{fontSize:11}} onClick={()=>{updateJob(job.id,{paymentStatus:isComplete?"paid":"partial"});notify(isComplete?"Marked as paid":"Marked partial payment")}}>Mark {isComplete?"Paid":"Partial"}</Btn>}{job.paymentStatus==="paid"&&<Badge label="paid" color="#059669"/>}</div><Bar value={invoicedTotal} max={fullTotal||1} color={isComplete?"#059669":"#d97706"} height={4}/></Card>})}</div></>}
