...
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,getJobFinancials,notify,qbConfig,setPage,triggerPrint}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices, Commission Statements"/>
    ...
    {tab==="commissions"&&<div>{reps.filter(r=>!r.id.includes("SEED_FLAG")).map(rep=>{const rj=jobs.filter(j=>j.salesRep===rep.id);const totalRev=rj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const comm=totalRev*rep.commissionRate;const paidRev=rj.filter(j=>j.paymentStatus==="paid").reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const paidComm=paidRev*rep.commissionRate;const unpaidComm=comm-paidComm;return <Card key={rep.id} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{rep.name}</span><Badge label={rep.tier} color="#c8a25c"/>{rep.commissionRate>0&&<Badge label={pct(rep.commissionRate*100)} color="#059669"/>}</div><div style={{display:"flex",gap:16,fontSize:12,color:"#6b7280",flexWrap:"wrap"}}><span>{rep.territory}</span><span>{rj.length} jobs</span><span>Pipeline: {fmt(totalRev)}</span>{rep.commissionRate>0&&<><span style={{color:"#059669"}}>Earned: {fmt(paidComm)}</span><span style={{color:"#d97706"}}>Pending: {fmt(unpaidComm)}</span><span style={{fontWeight:600,color:"#c8a25c"}}>Total: {fmt(comm)}</span></>}</div></div>{rj.length>0&&rep.commissionRate>0?<Btn onClick={()=>{const items=rj.map(j=>{const f=getJobFinancials(j.id);return{description:j.name+' ('+((customers.find(c=>c.id===j.customer))?.name||'')+') — '+j.paymentStatus,displayQty:1,displayPrice:f.totalRevenue*rep.commissionRate}});setPreviewDoc({type:"commission",data:{rep,items,total:comm,docNum:genDocNum('COMM-')},job:{id:'ALL',name:rep.name+' Commission Statement',notes:'Period: '+new Date().toLocaleDateString()+' · Earned (paid): '+fmt(paidComm)+' · Pending (unpaid): '+fmt(unpaidComm)}});setTab("preview")}}><I n="file" s={14}/> Generate Statement</Btn>:rj.length===0?<span style={{fontSize:11,color:"#4b5563",padding:"8px 12px"}}>No jobs assigned</span>:<span style={{fontSize:11,color:"#4b5563",padding:"8px 12px"}}>No commission rate set</span>}</div>{rj.length>0&&<div style={{marginTop:8}}><Bar value={paidRev} max={totalRev||1} color="#059669" height={3}/><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:10,color:"#059669"}}>Paid: {fmt(paidRev)}</span><span style={{fontSize:10,color:"#d97706"}}>Unpaid: {fmt(totalRev-paidRev)}</span><span style={{fontSize:10,color:"#6b7280"}}>Total: {fmt(totalRev)}</span></div></div>}</Card>})}</div>}
