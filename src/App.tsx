...
function JobDetail({job,ctx}){
  ...
  const [newItem,setNewItem]=useState({description:"",vendor:vendors[0]?.id||"",tag:"",manufacturer:"",modelNumber:"",color:"",listPrice:0,unitCost:0,unitPrice:0,shippingPerUnit:0,installPerUnit:0,qtyOrdered:0,qtyReceived:0,qtyInvoiced:0});
  ...
  const saveNewItem=()=>{if(!newItem.description)return;addLineItem({...newItem,jobId:job.id,tag:newItem.tag||"",manufacturer:newItem.manufacturer||"",modelNumber:newItem.modelNumber||"",color:newItem.color||"",listPrice:parseFloat(newItem.listPrice)||0,unitCost:parseFloat(newItem.unitCost)||0,unitPrice:parseFloat(newItem.unitPrice)||0,shippingPerUnit:parseFloat(newItem.shippingPerUnit)||0,installPerUnit:parseFloat(newItem.installPerUnit)||0,qtyOrdered:parseInt(newItem.qtyOrdered)||0,qtyReceived:parseInt(newItem.qtyReceived)||0,qtyInvoiced:parseInt(newItem.qtyInvoiced)||0});setAddingItem(false);setNewItem({description:"",vendor:vendors[0]?.id||"",tag:"",manufacturer:"",modelNumber:"",color:"",listPrice:0,unitCost:0,unitPrice:0,shippingPerUnit:0,installPerUnit:0,qtyOrdered:0,qtyReceived:0,qtyInvoiced:0});notify("Line item added -- financials updated")};
  ...
  triggerPrint('Job Packet - '+job.name,coverHtml+'<div style="page-break-before:always"></div>'+quoteHtml+'<div style="page-break-before:always"></div><h2 style="color:#2563eb;border-bottom:2px solid #2563eb;padding-bottom:8px">PURCHASE ORDERS</h2>'+poSections+'<div style="page-break-before:always"></div>'+deliveryHtml+summaryHtml+sigHtml)}} style={{fontSize:12,padding:"6px 10px"}}><I n="download" s={12}/> Job Packet</Btn></div>
    ...
    {addingItem&&<Card style={{marginBottom:20,border:"1px solid #05966930"}}><div style={{fontSize:14,fontWeight:700,marginBottom:12,color:"#34d399"}}>Add Line Item</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:8}}><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:3}}>Tag/Room</label><input value={newItem.tag} onChange={e=>setNewItem({...newItem,tag:e.target.value})} placeholder="e.g. 100A" style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:3}}>Manufacturer</label><input value={newItem.manufacturer} onChange={e=>setNewItem({...newItem,manufacturer:e.target.value})} placeholder="e.g. Smith System" style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:3}}>Model #</label><input value={newItem.modelNumber} onChange={e=>setNewItem({...newItem,modelNumber:e.target.value})} placeholder="e.g. 714" style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:3}}>Color/Finish</label><input value={newItem.color} onChange={e=>setNewItem({...newItem,color:e.target.value})} placeholder="e.g. Black/Chrome" style={{...inputStyle,fontSize:12}}/></div></div><div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr",gap:8,marginBottom:8}}><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:3}}>Description</label><input value={newItem.description} onChange={e=>setNewItem({...newItem,description:e.target.value})} style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:3}}>Vendor</label><select value={newItem.vendor} onChange={e=>{const vid=e.target.value;const calc=autoCalcFromList(parseFloat(newItem.listPrice)||0,vid);setNewItem({...newItem,vendor:vid,...calc})}} style={{...inputStyle,fontSize:12}}>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}{v.discountRate?" ("+((v.discountRate*100).toFixed(0))+"% off)":""}</option>)}</select></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:3}}>List Price</label><input type="number" value={newItem.listPrice} onChange={e=>{const lp=parseFloat(e.target.value)||0;const calc=autoCalcFromList(lp,newItem.vendor);setNewItem({...newItem,listPrice:lp,...calc})}} style={{...inputStyle,fontSize:12}} placeholder="List price"/></div><div><label style={{fontSize:12,color:"#34d399",display:"block",marginBottom:3}}>Net Cost (auto)</label><input type="number" value={newItem.unitCost} onChange={e=>setNewItem({...newItem,unitCost:parseFloat(e.target.value)||0})} style={{...inputStyle,fontSize:12,borderColor:"#05966930"}}/></div><div><label style={{fontSize:12,color:"#2dd4bf",display:"block",marginBottom:3}}>Your Price</label><input type="number" value={newItem.unitPrice} onChange={e=>setNewItem({...newItem,unitPrice:parseFloat(e.target.value)||0})} style={{...inputStyle,fontSize:12,borderColor:"#2dd4bf30"}}/></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8,marginBottom:12}}><div><label style={{fontSize:12,color:"#fbbf24",display:"block",marginBottom:3}}>Shipping/Unit</label><input type="number" value={newItem.shippingPerUnit} onChange={e=>setNewItem({...newItem,shippingPerUnit:e.target.value})} placeholder="0.00" style={{...inputStyle,fontSize:12,borderColor:"#fbbf2430"}}/></div><div><label style={{fontSize:12,color:"#a78bfa",display:"block",marginBottom:3}}>Install/Unit</label><input type="number" value={newItem.installPerUnit} onChange={e=>setNewItem({...newItem,installPerUnit:e.target.value})} placeholder="0.00" style={{...inputStyle,fontSize:12,borderColor:"#a78bfa30"}}/></div><div><label style={{fontSize:12,color:"#a3a3a3",display:"block",marginBottom:3}}>Qty Ordered</label><input type="number" value={newItem.qtyOrdered} onChange={e=>setNewItem({...newItem,qtyOrdered:e.target.value})} style={{...inputStyle,fontSize:12}}/></div><div style={{display:"flex",alignItems:"flex-end"}}>{newItem.qtyOrdered>0&&newItem.unitPrice>0&&<div style={{fontSize:12,color:"#2dd4bf",padding:"8px 0"}}>Total: {fmt(((parseFloat(newItem.unitPrice)||0)+(parseFloat(newItem.shippingPerUnit)||0)+(parseFloat(newItem.installPerUnit)||0))*(parseInt(newItem.qtyOrdered)||0))}</div>}</div></div>{newItem.listPrice>0&&<div style={{fontSize:12,color:"#34d399",marginBottom:8}}>Vendor discount: {((vendors.find(v=>v.id===newItem.vendor)?.discountRate||0)*100).toFixed(0)}% off list -- Cost: {fmt(newItem.unitCost)} -- Sell: {fmt(newItem.unitPrice)} -- Margin: {newItem.unitPrice>0?((1-newItem.unitCost/newItem.unitPrice)*100).toFixed(1):0}%</div>}<div style={{display:"flex",gap:8}}><Btn onClick={saveNewItem}>Add Item</Btn><Btn v="secondary" onClick={()=>setAddingItem(false)}>Cancel</Btn></div></Card>}

    <Header title="Line Items" sub="Click any cell value to edit inline -- changes propagate across all views"/>
    <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",borderRadius:10,border:"1px solid #222222"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:1200}}><thead><tr style={{background:"#111111"}}>{["Tag","Manuf.","Model #","Description","Color","Vendor","Net Each","Your Price","Ship/Unit","Install/Unit","Qty","Received","Total",""].map((h,i)=><th key={i} style={{padding:"8px 10px",textAlign:"left",fontWeight:600,color:"#a3a3a3",fontSize:12,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid #222222",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{items.map(item=>{
      ...
      return <tr key={item.id} style={{borderBottom:"1px solid #1a1d2708"}}>
        ...
        <td style={{padding:"8px 10px",color:"#a3a3a3",minWidth:120}}>{isEditing?<select value={item.vendor} onChange={e=>updateLineItem(item.id,{vendor:e.target.value})} style={{...inputStyle,fontSize:12,padding:"4px 8px"}}>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}{v.discountRate?' ('+((v.discountRate*100).toFixed(0))+'% off)':''}</option>)}</select>:<span>{vendors.find(v=>v.id===item.vendor)?.name}{(()=>{const v=vendors.find(v=>v.id===item.vendor);return v?.discountRate?<span style={{fontSize:12,color:"#34d399",marginLeft:4}}>({(v.discountRate*100).toFixed(0)}% disc)</span>:null})()}</span>}</td>
        ...
        <td style={{padding:"8px 10px"}}><div style={{display:"flex",gap:4}}><Btn v={isEditing?"primary":"ghost"} style={{fontSize:12,padding:"3px 8px"}} onClick={()=>setEditingItem(isEditing?null:item.id)}>{isEditing?"Done":"Edit"}</Btn>{isEditing&&<Btn v="danger" style={{fontSize:12,padding:"3px 8px"}} onClick={()=>{deleteLineItem(item.id);setEditingItem(null);notify("Line item deleted")}}>Del</Btn>}</div></td>
      </tr>})}</tbody></table></div>
