...
const inputStyle = {width:"100%",padding:"11px 14px",background:"rgba(58,58,60,0.5)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,color:"#e8e6e3",fontSize:13,outline:"none",fontFamily:"inherit",transition:"border 0.2s ease, background 0.2s ease"};

// ─── SHARED COMPONENTS ───────────────────────────────────────
const Card = ({children,style,onClick,hover}) => <div onClick={onClick} style={{background:"rgba(44,44,46,0.72)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:16,padding:22,cursor:onClick?"pointer":"default",transition:"all 0.25s cubic-bezier(0.25,0.1,0.25,1)",boxShadow:"0 2px 20px rgba(0,0,0,0.25), inset 0 0.5px 0 rgba(255,255,255,0.05)",...style}} onMouseEnter={e=>{if(hover||onClick){e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 8px 30px rgba(0,0,0,0.35), inset 0 0.5px 0 rgba(255,255,255,0.08)"}}} onMouseLeave={e=>{if(hover||onClick){e.currentTarget.style.transform="translateY(0)";e.currentTarget.style.boxShadow="0 2px 20px rgba(0,0,0,0.25), inset 0 0.5px 0 rgba(255,255,255,0.05)"}}}>{children}</div>;
...
const StatCard = ({label,value,sub,icon,color="#c8a25c"}) => <Card style={{display:"flex",flexDirection:"column",gap:8,cursor:"default"}} hover>{<div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:"#8e8e93",fontWeight:500,textTransform:"uppercase",letterSpacing:1.2}}>{label}</span><span style={{color,opacity:0.5}}><I n={icon} s={16}/></span></div>}<div style={{fontSize:28,fontWeight:700,color:"#f5f5f4",fontFamily:"'DM Mono',monospace",letterSpacing:-0.5}}>{value}</div>{sub&&<div style={{fontSize:12,color:"#8e8e93"}}>{sub}</div>}</Card>;
...
const Btn = ({children,onClick,v="primary",style:s}) => {const st={primary:{background:"linear-gradient(135deg,#c8a25c,#b8923c)",color:"#161618",fontWeight:600},secondary:{background:"#48484a",color:"#e8e6e3",border:"1px solid #2a2d37"},ghost:{background:"transparent",color:"#c8a25c",border:"1px solid #c8a25c30"},danger:{background:"#ef444420",color:"#ef4444",border:"1px solid #ef444430"}};return <button onClick={onClick} style={{padding:"8px 16px",borderRadius:8,border:"none",cursor:"pointer",fontSize:13,fontFamily:"inherit",display:"inline-flex",alignItems:"center",gap:6,transition:"all 0.2s",...st[v],...s}}>{children}</button>};
const Bar = ({value,max,color="#c8a25c",height=6}) => <div style={{width:"100%",height,background:"rgba(255,255,255,0.06)",borderRadius:height,overflow:"hidden"}}><div style={{width:`${Math.min((value/(max||1))*100,100)}%`,height:"100%",background:color,borderRadius:height,transition:"width 0.6s cubic-bezier(0.25,0.1,0.25,1)"}}/></div>;

const Tbl = ({columns,data,onRowClick}) => <div style={{overflowX:"auto",borderRadius:10,border:"1px solid #1e2130"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}><thead><tr style={{background:"#2c2c2e"}}>{columns.map((c,i)=><th key={i} style={{padding:"10px 14px",textAlign:"left",fontWeight:600,color:"#6b7280",fontSize:11,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid #1e2130"}}>{c.header}</th>)}</tr></thead><tbody>{data.map((row,ri)=><tr key={row.id||ri} onClick={()=>onRowClick?.(row)} style={{cursor:onRowClick?"pointer":"default",transition:"background 0.15s",background:ri%2===1?"#0d0f1440":"transparent"}} onMouseEnter={e=>{e.currentTarget.style.background=onRowClick?"#c8a25c0a":"#16182144"}} onMouseLeave={e=>{e.currentTarget.style.background=ri%2===1?"#0d0f1440":"transparent"}}>{columns.map((c,ci)=><td key={ci} style={{padding:"10px 14px",borderBottom:"1px solid #1a1d2710",color:"#d1d5db"}}>{c.render?c.render(row):row[c.key]}</td>)}</tr>)}</tbody></table></div>;
  ...
  const ConfirmDialog = () => {
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 99999, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => respond(false)}>
        <div onClick={e => e.stopPropagation()} style={{ background: "#2c2c2e", border: "1px solid #1e2130", borderRadius: 12, padding: 24, maxWidth: 400, width: "90%" }}>
          <div style={{ fontSize: 15, fontWeight: 600, color: "#f5f5f4", marginBottom: 16 }}>{state.message}</div>
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button onClick={() => respond(false)} style={{ padding: "8px 16px", borderRadius: 8, border: "1px solid #2a2d37", background: "#48484a", color: "#e8e6e3", fontSize: 13, cursor: "pointer", fontFamily: "inherit" }}>Cancel</button>
...
export default function MidwestAIOS() {
  ...
  if (!appReady) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",width:"100vw",background:"#1c1c1e",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{textAlign:"center"}}>
        ...
        <div style={{width:120,height:3,background:"#48484a",borderRadius:2,margin:"0 auto",overflow:"hidden"}}><div style={{width:"40%",height:"100%",background:"#c8a25c",borderRadius:2,animation:"loadSlide 1.2s ease-in-out infinite"}}/></div>
  ...
  return (
    <div style={{display:"flex",height:"100vh",width:"100vw",fontFamily:"'DM Sans','Satoshi',sans-serif",background:"#161618",color:"#e8e6e3",overflow:"hidden"}}>
      ...
      {searchOpen&&<div style={{position:"fixed",inset:0,zIndex:99998,display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:"15vh"}}><div onClick={()=>setSearchOpen(false)} style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.6)",backdropFilter:"blur(4px)"}}/><div style={{position:"relative",width:"100%",maxWidth:560,background:"rgba(44,44,46,0.95)",backdropFilter:"blur(24px)",borderRadius:16,border:"1px solid rgba(255,255,255,0.08)",boxShadow:"0 24px 80px rgba(0,0,0,0.5)",overflow:"hidden",animation:"fadeUp 0.2s cubic-bezier(0.25,0.1,0.25,1)"}}><div style={{padding:"16px 20px",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",alignItems:"center",gap:12}}><I n="search" s={18}/><input autoFocus value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} placeholder="Search jobs, vendors, customers, items..." style={{flex:1,background:"transparent",border:"none",outline:"none",color:"#f5f5f4",fontSize:16,fontFamily:"inherit"}}/><span style={{fontSize:11,color:"#636366",background:"#3a3a3c",padding:"2px 8px",borderRadius:4}}>ESC</span></div>{searchResults.length>0&&<div style={{maxHeight:400,overflow:"auto",padding:"8px"}}>{searchResults.map((r,i)=><div key={r.id+i} onClick={r.action} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",borderRadius:10,cursor:"pointer",transition:"background 0.15s"}} onMouseEnter={e=>e.currentTarget.style.background="rgba(255,255,255,0.06)"} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><div style={{width:32,height:32,borderRadius:8,background:r.type==="Job"?"#c8a25c18":r.type==="Vendor"?"#2563eb18":r.type==="Customer"?"#05966918":"#d9770618",display:"flex",alignItems:"center",justifyContent:"center"}}><I n={r.type==="Job"?"briefcase":r.type==="Vendor"?"package":r.type==="Customer"?"users":"file"} s={14}/></div><div style={{flex:1,minWidth:0}}><div style={{fontSize:13,fontWeight:600,color:"#f5f5f4",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{r.name}</div><div style={{fontSize:11,color:"#8e8e93"}}>{r.sub}</div></div><Badge label={r.type} color={r.type==="Job"?"#c8a25c":r.type==="Vendor"?"#2563eb":"#059669"}/></div>)}</div>}{searchQuery.length>=2&&searchResults.length===0&&<div style={{padding:40,textAlign:"center",color:"#636366",fontSize:13}}>No results for "{searchQuery}"</div>}{searchQuery.length<2&&<div style={{padding:30,textAlign:"center",color:"#636366",fontSize:12}}>Type at least 2 characters to search</div>}</div></div>}
      {notification&&<div style={{position:"fixed",top:20,right:20,zIndex:99999,background:notification.type==="error"?"rgba(255,69,58,0.95)":"rgba(48,209,88,0.95)",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)",color:"#fff",padding:"14px 24px",borderRadius:50,fontSize:13,fontWeight:600,boxShadow:"0 8px 32px rgba(0,0,0,0.4)",animation:"fadeUp 0.35s cubic-bezier(0.25,0.1,0.25,1)",maxWidth:"calc(100vw - 48px)",display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:16}}>{notification.type==="error"?"!":"+"}</span>{notification.msg}</div>}
      <PrintOverlay />
      <ConfirmDialog />

      {/* Mobile hamburger */}
      {!mobileMenuOpen&&<button onClick={()=>setMobileMenuOpen(true)} className="mobile-menu-btn" style={{display:"none",position:"fixed",top:12,left:12,zIndex:10001,width:40,height:40,borderRadius:8,background:"#2c2c2e",border:"1px solid #1e2130",cursor:"pointer",alignItems:"center",justifyContent:"center",color:"#c8a25c"}}><I n="dashboard" s={20}/></button>}
      {mobileMenuOpen&&<div onClick={()=>setMobileMenuOpen(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:9999}}/>}

      {/* SIDEBAR */}
      <div className={"sidebar"+(mobileMenuOpen?" open":"")} style={{width:sidebarCollapsed?64:240,minWidth:sidebarCollapsed?64:240,background:"rgba(28,28,30,0.82)",backdropFilter:"blur(24px)",WebkitBackdropFilter:"blur(24px)",borderRight:"1px solid rgba(255,255,255,0.06)",display:"flex",flexDirection:"column",transition:"all 0.3s",overflow:"hidden"}}>
        <div style={{padding:"16px",borderBottom:"1px solid #1a1d27",display:"flex",alignItems:"center",gap:10}}>
          ...
          <button className="mobile-close-btn" onClick={()=>setMobileMenuOpen(false)} style={{display:"none",width:28,height:28,borderRadius:6,background:"transparent",border:"1px solid #2a2d37",cursor:"pointer",alignItems:"center",justifyContent:"center",color:"#6b7280",flexShrink:0,padding:0}}><I n="close" s={14}/></button>
        </div>
        ...
        {!sidebarCollapsed&&<div style={{padding:"12px 20px",borderTop:"1px solid #1a1d27",fontSize:10,color:"#4b5563"}}>
          ...
          <div style={{color:"#c8a25c",fontSize:9}}>Powered by DYFRENT</div>
        </div>}
      </div>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        button{-webkit-tap-highlight-color:transparent}button:active{transform:scale(0.97)!important;transition:transform 0.1s!important}
...
function Dashboard({jobs,lineItems,reps,vendors,customers,getJobFinancials,getJobItems,setPage,setSelectedJob,dateFilter,setDateFilter}){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}><div style={{marginBottom:28}}><div style={{display:"flex",alignItems:"center",gap:12,marginBottom:8,flexWrap:"wrap"}}><h1 style={{fontSize:28,fontWeight:800,color:"#f5f5f4",letterSpacing:-1}}>Command Center</h1><div style={{padding:"4px 10px",background:"#059669",borderRadius:20,fontSize:10,fontWeight:700,color:"#fff",letterSpacing:0.5}}>LIVE</div>
    <div style={{marginLeft:"auto",display:"flex",gap:4,background:"#2c2c2e",padding:3,borderRadius:8}}>{[["all","All Time"],["ytd","YTD"],["quarter","Quarter"],["month","Month"]].map(([v,l])=> <button key={v} onClick={()=>setDateFilter(v)} style={{padding:"5px 12px",borderRadius:6,border:"none",cursor:"pointer",background:dateFilter===v?"#c8a25c":"transparent",color:dateFilter===v?"#161618":"#6b7280",fontSize:11,fontWeight:dateFilter===v?600:400,fontFamily:"inherit"}}>{l}</button>)}</div>
  </div><div style={{display:"flex",gap:6,marginBottom:8}}><Btn v="secondary" style={{fontSize:10,padding:"4px 10px"}} onClick={()=>{exportCSV(["Job","Customer","Phase","Revenue","Cost","Margin","Payment"],jobs.map(j=>{const f=getJobFinancials(j.id);return[j.name,customers?.find(c=>c.id===j.customer)?.name||"",j.phase,f.totalRevenue.toFixed(2),f.totalCost.toFixed(2),f.margin.toFixed(1)+"%",j.paymentStatus]}),"midwest-jobs-export.csv")}}>Export Jobs CSV</Btn><Btn v="secondary" style={{fontSize:10,padding:"4px 10px"}} onClick={()=>{exportCSV(["Vendor","Category","Discount","Spend"],vendors.map(v=>{const spend=lineItems.filter(i=>i.vendor===v.id).reduce((s,i)=>s+i.unitCost*i.qtyOrdered,0);return[v.name,v.category,(v.discountRate*100).toFixed(0)+"%",spend.toFixed(2)]}),"midwest-vendors-export.csv")}}>Export Vendors CSV</Btn></div><p style={{fontSize:14,color:"#6b7280"}}>Midwest Educational Furnishings — {dateFilter==="all"?"All time":dateFilter==="ytd"?"Year to date":dateFilter==="quarter"?"This quarter":"This month"} · {filtered.length} jobs</p></div>
    ...
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:24}}>
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:16,display:"flex",alignItems:"center",gap:8}}><I n="alert" s={16}/> Action Required</div><div style={{display:"flex",flexDirection:"column",gap:10}}>
        {pendingInv>0&&<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:"#d9770610",border:"1px solid #d9770625",borderRadius:8,cursor:"pointer"}} onClick={()=>setPage("documents")}><div style={{width:8,height:8,borderRadius:"50%",background:"#d97706"}}/><span style={{fontSize:13,color:"#d97706",flex:1}}><strong>{pendingInv}</strong> line items received but not yet invoiced</span><Btn v="ghost" style={{fontSize:11,padding:"4px 10px"}} onClick={()=>setPage("documents")}>Draft Invoices &rarr;</Btn></div>}
        {pendingDel>0&&<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:"#2563eb10",border:"1px solid #2563eb25",borderRadius:8,cursor:"pointer"}} onClick={()=>setPage("deliveries")}><div style={{width:8,height:8,borderRadius:"50%",background:"#2563eb"}}/><span style={{fontSize:13,color:"#93c5fd",flex:1}}><strong>{pendingDel}</strong> line items awaiting delivery</span><Btn v="ghost" style={{fontSize:11,padding:"4px 10px"}} onClick={()=>setPage("deliveries")}>Track &rarr;</Btn></div>}
      ...
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:16,display:"flex",alignItems:"center",gap:8}}><I n="briefcase" s={16}/> Active Jobs</div>{jobs.length===0&&<div style={{textAlign:"center",padding:20,color:"#6b7280",fontSize:13}}>No jobs yet. Go to Job Records to create your first project.</div>}<div style={{display:"flex",flexDirection:"column",gap:8}}>{jobs.map(job=>{const f=getJobFinancials(job.id);return <div key={job.id} onClick={()=>{setSelectedJob(job.id);setPage("jobs")}} style={{padding:"10px 12px",background:"#3a3a3c",borderRadius:8,cursor:"pointer",transition:"all 0.15s"}} onMouseEnter={e=>e.currentTarget.style.background="#48484a"} onMouseLeave={e=>e.currentTarget.style.background="#3a3a3c"}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13,fontWeight:600,color:"#e8e6e3"}}>{job.name}</span><Badge label={job.phase} color={statusColor(job.phase)}/></div><Bar value={f.totalReceived} max={f.totalOrdered||1} color={statusColor(job.phase)} height={4}/><div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontSize:11,color:"#6b7280"}}>{fmtN(f.totalReceived)}/{fmtN(f.totalOrdered)} units</span><span style={{fontSize:11,color:"#6b7280"}}>{fmt(f.totalRevenue)}</span></div></div>})}</div></Card>
    </div>
    {/* ─── PIPELINE DONUT + REVENUE BREAKDOWN ──────────────── */}
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:20}}>Pipeline by Phase</div>
        {(()=>{const phases=["Quoting","In Progress","Invoiced","Complete"];const data=phases.map(p=>{const pj=filtered.filter(j=>j.phase===p);return{phase:p,count:pj.length,rev:pj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0)}});const total=data.reduce((s,d)=>s+d.rev,0)||1;const cx=100,cy=100,r=80,r2=60;let cumAngle=-Math.PI/2;return <div style={{display:"flex",alignItems:"center",gap:24}}><svg viewBox="0 0 200 200" style={{width:180,height:180,flexShrink:0}}>{data.map((d,i)=>{const angle=(d.rev/total)*Math.PI*2;const startAngle=cumAngle;cumAngle+=angle;const endAngle=cumAngle;const x1=cx+r*Math.cos(startAngle),y1=cy+r*Math.sin(startAngle);const x2=cx+r*Math.cos(endAngle),y2=cy+r*Math.sin(endAngle);const x3=cx+r2*Math.cos(endAngle),y3=cy+r2*Math.sin(endAngle);const x4=cx+r2*Math.cos(startAngle),y4=cy+r2*Math.sin(startAngle);const large=angle>Math.PI?1:0;if(d.rev===0)return null;return <path key={i} d={`M${x1},${y1} A${r},${r} 0 ${large},1 ${x2},${y2} L${x3},${y3} A${r2},${r2} 0 ${large},0 ${x4},${y4} Z`} fill={statusColor(d.phase)} opacity={0.85} style={{transition:"opacity 0.2s",cursor:"pointer"}} onMouseEnter={e=>e.currentTarget.style.opacity="1"} onMouseLeave={e=>e.currentTarget.style.opacity="0.85"}/>})}<text x={cx} y={cy-6} textAnchor="middle" fill="#f5f5f4" fontSize={20} fontWeight={700} fontFamily="DM Mono">{fmt(total===1?0:total)}</text><text x={cx} y={cy+12} textAnchor="middle" fill="#8e8e93" fontSize={9}>PIPELINE</text></svg><div style={{display:"flex",flexDirection:"column",gap:10}}>{data.map(d=><div key={d.phase} style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:10,height:10,borderRadius:3,background:statusColor(d.phase),flexShrink:0}}/><div><div style={{fontSize:12,fontWeight:600,color:"#e8e6e3"}}>{d.phase}</div><div style={{fontSize:11,color:"#8e8e93"}}>{d.count} job{d.count!==1?"s":""} · {fmt(d.rev)}</div></div></div>)}</div></div>})()}
      </Card>

      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:20}}>Revenue & Margin by Job</div>
        {(()=>{const data=filtered.map(j=>({name:j.name.length>18?j.name.slice(0,18)+"...":j.name,rev:getJobFinancials(j.id).totalRevenue,margin:getJobFinancials(j.id).margin}));const mx=Math.max(...data.map(d=>d.rev),1);return <div style={{display:"flex",flexDirection:"column",gap:6}}>{data.map((d,i)=><div key={i} style={{display:"flex",alignItems:"center",gap:10}}><span style={{fontSize:11,color:"#8e8e93",width:120,flexShrink:0,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{d.name}</span><div style={{flex:1,position:"relative",height:24,background:"rgba(255,255,255,0.04)",borderRadius:6,overflow:"hidden"}}><div style={{position:"absolute",left:0,top:0,bottom:0,width:(d.rev/mx*100)+"%",background:"linear-gradient(90deg,#c8a25c,#d4a843)",borderRadius:6,transition:"width 0.6s cubic-bezier(0.25,0.1,0.25,1)"}}/><span style={{position:"absolute",right:8,top:4,fontSize:10,fontWeight:600,color:"#fff",fontFamily:"'DM Mono',monospace",zIndex:1}}>{fmt(d.rev)}</span></div><span style={{fontSize:11,fontWeight:600,color:d.margin>=30?"#30d158":d.margin>=20?"#ff9f0a":"#ff453a",width:44,textAlign:"right",flexShrink:0}}>{d.margin.toFixed(0)}%</span></div>)}</div>})()}
        <div style={{display:"flex",gap:16,marginTop:12,justifyContent:"center"}}><span style={{fontSize:10,color:"#c8a25c",display:"flex",alignItems:"center",gap:4}}><span style={{width:12,height:6,background:"linear-gradient(90deg,#c8a25c,#d4a843)",borderRadius:2,display:"inline-block"}}/> Revenue</span><span style={{fontSize:10,color:"#30d158",display:"flex",alignItems:"center",gap:4}}>Green = 30%+</span><span style={{fontSize:10,color:"#ff9f0a",display:"flex",alignItems:"center",gap:4}}>Amber = 20-30%</span><span style={{fontSize:10,color:"#ff453a",display:"flex",alignItems:"center",gap:4}}>Red = &lt;20%</span></div>
      </Card>
    </div>

    {/* ─── TEAM LEADERBOARD + TOP VENDORS ──────────────────── */}
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:16}}>
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:16}}>Team Leaderboard</div>{[...reps].filter(r=>!r.id.includes("SEED_FLAG")).sort((a,b)=>{const ar=jobs.filter(j=>j.salesRep===a.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const br=jobs.filter(j=>j.salesRep===b.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return br-ar}).map((rep,idx)=>{const rj=jobs.filter(j=>j.salesRep===rep.id);const rv=rj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return <div key={rep.id} style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}><div style={{width:28,height:28,borderRadius:8,background:idx===0?"#c8a25c20":idx===1?"#9ca3af20":"#63636620",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:idx===0?"#c8a25c":idx===1?"#9ca3af":"#636366"}}>{idx+1}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#e8e6e3"}}>{rep.name}</div><div style={{fontSize:11,color:"#8e8e93"}}>{rep.territory} · {rj.length} jobs · {pct(rep.commissionRate*100)}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:14,fontWeight:700,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{fmt(rv)}</div><div style={{fontSize:10,color:"#30d158"}}>comm: {fmt(rv*rep.commissionRate)}</div></div></div>})}</Card>
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:16}}>Top Vendors by Spend</div>{(()=>{const vt={};lineItems.forEach(i=>{const v=vendors.find(v=>v.id===i.vendor);if(v)vt[v.name]=(vt[v.name]||0)+i.unitCost*i.qtyOrdered});const sorted=Object.entries(vt).sort((a,b)=>b[1]-a[1]).slice(0,8);const mx=sorted[0]?.[1]||1;return sorted.map(([name,total],i)=><div key={name} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}><div style={{width:28,height:28,borderRadius:8,background:"#2563eb18",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:700,color:"#60a5fa"}}>{i+1}</div><div style={{flex:1}}><div style={{fontSize:12,color:"#e8e6e3",marginBottom:4}}>{name}</div><Bar value={total} max={mx} color="#60a5fa" height={4}/></div><span style={{fontSize:12,fontWeight:600,color:"#60a5fa",fontFamily:"'DM Mono',monospace",flexShrink:0}}>{fmt(total)}</span></div>)})()}</Card>
    </div>
  </div>;
}
...
function JobsPage(ctx){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}>
    ...
    <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"center",flexWrap:"wrap"}}>
      <input value={ctx.globalSearch} onChange={e=>ctx.setGlobalSearch(e.target.value)} placeholder="Search jobs..." style={{...inputStyle,maxWidth:300,background:"#2c2c2e",border:"1px solid #1e2130",padding:"10px 14px",fontSize:14}}/>
      <div style={{display:"flex",gap:4,background:"#2c2c2e",padding:3,borderRadius:8}}>
        {[["table","Table"],["kanban","Kanban"]].map(([v,l])=><button key={v} onClick={()=>setViewMode(v)} style={{padding:"5px 12px",borderRadius:6,border:"none",cursor:"pointer",background:viewMode===v?"#c8a25c":"transparent",color:viewMode===v?"#161618":"#6b7280",fontSize:11,fontWeight:viewMode===v?600:400,fontFamily:"inherit"}}>{l}</button>)}
      </div>
      <div style={{display:"flex",gap:4,background:"#2c2c2e",padding:3,borderRadius:8,marginLeft:"auto"}}>
    ...
    {viewMode==="kanban"&&<div className="kanban-grid" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,minHeight:400}}>
      {["Quoting","In Progress","Invoiced","Complete"].map(phase=><div key={phase} onDrop={e=>handleDrop(e,phase)} onDragOver={handleDragOver} style={{background:"#2c2c2e",borderRadius:12,padding:12,border:"1px solid #1e2130",minHeight:300}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {sortedJobs.filter(j=>j.phase===phase).map(job=>{const f=getJobFinancials(job.id);return <div key={job.id} draggable onDragStart={e=>handleDragStart(e,job.id)} onClick={()=>setSelectedJob(job.id)} style={{background:"#3a3a3c",borderRadius:8,padding:12,cursor:"grab",border:"1px solid #2a2d37",transition:"all 0.15s"}} onMouseEnter={e=>{e.currentTarget.style.borderColor="#c8a25c44";e.currentTarget.style.transform="translateY(-1px)"}} onMouseLeave={e=>{e.currentTarget.style.borderColor="#636366";e.currentTarget.style.transform="translateY(0)"}}>
...
function JobDetail({job,ctx}){
  ...
  return <div style={{animation:"fadeUp 0.4s cubic-bezier(0.25,0.1,0.25,1)"}}>
    <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(22,22,24,0.9)",backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",margin:"-28px -36px 0",padding:"16px 36px 0",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
      <button onClick={()=>setSelectedJob(null)} style={{background:"none",border:"none",color:"#c8a25c",cursor:"pointer",fontSize:13,fontFamily:"inherit",marginBottom:12,display:"flex",alignItems:"center",gap:6}}>&larr; All Jobs</button>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
        <div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:2}}><h2 style={{fontSize:22,fontWeight:800,color:"#f5f5f4",margin:0}}>{job.name}</h2><Badge label={job.phase} color={statusColor(job.phase)}/><Badge label={job.paymentStatus} color={statusColor(job.paymentStatus)}/></div><div style={{fontSize:12,color:"#8e8e93"}}>{job.id} · {customer?.name} · {rep?.name} · {fmt(f.totalRevenue)} rev · {pct(f.margin)} margin</div></div>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}><Btn v="danger" onClick={()=>deleteJob(job.id)} style={{fontSize:11,padding:"6px 10px"}}><I n="close" s={12}/> Delete</Btn><Btn v="secondary" onClick={()=>setEditing(!editing)} style={{fontSize:11,padding:"6px 10px"}}><I n="edit" s={12}/> Edit</Btn><Btn v="secondary" onClick={duplicateJob} style={{fontSize:11,padding:"6px 10px"}}><I n="package" s={12}/> Duplicate</Btn><Btn onClick={()=>setAddingItem(true)} style={{fontSize:11,padding:"6px 10px"}}><I n="plus" s={12}/> Add Item</Btn></div>
      </div>
      <div style={{display:"flex",gap:0,marginBottom:0}}>{[["items","Line Items",items.length],["details","Details",null],["timeline","Timeline",null],["activity","Activity",activities.length]].map(([id,label,count])=><button key={id} onClick={()=>setDetailTab(id)} style={{padding:"10px 18px",border:"none",borderBottom:detailTab===id?"2px solid #c8a25c":"2px solid transparent",background:"transparent",color:detailTab===id?"#c8a25c":"#8e8e93",fontSize:13,fontWeight:detailTab===id?600:400,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>{label}{count!==null&&<span style={{fontSize:10,color:detailTab===id?"#c8a25c":"#636366"}}>{count}</span>}</button>)}</div>
    </div>
    ...
    <div className="resp-grid-4" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:24}}>
      <Card style={{padding:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:"#6b7280"}}>Payment</span><Badge label={job.paymentStatus} color={statusColor(job.paymentStatus)}/></div></Card>
      <Card style={{padding:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:"#6b7280"}}>Delivery</span><span style={{fontSize:12,fontWeight:600,color:f.totalReceived===f.totalOrdered?"#059669":f.totalReceived>0?"#d97706":"#6b7280"}}>{fmtN(f.totalReceived)}/{fmtN(f.totalOrdered)}</span></div><Bar value={f.totalReceived} max={f.totalOrdered||1} color={f.totalReceived===f.totalOrdered?"#059669":"#d97706"} height={3}/></Card>
      <Card style={{padding:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:"#6b7280"}}>Invoiced</span><span style={{fontSize:12,fontWeight:600,color:items.every(i=>i.qtyInvoiced>=i.qtyOrdered)?"#059669":"#c8a25c"}}>{fmt(items.reduce((s,i)=>s+i.unitPrice*i.qtyInvoiced,0))}</span></div><Bar value={items.reduce((s,i)=>s+i.qtyInvoiced,0)} max={items.reduce((s,i)=>s+i.qtyOrdered,0)||1} color="#c8a25c" height={3}/></Card>
      <Card style={{padding:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{fontSize:11,color:"#6b7280"}}>Due Date</span><span style={{fontSize:12,fontWeight:600,color:job.dueDate&&new Date(job.dueDate)<new Date()&&job.paymentStatus!=="paid"?"#ef4444":"#d1d5db"}}>{job.dueDate||"Not set"}</span></div>{job.dueDate&&new Date(job.dueDate)<new Date()&&job.paymentStatus!=="paid"&&<span style={{fontSize:10,color:"#ef4444"}}>OVERDUE</span>}</Card>
    </div>

    {addingItem&&<Card style={{marginBottom:20,border:"1px solid #05966930"}}><div style={{fontSize:14,fontWeight:700,marginBottom:12,color:"#059669"}}>Add Line Item</div><div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr 1fr 1fr 1fr",gap:8,marginBottom:12}}><div><label style={{fontSize:10,color:"#6b7280",display:"block",marginBottom:3}}>Description</label><input value={newItem.description} onChange={e=>setNewItem({...newItem,description:e.target.value})} style={{...inputStyle,fontSize:12}}/></div><div><label style={{fontSize:10,color:"#6b7280",display:"block",marginBottom:3}}>Vendor</label><select value={newItem.vendor} onChange={e=>{const vid=e.target.value;const calc=autoCalcFromList(parseFloat(newItem.listPrice)||0,vid);setNewItem({...newItem,vendor:vid,...calc})}} style={{...inputStyle,fontSize:12}}>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}{v.discountRate?" ("+((v.discountRate*100).toFixed(0))+"% off)":""}</option>)}</select></div><div><label style={{fontSize:10,color:"#6b7280",display:"block",marginBottom:3}}>List Price</label><input type="number" value={newItem.listPrice} onChange={e=>{const lp=parseFloat(e.target.value)||0;const calc=autoCalcFromList(lp,newItem.vendor);setNewItem({...newItem,listPrice:lp,...calc})}} style={{...inputStyle,fontSize:12}} placeholder="Enter list price"/></div><div><label style={{fontSize:10,color:"#059669",display:"block",marginBottom:3}}>Cost (auto)</label><input type="number" value={newItem.unitCost} onChange={e=>setNewItem({...newItem,unitCost:parseFloat(e.target.value)||0})} style={{...inputStyle,fontSize:12,borderColor:"#05966930"}}/></div><div><label style={{fontSize:10,color:"#c8a25c",display:"block",marginBottom:3}}>Sell Price</label><input type="number" value={newItem.unitPrice} onChange={e=>setNewItem({...newItem,unitPrice:parseFloat(e.target.value)||0})} style={{...inputStyle,fontSize:12,borderColor:"#c8a25c30"}}/></div><div><label style={{fontSize:10,color:"#6b7280",display:"block",marginBottom:3}}>Qty Ordered</label><input type="number" value={newItem.qtyOrdered} onChange={e=>setNewItem({...newItem,qtyOrdered:e.target.value})} style={{...inputStyle,fontSize:12}}/></div></div>{newItem.listPrice>0&&<div style={{fontSize:11,color:"#059669",marginBottom:8}}>Vendor discount: {((vendors.find(v=>v.id===newItem.vendor)?.discountRate||0)*100).toFixed(0)}% off list — Cost: {fmt(newItem.unitCost)} — Sell: {fmt(newItem.unitPrice)} — Margin: {newItem.unitPrice>0?((1-newItem.unitCost/newItem.unitPrice)*100).toFixed(1):0}%</div>}<div style={{display:"flex",gap:8}}><Btn onClick={saveNewItem}>Add Item</Btn><Btn v="secondary" onClick={()=>setAddingItem(false)}>Cancel</Btn></div></Card>}

    </>
    }{detailTab==="items"&&<><Header title="Line Items" sub="Click any cell value to edit inline — changes propagate across all views"/>
    <div style={{overflowX:"auto",borderRadius:10,border:"1px solid #1e2130"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{background:"#2c2c2e"}}>{["Item","Vendor","Unit Cost","Unit Price","Ordered","Received","Invoiced","Status","Line Total",""].map((h,i)=><th key={i} style={{padding:"8px 10px",textAlign:"left",fontWeight:600,color:"#6b7280",fontSize:10,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid #1e2130"}}>{h}</th>)}</tr></thead><tbody>{items.map(item=>{
    ...
    <Card style={{marginTop:24}}>
      <div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:16}}>Job Timeline</div>
      <div style={{display:"flex",alignItems:"center",gap:0,overflow:"auto",padding:"8px 0"}}>
        {[
          {label:"Created",date:job.createdDate,done:!!job.createdDate,color:"#6b7280"},
          {label:"Started",date:job.startDate,done:!!job.startDate,color:"#2563eb"},
          {label:"Items Ordered",date:items.length>0?items[0]?.poDate:"",done:items.some(i=>i.qtyOrdered>0),color:"#8b5cf6"},
          {label:"Items Received",date:items.find(i=>i.qtyReceived>0)?.deliveryDate||"",done:items.some(i=>i.qtyReceived>0),color:"#d97706"},
          {label:"Invoiced",date:items.find(i=>i.qtyInvoiced>0)?.invoiceDate||"",done:items.some(i=>i.qtyInvoiced>0),color:"#c8a25c"},
          {label:"Paid",date:job.paymentStatus==="paid"?job.endDate||"":"",done:job.paymentStatus==="paid",color:"#059669"},
        ].map((step,i,arr)=><div key={i} style={{display:"flex",alignItems:"center",flex:1,minWidth:100}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            <div style={{width:24,height:24,borderRadius:"50%",background:step.done?step.color:"#48484a",border:"2px solid "+(step.done?step.color:"#636366"),display:"flex",alignItems:"center",justifyContent:"center"}}>{step.done&&<I n="check" s={12}/>}</div>
            <div style={{fontSize:10,fontWeight:600,color:step.done?step.color:"#4b5563",textAlign:"center"}}>{step.label}</div>
            {step.date&&<div style={{fontSize:9,color:"#6b7280"}}>{step.date}</div>}
          </div>
          {i<arr.length-1&&<div style={{flex:1,height:2,background:step.done?"#636366":"#3a3a3c",margin:"0 4px",marginBottom:20}}/>}
        </div>)}
      </div>
    ...
    <Card style={{marginTop:16}}>
      <div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:12}}>Activity Log</div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:0,maxHeight:300,overflow:"auto"}}>{activities.length===0?<div style={{fontSize:12,color:"#4b5563",padding:12,textAlign:"center"}}>No activity yet. Add notes, status updates, or reminders.</div>:activities.map(a=><div key={a.id} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:"1px solid #1e2130"}}><div style={{width:6,height:6,borderRadius:"50%",background:"#c8a25c",marginTop:6,flexShrink:0}}/><div style={{flex:1}}><div style={{fontSize:12,color:"#d1d5db"}}>{a.text}</div><div style={{fontSize:10,color:"#4b5563",marginTop:2}}>{new Date(a.time).toLocaleString()}</div></div></div>)}</div>
    </Card>
    </>
    }
    </div>
...
function DeliveryPage({jobs,lineItems,vendors,getItemStatus,updateLineItem,notify}){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}><Header title="Delivery Tracker" sub="Grouped by job — quantity-specific tracking — updates propagate everywhere"/>
    <div className="resp-grid-4" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:24}}>
        ...
        <div style={{overflowX:"auto",marginTop:12,borderRadius:8,border:"1px solid #1e2130"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            ...
            <tbody>{items.map(item=><tr key={item.id} style={{borderBottom:"1px solid #1a1d2720"}}>
              ...
              <td style={{padding:"8px 10px"}}><div style={{width:80,margin:"0 auto"}}><Bar value={item.qtyReceived} max={item.qtyOrdered} color={item.qtyReceived>0?"#d97706":"#636366"} height={4}/></div></td>
...
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,getJobFinancials,updateJob,updateLineItem,notify,qbConfig,setPage,triggerPrint,...ctx}){
  ...
  const StatusBadge=({docNum})=>{const s=docStatuses[docNum];if(!s)return <Badge label="new" color="#6b7280"/>;if(s==="drafted")return <Badge label="drafted" color="#d97706"/>;if(s==="sent")return <Badge label="sent" color="#059669"/>;if(s==="approved")return <Badge label="approved" color="#2563eb"/>;return <Badge label={s} color="#6b7280"/>};
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices, Commission Statements"/>
    <div style={{display:"flex",gap:0,alignItems:"center",marginBottom:16,padding:"10px 16px",background:"#2c2c2e",borderRadius:8,fontSize:11,color:"#6b7280"}}><span style={{color:"#c8a25c",fontWeight:600}}>Workflow:</span><span style={{margin:"0 8px"}}>Quote</span><span style={{color:"#4b5563"}}>→</span><span style={{margin:"0 8px",color:"#c8a25c"}}>Approve/Send</span><span style={{color:"#4b5563"}}>→</span><span style={{margin:"0 8px"}}>Purchase Orders</span><span style={{color:"#4b5563"}}>→</span><span style={{margin:"0 8px"}}>Draft/Send</span><span style={{color:"#4b5563"}}>→</span><span style={{margin:"0 8px"}}>Invoices</span><span style={{color:"#4b5563"}}>→</span><span style={{margin:"0 8px",color:"#059669"}}>Mark Paid</span></div>

    <div style={{display:"flex",gap:0,marginBottom:24,background:"#2c2c2e",borderRadius:12,padding:4,overflow:"hidden"}}>
      {[["quotes","Quotes",totalQuotes,"#c8a25c"],["pos","Purchase Orders",totalPOs,"#2563eb"],["invoices","Invoices",pendingInvJobs.length+" pending","#d97706"],["commissions","Commissions",reps.filter(r=>!r.id.includes("SEED_FLAG")).length+" reps","#059669"]].map(([id,label,count,color])=>
        <button key={id} onClick={()=>setTab(id)} style={{flex:1,padding:"14px 12px",borderRadius:8,border:"none",cursor:"pointer",background:tab===id?color+"18":"transparent",color:tab===id?color:"#6b7280",fontSize:13,fontWeight:tab===id?700:400,fontFamily:"inherit",transition:"all 0.2s",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
          <span>{label}</span>
          <span style={{fontSize:18,fontWeight:700,fontFamily:"'DM Mono',monospace"}}>{count}</span>
        </button>
      )}
    </div>

    {tab==="quotes"&&<div>{jobs.map(job=>{const q=genQuote(job);const customer=customers.find(c=>c.id===job.customer);return <Card key={job.id} style={{marginBottom:12}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{job.name}</span><Badge label={job.phase} color={statusColor(job.phase)}/></div><div style={{fontSize:12,color:"#6b7280"}}>{customer?.name} · {q.items.length} items · {fmt(q.total)}</div></div><div style={{display:"flex",alignItems:"center",gap:8}}><StatusBadge docNum={q.docNum}/>{docStatuses[q.docNum]==="approved"&&<span style={{fontSize:10,color:"#059669"}}>POs unlocked</span>}<Btn onClick={()=>{setPreviewDoc({type:"quote",data:q,job});setTab("preview")}}><I n="file" s={14}/> Generate Quote</Btn></div></div></Card>})}</div>}

    {tab==="pos"&&<div>{jobs.map(job=>{const pos=genPOs(job);if(pos.length===0)return null;const quoteDocNum=stableNum('QT-',job.id,job.customer);const quoteStatus=docStatuses[quoteDocNum];const quoteApproved=quoteStatus==="approved"||quoteStatus==="sent";return <Card key={job.id} style={{marginBottom:12,opacity:quoteApproved?1:0.6}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}><div><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{job.name}</span><span style={{fontSize:12,color:"#6b7280",marginLeft:8}}>{pos.length} vendor POs · {fmt(pos.reduce((s,p)=>s+p.total,0))}</span>{!quoteApproved&&<span style={{fontSize:11,color:"#d97706",marginLeft:8}}>Quote must be approved or sent first</span>}</div></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:10}}>{pos.map((po,i)=><div key={i} onClick={()=>{if(!quoteApproved){notify("Approve or send the quote for this job before creating POs","error");return}setPreviewDoc({type:"po",data:po,job});setTab("preview")}} style={{padding:14,background:"#3a3a3c",borderRadius:8,border:"1px solid #1e2130",cursor:"pointer",transition:"border 0.15s"}} onMouseEnter={e=>e.currentTarget.style.borderColor="#c8a25c44"} onMouseLeave={e=>e.currentTarget.style.borderColor="#48484a"}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:13,fontWeight:600,color:"#e8e6e3"}}>{po.vendor?.name}</span><StatusBadge docNum={po.docNum}/></div><div style={{fontSize:12,color:"#6b7280"}}>{po.items.length} items</div><div style={{fontSize:16,fontWeight:700,color:"#c8a25c",fontFamily:"'DM Mono',monospace",marginTop:4}}>{fmt(po.total)}</div></div>)}</div></Card>})}</div>}

    {tab==="invoices"&&<><div style={{marginBottom:12,padding:"12px 16px",background:"#2c2c2e",borderRadius:8,border:"1px solid #1e2130"}}><div style={{fontSize:13,fontWeight:600,color:"#c8a25c",marginBottom:4}}>How Partial Invoicing Works</div><div style={{fontSize:12,color:"#9ca3af"}}>Invoices are auto-generated from received quantities. If 200 units were ordered, 120 received, and 80 already invoiced, the system drafts an invoice for the remaining 40 units. Each invoice only includes items with uninvoiced received quantities.</div></div><div>{[...jobs].sort((a,b)=>{const aHas=getJobItems(a.id).some(i=>i.qtyReceived>i.qtyInvoiced)?0:1;const bHas=getJobItems(b.id).some(i=>i.qtyReceived>i.qtyInvoiced)?0:1;return aHas-bHas}).map(job=>{const inv=genInvoice(job);const items=getJobItems(job.id);const invoicedTotal=items.reduce((s,i)=>s+i.unitPrice*i.qtyInvoiced,0);const fullTotal=items.reduce((s,i)=>s+i.unitPrice*i.qtyOrdered,0);const isComplete=items.length>0&&items.every(i=>i.qtyInvoiced>=i.qtyOrdered);const hasPending=inv.items.length>0;const customer=customers.find(c=>c.id===job.customer);return <Card key={job.id} style={{marginBottom:12,opacity:isComplete?0.65:1,border:hasPending?"1px solid #d9770630":"1px solid #1e2130"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{fontSize:14,fontWeight:700,color:"#f5f5f4"}}>{job.name}</span>{isComplete?<Badge label="fully invoiced" color="#059669"/>:hasPending?<Badge label="partial ready" color="#d97706"/>:<Badge label="awaiting delivery" color="#6b7280"/>}<StatusBadge docNum={inv.docNum}/></div><div style={{fontSize:12,color:"#6b7280"}}>{customer?.name} · {fmt(invoicedTotal)} of {fmt(fullTotal)} invoiced</div></div><div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>{(()=>{const jobPOs=genPOs(job);const anyPODrafted=jobPOs.some(po=>docStatuses[po.docNum]&&docStatuses[po.docNum]!=="new");return hasPending?anyPODrafted?<Btn onClick={()=>{setPreviewDoc({type:"invoice",data:inv,job});setTab("preview")}}><I n="receipt" s={14}/> Draft Partial Invoice ({fmt(inv.total)})</Btn>:<span style={{fontSize:11,color:"#d97706",padding:"6px 10px"}}>Draft POs first</span>:null})()}{job.paymentStatus!=="paid"&&<Btn v="ghost" style={{fontSize:11}} onClick={()=>{updateJob(job.id,{paymentStatus:isComplete?"paid":"partial"});notify(isComplete?"Marked as paid":"Marked partial payment")}}>Mark {isComplete?"Paid":"Partial"}</Btn>}{job.paymentStatus==="paid"&&<Badge label="paid" color="#059669"/>}</div></div><Bar value={invoicedTotal} max={fullTotal||1} color={isComplete?"#059669":"#d97706"} height={4}/>{hasPending&&<div style={{marginTop:10,borderTop:"1px solid #1e2130",paddingTop:10}}><div style={{fontSize:11,fontWeight:600,color:"#d97706",marginBottom:6}}>Ready to invoice ({inv.items.length} items · {fmt(inv.total)}):</div>{inv.items.map((item,idx)=><div key={idx} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"4px 0",fontSize:11,color:"#9ca3af"}}><span style={{flex:1}}>{item.description}</span><span style={{display:"flex",gap:12,fontFamily:"'DM Mono',monospace",fontSize:10}}><span>ordered: {fmtN(item.qtyOrdered)}</span><span>received: {fmtN(item.qtyReceived)}</span><span>invoiced: {fmtN(item.qtyInvoiced)}</span><span style={{color:"#d97706",fontWeight:600}}>this invoice: {fmtN(item.displayQty)} x {fmt(item.displayPrice)} = {fmt(item.displayQty*item.displayPrice)}</span></span></div>)}</div>}</Card>})}</div></>}

    {tab==="commissions"&&<div>{reps.filter(r=>!r.id.includes("SEED_FLAG")&&r.commissionRate>0).map(rep=>{const rj=jobs.filter(j=>j.salesRep===rep.id);const totalRev=rj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const comm=totalRev*rep.commissionRate;const paidRev=rj.filter(j=>j.paymentStatus==="paid").reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const paidComm=paidRev*rep.commissionRate;const unpaidComm=comm-paidComm;const docNum=stableNum('COMM-',rep.id,'stmt');return <Card key={rep.id} style={{marginBottom:12,border:unpaidComm>0?"1px solid #d9770625":"1px solid #1e2130"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{flex:1}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}><span style={{fontSize:15,fontWeight:700,color:"#f5f5f4"}}>{rep.name}</span><Badge label={rep.tier} color="#c8a25c"/><Badge label={pct(rep.commissionRate*100)} color="#059669"/><StatusBadge docNum={docNum}/></div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(100px,1fr))",gap:8,fontSize:12}}><div style={{padding:"6px 10px",background:"#2c2c2e",borderRadius:6,textAlign:"center"}}><div style={{fontSize:10,color:"#6b7280"}}>Jobs</div><div style={{fontWeight:700,color:"#f5f5f4",fontFamily:"'DM Mono',monospace"}}>{rj.length}</div></div><div style={{padding:"6px 10px",background:"#2c2c2e",borderRadius:6,textAlign:"center"}}><div style={{fontSize:10,color:"#6b7280"}}>Pipeline</div><div style={{fontWeight:700,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{fmt(totalRev)}</div></div><div style={{padding:"6px 10px",background:"#05966910",borderRadius:6,textAlign:"center"}}><div style={{fontSize:10,color:"#059669"}}>Earned</div><div style={{fontWeight:700,color:"#059669",fontFamily:"'DM Mono',monospace"}}>{fmt(paidComm)}</div></div><div style={{padding:"6px 10px",background:"#d9770610",borderRadius:6,textAlign:"center"}}><div style={{fontSize:10,color:"#d97706"}}>Pending</div><div style={{fontWeight:700,color:"#d97706",fontFamily:"'DM Mono',monospace"}}>{fmt(unpaidComm)}</div></div><div style={{padding:"6px 10px",background:"#c8a25c10",borderRadius:6,textAlign:"center"}}><div style={{fontSize:10,color:"#c8a25c"}}>Total Comm.</div><div style={{fontWeight:700,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{fmt(comm)}</div></div></div></div><div style={{display:"flex",flexDirection:"column",gap:6,marginLeft:16}}><Btn onClick={()=>{const items=rj.map(j=>{const f=getJobFinancials(j.id);const isPaid=j.paymentStatus==="paid";return{description:j.name+' — '+(customers.find(c=>c.id===j.customer)?.name||'')+' — '+(isPaid?'PAID':'PENDING'),displayQty:1,displayPrice:f.totalRevenue*rep.commissionRate}});setPreviewDoc({type:"commission",data:{rep,items,total:comm,docNum},job:{id:'ALL',name:rep.name+' Commission Statement',notes:'Period: '+new Date().toLocaleDateString()+'\nEarned (paid jobs): '+fmt(paidComm)+'\nPending (unpaid jobs): '+fmt(unpaidComm)+'\nTotal commission: '+fmt(comm)}});setTab("preview")}}><I n="file" s={14}/> Generate Statement</Btn>{rj.length>0&&<div style={{display:"flex",gap:4}}>{["drafted","sent","approved"].map(s=><button key={s} onClick={()=>setDocStatus(docNum,s)} style={{padding:"3px 8px",borderRadius:4,border:"1px solid "+(docStatuses[docNum]===s?"#c8a25c":"#636366"),background:docStatuses[docNum]===s?"#c8a25c15":"transparent",color:docStatuses[docNum]===s?"#c8a25c":"#4b5563",fontSize:9,fontFamily:"inherit",cursor:"pointer"}}>{s}</button>)}</div>}</div></div>{rj.length>0&&<div style={{marginTop:10}}><Bar value={paidRev} max={totalRev||1} color="#059669" height={3}/></div>}{rj.length>0&&<div style={{marginTop:8,fontSize:11,color:"#6b7280"}}><strong>Jobs:</strong> {rj.map(j=>{const isPaid=j.paymentStatus==="paid";return <span key={j.id} style={{marginRight:8}}><span style={{color:isPaid?"#059669":"#d97706"}}>{isPaid?"●":"○"}</span> {j.name} ({fmt(getJobFinancials(j.id).totalRevenue*rep.commissionRate)})</span>})}</div>}</Card>})}{reps.filter(r=>!r.id.includes("SEED_FLAG")&&r.commissionRate===0&&jobs.some(j=>j.salesRep===r.id)).length>0&&<Card style={{marginTop:8,opacity:0.6}}><div style={{fontSize:13,color:"#6b7280"}}>Team members without commission rates: {reps.filter(r=>!r.id.includes("SEED_FLAG")&&r.commissionRate===0).map(r=>r.name).join(', ')}</div></Card>}</div>}

    {tab==="preview"&&previewDoc&&<Card style={{border:"1px solid #c8a25c30"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}><div style={{display:"flex",alignItems:"center",gap:12}}><div style={{fontSize:16,fontWeight:700,color:"#c8a25c"}}>{previewDoc.type==="quote"?"PROJECT QUOTE":previewDoc.type==="po"?"PURCHASE ORDER":previewDoc.type==="commission"?"COMMISSION STATEMENT":"INVOICE"}</div><StatusBadge docNum={previewDoc.data.docNum}/></div><div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{previewDoc.data.docNum&&<div style={{display:"flex",gap:4}}>{["drafted","sent","approved"].map(s=><button key={s} onClick={()=>setDocStatus(previewDoc.data.docNum,s)} style={{padding:"4px 10px",borderRadius:6,border:"1px solid "+(docStatuses[previewDoc.data.docNum]===s?"#c8a25c":"#636366"),background:docStatuses[previewDoc.data.docNum]===s?"#c8a25c20":"transparent",color:docStatuses[previewDoc.data.docNum]===s?"#c8a25c":"#6b7280",fontSize:10,fontFamily:"inherit",cursor:"pointer",textTransform:"capitalize"}}>{s}</button>)}</div>}<Btn onClick={()=>handleExportPDF(previewDoc)}><I n="download" s={14}/> Export PDF</Btn><Btn v="ghost" onClick={()=>{const doc=previewDoc;const isQuote=doc.type==="quote";const isPO=doc.type==="po";const isComm=doc.type==="commission";const recipient=isQuote?doc.data.customer?.email:isPO?doc.data.vendor?.email:isComm?doc.data.rep?.email:doc.data.customer?.email;setEmailTo(recipient||"");setEmailSubject((isQuote?"Project Quote":isPO?"Purchase Order":isComm?"Commission Statement":"Invoice")+" - "+doc.job.name+" - "+(doc.data.docNum||""));setEmailModal(doc)}}><I n="send" s={14}/> Email</Btn>{previewDoc.type!=="quote"&&previewDoc.type!=="commission"&&<Btn v={qbConfig.connected?"primary":"secondary"} onClick={()=>qbConfig.connected?pushToQB(previewDoc):setPage("qbsetup")} style={pushing?{opacity:0.6,pointerEvents:"none"}:{}}><I n="send" s={14}/> {pushing?"Pushing...":qbConfig.connected?"Push to QB":"Connect QB"}</Btn>}<Btn v="secondary" onClick={()=>setTab(previewDoc.type==="commission"?"commissions":previewDoc.type==="quote"?"quotes":previewDoc.type==="po"?"pos":"invoices")}>&larr; Back</Btn>{previewDoc.type==="invoice"&&previewDoc.data.items.length>0&&<Btn v="ghost" style={{fontSize:11,color:"#059669",border:"1px solid #05966930"}} onClick={()=>{previewDoc.data.items.forEach(item=>{if(item.qtyReceived>item.qtyInvoiced){ctx.updateLineItem(item.id,{qtyInvoiced:item.qtyReceived,invoiceDate:new Date().toISOString().split("T")[0]})}});notify("All "+previewDoc.data.items.length+" items marked as invoiced")}}>Mark All Invoiced</Btn>}</div></div>
      {emailModal&&<div style={{background:"#3a3a3c",border:"1px solid #c8a25c30",borderRadius:8,padding:20,marginBottom:16}}><div style={{fontSize:14,fontWeight:700,color:"#c8a25c",marginBottom:12}}>Email Document</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}><div><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:4}}>From (your email)</label><input value={emailFrom} onChange={e=>setEmailFrom(e.target.value)} placeholder="lisa@mwfurnishings.com" style={inputStyle}/></div><div><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:4}}>To (recipient)</label><input value={emailTo} onChange={e=>setEmailTo(e.target.value)} placeholder="recipient@email.com" style={inputStyle}/></div></div><div style={{marginBottom:12}}><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:4}}>Subject</label><input value={emailSubject} onChange={e=>setEmailSubject(e.target.value)} style={inputStyle}/></div><div style={{display:"flex",gap:8}}><Btn onClick={sendEmail} style={emailSending?{opacity:0.6,pointerEvents:"none"}:{}}>{emailSending?"Sending...":"Send Email"}</Btn><Btn v="secondary" onClick={()=>setEmailModal(null)}>Cancel</Btn></div><div style={{fontSize:10,color:"#4b5563",marginTop:8}}>To send to any recipient, verify your domain at resend.com/domains. Without domain verification, emails can only be sent to the Resend account owner email.</div></div>}
      <div style={{background:"#fff",color:"#111",borderRadius:8,padding:32,fontFamily:"'DM Sans',sans-serif"}}><div style={{display:"flex",justifyContent:"space-between",borderBottom:"2px solid #c8a25c",paddingBottom:16,marginBottom:20}}><div><div style={{fontSize:20,fontWeight:800}}>MIDWEST EDUCATIONAL FURNISHINGS</div><div style={{fontSize:11,color:"#999",marginTop:2}}>Designing Spaces | Building Futures</div><div style={{fontSize:12,color:"#666",marginTop:4}}>(847) 847-1865 · info@mwfurnishings.com · www.mwfurnishings.com</div></div><div style={{textAlign:"right"}}><div style={{fontSize:14,fontWeight:700,color:"#c8a25c",textTransform:"uppercase"}}>{previewDoc.type==="quote"?"Project Quote":previewDoc.type==="po"?"Purchase Order":previewDoc.type==="commission"?"Commission Statement":"Invoice"}</div><div style={{fontSize:12,fontWeight:600,color:"#333"}}>{previewDoc.data.docNum||""}</div><div style={{fontSize:12,color:"#666",marginTop:4}}>Date: {new Date().toLocaleDateString()}</div><div style={{fontSize:12,color:"#666"}}>Job: {previewDoc.job.id}</div>{previewDoc.type==="quote"&&<div style={{fontSize:11,color:"#999",marginTop:2}}>Valid for 30 days</div>}</div></div>
...
function CommissionsPage({jobs,reps,customers,updateRep,addRep,deleteRep,getJobFinancials,notify,triggerPrint}){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}><Header title="Commission Engine" sub="Editable reps, customizable rates — auto-calculated on every job" action={<Btn onClick={()=>setAddingRep(true)}><I n="plus" s={14}/> Add Sales Rep</Btn>}/>
...
function SalesPortalPage({jobs,reps,customers,lineItems,getJobFinancials,vendors}){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}><Header title="Sales Portal" sub="Company overview and per-rep dashboards"/>
    <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}><button onClick={()=>setActiveRep("overview")} style={{padding:"8px 16px",borderRadius:8,cursor:"pointer",background:isOverview?"linear-gradient(135deg,#c8a25c,#b8923c)":"#2c2c2e",color:isOverview?"#161618":"#9ca3af",fontSize:13,fontWeight:isOverview?600:400,fontFamily:"inherit",border:isOverview?"none":"1px solid #1e2130"}}>Overview</button>{reps.filter(r=>!r.id.includes("SEED_FLAG")).map(r=><button key={r.id} onClick={()=>setActiveRep(r.id)} style={{padding:"8px 16px",borderRadius:8,cursor:"pointer",background:activeRep===r.id?"linear-gradient(135deg,#c8a25c,#b8923c)":"#2c2c2e",color:activeRep===r.id?"#161618":"#9ca3af",fontSize:13,fontWeight:activeRep===r.id?600:400,fontFamily:"inherit",border:activeRep===r.id?"none":"1px solid #1e2130"}}>{r.name}</button>)}</div>
    <Card style={{marginBottom:20,background:"linear-gradient(135deg,#12141b,#1a1d27)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:16}}><div><div style={{fontSize:20,fontWeight:800,color:"#f5f5f4"}}>{isOverview?"Midwest Educational Furnishings":rep.name}</div><div style={{fontSize:13,color:"#6b7280"}}>{isOverview?"All team members · All territories":rep.territory+" · "+rep.tier+(commRate>0?" · "+pct(commRate*100)+" rate":"")}</div></div><div style={{display:"flex",gap:20,flexWrap:"wrap"}}><div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#6b7280"}}>Total Pipeline</div><div style={{fontSize:22,fontWeight:700,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{fmt(totalRev)}</div></div><div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#6b7280"}}>Quoting</div><div style={{fontSize:22,fontWeight:700,color:"#8b5cf6",fontFamily:"'DM Mono',monospace"}}>{quotingJobs.length}</div></div><div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#6b7280"}}>Active</div><div style={{fontSize:22,fontWeight:700,color:"#2563eb",fontFamily:"'DM Mono',monospace"}}>{activeJobs.length}</div></div><div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#6b7280"}}>Paid Revenue</div><div style={{fontSize:22,fontWeight:700,color:"#059669",fontFamily:"'DM Mono',monospace"}}>{fmt(paidRev)}</div></div>{!isOverview&&commRate>0&&<div style={{textAlign:"center"}}><div style={{fontSize:10,color:"#6b7280"}}>Commission</div><div style={{fontSize:22,fontWeight:700,color:"#d97706",fontFamily:"'DM Mono',monospace"}}>{fmt(totalRev*commRate)}</div></div>}</div></div></Card>

    {isOverview&&<Card style={{marginBottom:16}}><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:12}}>Team Performance</div>{reps.filter(r=>!r.id.includes("SEED_FLAG")).map(r=>{const rJobs=jobs.filter(j=>j.salesRep===r.id);const rv=rJobs.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return <div key={r.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #1e2130",cursor:"pointer"}} onClick={()=>setActiveRep(r.id)}><div style={{width:36,height:36,borderRadius:8,background:"#3a3a3c",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#c8a25c"}}>{r.name.split(" ").map(n=>n[0]).join("")}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#f5f5f4"}}>{r.name}</div><div style={{fontSize:11,color:"#6b7280"}}>{r.territory} · {r.tier}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:600,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{fmt(rv)}</div><div style={{fontSize:10,color:"#6b7280"}}>{rJobs.length} jobs</div></div></div>})}</Card>}

    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:16}}>
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:12}}>Pipeline by Phase</div>{["Quoting","In Progress","Invoiced","Complete"].map(phase=>{const pj=rj.filter(j=>j.phase===phase);const pRev=pj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return <div key={phase} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:"1px solid #1e2130"}}><Badge label={phase} color={statusColor(phase)}/><span style={{flex:1,fontSize:12,color:"#d1d5db"}}>{pj.length} jobs</span><span style={{fontSize:13,fontWeight:600,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{fmt(pRev)}</span></div>})}</Card>
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:12}}>Pending Deliveries ({pendingItems.length})</div>{pendingItems.length===0?<div style={{fontSize:12,color:"#059669"}}>All deliveries complete</div>:pendingItems.slice(0,8).map(item=><div key={item.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #1e2130"}}><div><div style={{fontSize:12,color:"#d1d5db"}}>{item.description}</div><div style={{fontSize:10,color:"#6b7280"}}>{jobs.find(j=>j.id===item.jobId)?.name}</div></div><span style={{fontSize:11,color:"#d97706",fontFamily:"'DM Mono',monospace"}}>{fmtN(item.qtyOrdered-item.qtyReceived)} pending</span></div>)}</Card>
    </div>
    <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:12}}>{isOverview?"All Jobs":"Jobs for "+rep.name}</div><Tbl columns={[
      ...
      {header:"Payment",render:r=><Badge label={r.paymentStatus} color={statusColor(r.paymentStatus)}/>},
      {header:"Due",render:r=><span style={{fontSize:11,color:r.dueDate&&new Date(r.dueDate)<new Date()?"#ef4444":"#6b7280"}}>{r.dueDate||"TBD"}</span>},
    ]} data={rj}/></Card>
  </div>;
}
...
function BrainPage({jobs,reps,lineItems,vendors,customers,getJobFinancials,brainQuery,setBrainQuery,brainLoading,setBrainLoading}){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)",display:"flex",flexDirection:"column",height:"calc(100vh - 48px)"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:20}}><div><h2 style={{fontSize:22,fontWeight:700,color:"#f5f5f4",marginBottom:4,letterSpacing:-0.5}}>Midwest Brain</h2><p style={{fontSize:13,color:"#6b7280"}}>{activeProvider}{' · '}14 years of institutional knowledge</p></div><Btn v="secondary" onClick={()=>setShowApiKey(!showApiKey)} style={{fontSize:11}}><I n="settings" s={14}/> {showApiKey?'Hide':'AI Setup'}</Btn></div>
    ...
    <Card style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",padding:0}}>
      <div style={{flex:1,overflow:"auto",padding:20}}>{history.map((msg,i)=><div key={i} style={{marginBottom:16,maxWidth:"80%",marginLeft:msg.role==="user"?"auto":0}}><div style={{padding:"12px 16px",borderRadius:12,background:msg.role==="user"?"#c8a25c":msg.role==="system"?"#1e213044":"#3a3a3c",color:msg.role==="user"?"#161618":"#d1d5db",fontSize:13,lineHeight:1.6,whiteSpace:"pre-wrap"}}>{msg.content.split("**").map((part,pi)=>pi%2===1?<strong key={pi} style={{color:msg.role==="user"?"#161618":"#f5f5f4"}}>{part}</strong>:part)}</div><div style={{fontSize:10,color:"#4b5563",marginTop:4,textAlign:msg.role==="user"?"right":"left"}}>{msg.role==="user"?"You":"Midwest Brain"}</div></div>)}{brainLoading&&<div style={{padding:"12px 16px",background:"#3a3a3c",borderRadius:12,maxWidth:"80%",fontSize:13,color:"#6b7280"}}><span style={{animation:"pulse 1.5s infinite"}}>Searching operational data...</span></div>}</div>
      <div style={{padding:16,borderTop:"1px solid #1e2130",display:"flex",gap:8}}><input value={brainQuery} onChange={e=>setBrainQuery(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleQuery()} placeholder="Ask anything — vendors, pricing, job status, commissions..." style={{flex:1,padding:"10px 16px",background:"#3a3a3c",border:"1px solid #2a2d37",borderRadius:8,color:"#e8e6e3",fontSize:13,outline:"none",fontFamily:"inherit"}}/><Btn onClick={handleQuery} style={{padding:"10px 20px"}}><I n="send" s={14}/> Ask</Btn></div>
    </Card>
  </div>;
}
...
function DirectoryPage({vendors,customers,reps,updateVendor,addVendor,deleteVendor,updateCustomer,addCustomer,deleteCustomer,updateRep,addRep,deleteRep,notify}){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}><Header title="Directory" sub="Manage all vendors, customers, and sales reps — edit, add, sort" action={<Btn onClick={startAdd}><I n="plus" s={14}/> Add {tab==="vendors"?"Vendor":tab==="customers"?"Customer":"Sales Rep"}</Btn>}/>
    <div style={{display:"flex",gap:12,marginBottom:20,alignItems:"center",flexWrap:"wrap"}}>
      <div style={{display:"flex",gap:4,background:"#2c2c2e",padding:4,borderRadius:10}}>{[["vendors","Vendors ("+vendors.length+")"],["customers","Customers ("+customers.length+")"],["reps","Sales Reps ("+reps.length+")"]].map(([id,label])=><button key={id} onClick={()=>{setTab(id);setEditId(null);setAdding(false);setSort("name");setDirSearch("")}} style={{padding:"8px 16px",borderRadius:8,border:"none",cursor:"pointer",background:tab===id?"#c8a25c":"transparent",color:tab===id?"#161618":"#6b7280",fontSize:13,fontWeight:tab===id?600:400,fontFamily:"inherit"}}>{label}</button>)}</div>
      <input value={dirSearch} onChange={e=>setDirSearch(e.target.value)} placeholder={"Search "+tab+"..."} style={{...inputStyle,maxWidth:260,background:"#2c2c2e",border:"1px solid #1e2130",padding:"8px 14px"}}/>
    </div>
...
function ExitReadinessPage({jobs,vendors,customers,lineItems}){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}><Header title="Exit Readiness Score" sub="Buyer due diligence — systemized, transferable, auditable"/>
    ...
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>{criteria.map((s,si)=><Card key={si}><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:14}}>{s.category}</div>{s.items.map((item,ii)=><div key={ii} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:ii<s.items.length-1?"1px solid #1e213044":"none"}}><div style={{width:20,height:20,borderRadius:6,flexShrink:0,background:item.status===true?"#059669":item.status==="partial"?"#d97706":"#636366",display:"flex",alignItems:"center",justifyContent:"center"}}>{item.status===true&&<I n="check" s={12}/>}{item.status==="partial"&&<span style={{fontSize:10,fontWeight:700,color:"#fff"}}>~</span>}</div><div style={{flex:1}}><div style={{fontSize:13,color:item.status?"#e8e6e3":"#6b7280",fontWeight:500}}>{item.name}</div><div style={{fontSize:11,color:"#4b5563",marginTop:1}}>{item.note}</div></div></div>)}</Card>)}</div>
    <Card style={{marginTop:20,textAlign:"center",borderTop:"2px solid #c8a25c"}}><div style={{fontSize:11,color:"#6b7280",textTransform:"uppercase",letterSpacing:2,marginBottom:4}}>Powered by</div><div style={{fontSize:20,fontWeight:800,color:"#c8a25c",letterSpacing:1}}>DYFRENT</div></Card>
  </div>;
}
...
function QBSetupPage({notify,qbConfig,setQbConfig}){
  ...
  return <div style={{animation:"fadeUp 0.5s cubic-bezier(0.25,0.1,0.25,1)"}}><Header title="QuickBooks Integration" sub="Enter your API credentials to connect — invoices and POs push directly"/>
    ...
    {/* API Reference */}
    <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:12}}>API Endpoints Used</div><div style={{fontFamily:"'DM Mono',monospace",fontSize:12,color:"#d1d5db",background:"#161618",padding:16,borderRadius:8,lineHeight:2}}>
