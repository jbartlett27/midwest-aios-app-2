...
const inputStyle = {width:"100%",padding:"11px 14px",background:"rgba(58,58,60,0.5)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:10,color:"#e8e6e3",fontSize:13,outline:"none",fontFamily:"inherit"};

// ─── SHARED COMPONENTS ───────────────────────────────────────
const Card = ({children,style,onClick,hover}) => <div onClick={onClick} style={{background:"#12141b",border:"1px solid #1e2130",borderRadius:12,padding:20,cursor:onClick?"pointer":"default",transition:"all 0.2s",...style}} onMouseEnter={e=>{if(hover||onClick){e.currentTarget.style.borderColor="#c8a25c44";e.currentTarget.style.transform="translateY(-1px)"}}} onMouseLeave={e=>{if(hover||onClick){e.currentTarget.style.borderColor="#1e2130";e.currentTarget.style.transform="translateY(0)"}}}>{children}</div>;
const Badge = ({label,color}) => <span style={{display:"inline-flex",padding:"2px 8px",borderRadius:6,fontSize:11,fontWeight:600,letterSpacing:0.3,background:`${color}18`,color}}>{label}</span>;
...
const Bar = ({value,max,color="#c8a25c",height=6}) => <div style={{width:"100%",height,background:"#1e2130",borderRadius:height,overflow:"hidden"}}><div style={{width:`${Math.min((value/(max||1))*100,100)}%`,height:"100%",background:color,borderRadius:height,transition:"width 0.6s cubic-bezier(0.25,0.1,0.25,1)"}}/></div>;
...
export default function MidwestAIOS() {
  ...
  if (!appReady) return (
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"100vh",width:"100vw",background:"#0d0f14",fontFamily:"'DM Sans',sans-serif"}}>
    ...
    <div style={{display:"flex",height:"100vh",width:"100vw",fontFamily:"'DM Sans','Satoshi',sans-serif",background:"#0a0c10",color:"#e8e6e3",overflow:"hidden"}}>
      ...
      <div style={{flex:1,overflow:"auto",background:"#0a0c10"}}>
        <div className="main-content" style={{maxWidth:1400,margin:"0 auto",padding:"24px 32px"}}>
          ...
          {page==="calendar"&&<DeliveryCalendar {...ctx}/>}
...
function Dashboard({jobs,lineItems,reps,vendors,customers,getJobFinancials,getJobItems,setPage,setSelectedJob,dateFilter,setDateFilter}){
  ...
  const blob=new Blob([wb],{type:"text/csv"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="midwest-full-export-"+new Date().toISOString().split("T")[0]+".csv";a.click();URL.revokeObjectURL(url);notify("Full database exported — 5 tables")}}>Full Database Export</Btn></div><p style={{fontSize:14,color:"#6b7280"}}>Midwest Educational Furnishings — {dateFilter==="all"?"All time":dateFilter==="ytd"?"Year to date":dateFilter==="quarter"?"This quarter":"This month"} · {filtered.length} jobs</p></div>
    <div className="resp-grid-4" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16,marginBottom:24}}>
      <div className="stat-animate"><StatCard label="Pipeline Revenue" value={fmt(totalRev)} sub={`${jobs.length} total jobs`} icon="chart" color="#c8a25c"/></div>
      <div className="stat-animate"><StatCard label="Active Jobs" value={activeJobs} sub={`${jobs.filter(j=>j.phase==="Quoting").length} quoting`} icon="briefcase" color="#2563eb"/></div>
      <div className="stat-animate"><StatCard label="Avg Margin" value={pct(avgMargin)} sub={fmt(totalRev-totalCost)+" total profit"} icon="dollar" color="#059669"/></div>
      <div className="stat-animate"><StatCard label="Delivery Rate" value={pct(totalOrdered?(totalReceived/totalOrdered)*100:0)} sub={`${pendingDel} items pending`} icon="truck" color="#d97706"/></div>
    </div>
    ...
    <div className="resp-grid-2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginTop:16}}>
      <Card><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:16}}>Team Leaderboard</div>{[...reps].filter(r=>!r.id.includes("SEED_FLAG")).sort((a,b)=>{const ar=jobs.filter(j=>j.salesRep===a.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);const br=jobs.filter(j=>j.salesRep===b.id).reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return br-ar}).map((rep,idx)=>{const rj=jobs.filter(j=>j.salesRep===rep.id);const rv=rj.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return <div key={rep.id} className="slide-row" style={{display:"flex",alignItems:"center",gap:12,padding:"12px 0",borderBottom:"1px solid #1e213008"}}><div style={{width:28,height:28,borderRadius:8,background:idx===0?"#c8a25c20":idx===1?"#9ca3af20":"#63636620",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:idx===0?"#c8a25c":idx===1?"#9ca3af":"#2a2d37"}}>{idx+1}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#e8e6e3"}}>{rep.name}</div><div style={{fontSize:11,color:"#6b7280"}}>{rep.territory} · {rj.length} jobs · {pct(rep.commissionRate*100)}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:14,fontWeight:700,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{fmt(rv)}</div><div style={{fontSize:10,color:"#30d158"}}>comm: {fmt(rv*rep.commissionRate)}</div></div></div>})}</Card>
...
function JobDetail({job,ctx}){
  ...
  return <div style={{animation:"fadeUp 0.3s"}}>
    <div style={{position:"sticky",top:0,zIndex:100,background:"#0a0c10e8",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",margin:"-28px -36px 0",padding:"16px 36px 0",borderBottom:"1px solid rgba(255,255,255,0.06)"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12,flexWrap:"wrap",gap:8}}>
        <div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:2}}><h2 style={{fontSize:22,fontWeight:800,color:"#f5f5f4",margin:0}}>{job.name}</h2><Badge label={job.phase} color={statusColor(job.phase)}/><Badge label={job.paymentStatus} color={statusColor(job.paymentStatus)}/></div><div style={{fontSize:12,color:"#6b7280"}}>{job.id} · {customer?.name} · {rep?.name} · {fmt(f.totalRevenue)} rev · {pct(f.margin)} margin</div></div>
  ...
  triggerPrint('Job Packet - '+job.name,coverHtml+'<div style="page-break-before:always"></div>'+quoteHtml+'<div style="page-break-before:always"></div><h2 style="color:#2563eb;border-bottom:2px solid #2563eb;padding-bottom:8px">PURCHASE ORDERS</h2>'+poSections+'<div style="page-break-before:always"></div>'+deliveryHtml+summaryHtml+sigHtml)}} style={{fontSize:11,padding:"6px 10px"}}><I n="download" s={12}/> Job Packet</Btn></div>
      ...
      <div style={{display:"flex",gap:0,marginBottom:0}}>{[["items","Line Items",items.length],["details","Details",null],["timeline","Timeline",null],["activity","Activity",activities.length]].map(([id,label,count])=><button key={id} onClick={()=>setDetailTab(id)} style={{padding:"10px 18px",border:"none",borderBottom:detailTab===id?"2px solid #c8a25c":"2px solid transparent",background:"transparent",color:detailTab===id?"#c8a25c":"#6b7280",fontSize:13,fontWeight:detailTab===id?600:400,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>{label}{count!==null&&<span style={{fontSize:10,color:detailTab===id?"#c8a25c":"#2a2d37"}}>{count}</span>}</button>)}</div>
    </div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:0,overflow:"auto",padding:"8px 0"}}>
        {[
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
            <div style={{width:24,height:24,borderRadius:"50%",background:step.done?step.color:"#1e2130",border:"2px solid "+(step.done?step.color:"#2a2d37"),display:"flex",alignItems:"center",justifyContent:"center"}}>{step.done&&<I n="check" s={12}/>}</div>
          ...
          {i<arr.length-1&&<div style={{flex:1,height:2,background:step.done?"#2a2d37":"#1a1d27",margin:"0 4px",marginBottom:20}}/>}
        </div>)}
      </div>
...
function DeliveryCalendar({jobs,lineItems,vendors,customers,getJobItems,setPage,setSelectedJob,notify}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Delivery Calendar" sub={monthName+" "+year+" — expected deliveries for active jobs"} action={<div style={{display:"flex",gap:8}}><Btn v="secondary" onClick={()=>setMonthOffset(p=>p-1)}>&larr; Prev</Btn><Btn v="secondary" onClick={()=>setMonthOffset(0)}>Today</Btn><Btn v="secondary" onClick={()=>setMonthOffset(p=>p+1)}>Next &rarr;</Btn></div>}/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:1,background:"#1e2130",borderRadius:12,overflow:"hidden",border:"1px solid #1e2130"}}>
      {["Sun","Mon","Tue","Wed","Thu","Fri","Sat"].map(d=><div key={d} style={{padding:"10px 8px",textAlign:"center",fontSize:11,fontWeight:600,color:"#6b7280",background:"#12141b"}}>{d}</div>)}
      {days.map((day,i)=>{
        const dayEvents=day?events.filter(e=>e.day===day):[];
        const isToday=day&&month===now.getMonth()&&year===now.getFullYear()&&day===now.getDate();
        return <div key={i} style={{minHeight:90,padding:6,background:day?"#0a0c10":"#0d0f14",borderTop:"1px solid #1e2130",position:"relative"}}>
          {day&&<div style={{fontSize:12,fontWeight:isToday?700:400,color:isToday?"#c8a25c":"#6b7280",marginBottom:4}}>{isToday?<span style={{background:"#c8a25c",color:"#0a0c10",borderRadius:10,padding:"1px 6px",fontSize:11}}>{day}</span>:day}</div>}
    ...
    {events.length===0&&<Card style={{marginTop:16,textAlign:"center",padding:30}}><div style={{fontSize:14,color:"#6b7280"}}>No deliveries scheduled for {monthName} {year}. Set delivery dates on line items or due dates on jobs to see them here.</div></Card>}
    {events.length>0&&<Card style={{marginTop:16}}><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:12}}>{monthName} Deliveries ({events.length} items)</div>{events.sort((a,b)=>a.day-b.day).map((ev,i)=><div key={i} className="slide-row" onClick={()=>{if(ev.jobId){setSelectedJob(ev.jobId);setPage("jobs")}}} style={{display:"flex",alignItems:"center",gap:12,padding:"8px 0",borderBottom:"1px solid #1e2130",cursor:"pointer"}}><div style={{width:32,textAlign:"center",fontSize:14,fontWeight:700,color:"#c8a25c"}}>{ev.day}</div><div style={{width:4,height:24,borderRadius:2,background:ev.color}}/><div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#e8e6e3"}}>{ev.vendor} — {ev.item.slice(0,40)}</div><div style={{fontSize:11,color:"#6b7280"}}>{ev.job} · {ev.qty} units pending</div></div></div>)}</Card>}
  </div>;
}
...
function SalesPortalPage({jobs,reps,customers,lineItems,getJobFinancials,vendors}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Sales Portal" sub="Company overview and per-rep dashboards"/>
    <div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}><button onClick={()=>setActiveRep("overview")} style={{padding:"8px 16px",borderRadius:8,cursor:"pointer",background:isOverview?"linear-gradient(135deg,#c8a25c,#b8923c)":"#12141b",color:isOverview?"#0a0c10":"#9ca3af",fontSize:13,fontWeight:isOverview?600:400,fontFamily:"inherit",border:isOverview?"none":"1px solid #1e2130"}}>Overview</button>{reps.filter(r=>!r.id.includes("SEED_FLAG")).map(r=><button key={r.id} onClick={()=>setActiveRep(r.id)} style={{padding:"8px 16px",borderRadius:8,cursor:"pointer",background:activeRep===r.id?"linear-gradient(135deg,#c8a25c,#b8923c)":"#12141b",color:activeRep===r.id?"#0a0c10":"#9ca3af",fontSize:13,fontWeight:activeRep===r.id?600:400,fontFamily:"inherit",border:activeRep===r.id?"none":"1px solid #1e2130"}}>{r.name}</button>)}</div>
    ...
    {isOverview&&<Card style={{marginBottom:16}}><div style={{fontSize:14,fontWeight:700,color:"#f5f5f4",marginBottom:12}}>Team Performance</div>{reps.filter(r=>!r.id.includes("SEED_FLAG")).map(r=>{const rJobs=jobs.filter(j=>j.salesRep===r.id);const rv=rJobs.reduce((s,j)=>s+getJobFinancials(j.id).totalRevenue,0);return <div key={r.id} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:"1px solid #1e2130",cursor:"pointer"}} onClick={()=>setActiveRep(r.id)}><div style={{width:36,height:36,borderRadius:8,background:"#1a1d27",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,fontWeight:700,color:"#c8a25c"}}>{r.name.split(" ").map(n=>n[0]).join("")}</div><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600,color:"#f5f5f4"}}>{r.name}</div><div style={{fontSize:11,color:"#6b7280"}}>{r.territory} · {r.tier}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:600,color:"#c8a25c",fontFamily:"'DM Mono',monospace"}}>{fmt(rv)}</div><div style={{fontSize:10,color:"#6b7280"}}>{rJobs.length} jobs</div></div></div>})}</Card>}
