...
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,notify,qbConfig,setPage,triggerPrint}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Documents" sub="Quotes, Purchase Orders, Invoices — all auto-drafted from job records"/>
    ...
    {tab==="preview"&&previewDoc&&<Card style={{border:"1px solid #c8a25c30"}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{fontSize:16,fontWeight:700,color:"#c8a25c"}}>{previewDoc.type==="quote"?"PROJECT QUOTE":previewDoc.type==="po"?"PURCHASE ORDER":"INVOICE"} — Draft Preview</div><div style={{display:"flex",gap:8}}><Btn onClick={()=>handleExportPDF(previewDoc)}><I n="download" s={14}/> Export PDF</Btn>{previewDoc.type!=="quote"&&<Btn v={qbConfig.connected?"primary":"secondary"} onClick={()=>qbConfig.connected?pushToQB(previewDoc):setPage("qbsetup")} style={pushing?{opacity:0.6,pointerEvents:"none"}:{}}><I n="send" s={14}/> {pushing?"Pushing...":qbConfig.connected?"Push to QuickBooks":"Connect QB First"}</Btn>}</div></div>
      <div style={{background:"#fff",color:"#111",borderRadius:8,padding:32,fontFamily:"'DM Sans',sans-serif"}}><div style={{display:"flex",justifyContent:"space-between",borderBottom:"2px solid #c8a25c",paddingBottom:16,marginBottom:20}}><div><div style={{fontSize:20,fontWeight:800}}>MIDWEST EDUCATIONAL FURNISHINGS</div><div style={{fontSize:11,color:"#999",marginTop:2}}>Designing Spaces | Building Futures</div><div style={{fontSize:12,color:"#666",marginTop:4}}>(847) 847-1865 · info@mwfurnishings.com · www.mwfurnishings.com</div></div><div style={{textAlign:"right"}}><div style={{fontSize:14,fontWeight:700,color:"#c8a25c",textTransform:"uppercase"}}>{previewDoc.type==="quote"?"Project Quote":previewDoc.type==="po"?"Purchase Order":"Invoice"}</div><div style={{fontSize:12,fontWeight:600,color:"#333"}}>{previewDoc.data.docNum||""}</div><div style={{fontSize:12,color:"#666",marginTop:4}}>Date: {new Date().toLocaleDateString()}</div><div style={{fontSize:12,color:"#666"}}>Job: {previewDoc.job.id}</div>{previewDoc.type==="quote"&&<div style={{fontSize:11,color:"#999",marginTop:2}}>Valid for 30 days</div>}</div></div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:24}}><div><div style={{fontSize:11,fontWeight:700,color:"#999",textTransform:"uppercase",marginBottom:4}}>{previewDoc.type==="quote"?"Prepared For":previewDoc.type==="po"?"Vendor":"Bill To"}</div><div style={{fontSize:13,fontWeight:600}}>{previewDoc.type==="quote"?previewDoc.data.customer?.name:previewDoc.type==="po"?previewDoc.data.vendor?.name:previewDoc.data.customer?.name}</div>{(previewDoc.type==="quote"?previewDoc.data.customer?.address:previewDoc.type==="po"?previewDoc.data.vendor?.address:previewDoc.data.customer?.address)&&<div style={{fontSize:11,color:"#888",marginTop:2}}>{previewDoc.type==="quote"?previewDoc.data.customer?.address:previewDoc.type==="po"?previewDoc.data.vendor?.address:previewDoc.data.customer?.address}</div>}</div><div><div style={{fontSize:11,fontWeight:700,color:"#999",textTransform:"uppercase",marginBottom:4}}>Project</div><div style={{fontSize:13,fontWeight:600}}>{previewDoc.job.name}</div>{previewDoc.job.notes&&<div style={{fontSize:11,color:"#888",marginTop:2}}>{previewDoc.job.notes}</div>}</div></div>
