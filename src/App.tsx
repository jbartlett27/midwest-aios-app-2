  ...
  triggerPrint('Job Packet - '+job.name,coverHtml+'<div style="page-break-before:always"></div>'+quoteHtml+'<div style="page-break-before:always"></div>'+hd+poSections+'<div style="page-break-before:always"></div>'+hd+deliveryHtml+summaryHtml+sigHtml)}} style={{fontSize:12,padding:"6px 10px"}}><I n="download" s={12}/> Job Packet</Btn></div>
    ...
    <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",borderRadius:10,border:"1px solid #222222"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:1400}}><thead><tr style={{background:"#111111"}}>{["Tag","Manuf.","Model #","Description","Color","List","Net Each","Your Price","Ship/Unit","Install/Unit","Qty","Recd","Status","Total",""].map((h,i)=><th key={i} style={{padding:"6px 8px",textAlign:i>=5?"right":"left",fontWeight:600,color:"#a3a3a3",fontSize:11,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid #222222",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{(()=>{
      ...
      return <tr key={item.id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
        ...
        <td style={{padding:"6px 8px",minWidth:70}}>{isE?<div><input value={item.manufacturer||""} onChange={e=>{const val=e.target.value;const match=vendors.find(v=>v.name.toLowerCase()===val.toLowerCase());updateLineItem(item.id,{manufacturer:val,...(match?{vendor:match.id}:{})})}} list="vendor-list-tbl" style={{...eS,width:80}}/>{(()=>{const v=vendors.find(v=>v.id===item.vendor);return v?<div style={{display:"flex",alignItems:"center",gap:3,marginTop:2}}><input type="number" value={v.discountRate?Math.round(v.discountRate*100):""} onChange={e=>{const dr=(parseFloat(e.target.value)||0)/100;updateVendor(v.id,{discountRate:dr});if(item.listPrice){updateLineItem(item.id,{unitCost:Math.round((item.listPrice||0)*(1-dr)*100)/100})}}} style={{...eS,width:35,textAlign:"right",padding:"2px 3px",fontSize:10}} placeholder="0"/><span style={{fontSize:10,color:"#34d399"}}>% disc</span></div>:null})()}</div>:<span style={{color:"#c4c4c4"}}>{item.manufacturer||vendors.find(v=>v.id===item.vendor)?.name||""}{(()=>{const v=vendors.find(v=>v.id===item.vendor);return v?.discountRate?<span style={{fontSize:10,color:"#34d399",display:"block"}}>({(v.discountRate*100).toFixed(0)}% off)</span>:null})()}</span>}</td>
