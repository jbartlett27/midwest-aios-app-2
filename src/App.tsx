...
function JobDetail({job,ctx}){
  ...
  triggerPrint('Job Packet - '+job.name,coverHtml+'<div style="page-break-before:always"></div>'+quoteHtml+'<div style="page-break-before:always"></div><h2 style="color:#2563eb;border-bottom:2px solid #2563eb;padding-bottom:8px">PURCHASE ORDERS</h2>'+poSections+'<div style="page-break-before:always"></div>'+deliveryHtml+summaryHtml+sigHtml)}} style={{fontSize:12,padding:"6px 10px"}}><I n="download" s={12}/> Job Packet</Btn></div>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><div></div><Btn v="ghost" style={{fontSize:12}} onClick={()=>setEditingItem(editingItem==="ALL"?null:"ALL")}>{editingItem==="ALL"?"Done Editing":"Edit All"}</Btn></div>
    <div style={{overflowX:"auto",WebkitOverflowScrolling:"touch",borderRadius:10,border:"1px solid #222222"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:1400}}><thead><tr style={{background:"#111111"}}>{["Tag","Manuf.","Model #","Description","Color","List","Net Each","Your Price","Ship/Unit","Install/Unit","Qty","Recd","Status","Total",""].map((h,i)=><th key={i} style={{padding:"6px 8px",textAlign:i>=5?"right":"left",fontWeight:600,color:"#a3a3a3",fontSize:11,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid #222222",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead><tbody>{items.map(item=>{
      ...
      return <tr key={item.id} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
        ...
        <td style={{padding:"6px 8px",minWidth:70}}>{isE?<input value={item.manufacturer||""} onChange={e=>{const val=e.target.value;const match=vendors.find(v=>v.name.toLowerCase()===val.toLowerCase());updateLineItem(item.id,{manufacturer:val,...(match?{vendor:match.id}:{})})}} list="vendor-list-tbl" style={{...eS,width:80}}/>:<span style={{color:"#c4c4c4"}}>{item.manufacturer||vendors.find(v=>v.id===item.vendor)?.name||""}</span>}</td>
        ...
        <td style={{padding:"6px 8px"}}><div style={{display:"flex",gap:3}}>{editingItem!=="ALL"&&<Btn v={isE?"primary":"ghost"} style={{fontSize:11,padding:"2px 6px"}} onClick={()=>setEditingItem(isE?null:item.id)}>{isE?"Done":"Edit"}</Btn>}<Btn v="secondary" style={{fontSize:11,padding:"2px 6px"}} onClick={()=>{addLineItem({...item,id:"LI-"+Math.random().toString(36).slice(2,8),qtyReceived:0,qtyInvoiced:0});notify("Line item duplicated")}}>Dup</Btn>{isE&&<Btn v="danger" style={{fontSize:11,padding:"2px 6px"}} onClick={()=>{deleteLineItem(item.id);if(editingItem===item.id)setEditingItem(null);notify("Line item deleted")}}>Del</Btn>}</div></td>
      </tr>})}</tbody></table></div>
