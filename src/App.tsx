  ...
  return <div style={{animation:"fadeUp 0.3s"}}>
    ...
    <div style={{overflowX:"auto",borderRadius:10,border:"1px solid #1e2130"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}><thead><tr style={{background:"#12141b"}}>{["Item","Vendor","Unit Cost","Unit Price","Ordered","Received","Invoiced","Status","Line Total",""].map((h,i)=><th key={i} style={{padding:"8px 10px",textAlign:"left",fontWeight:600,color:"#6b7280",fontSize:10,textTransform:"uppercase",letterSpacing:0.8,borderBottom:"1px solid #1e2130"}}>{h}</th>)}</tr></thead><tbody>{items.map(item=>{
      ...
      return <tr key={item.id} style={{borderBottom:"1px solid #1a1d2708"}}>
        ...
        <td style={{padding:"8px 10px",color:"#d1d5db"}}>{isEditing?<select value={item.vendor} onChange={e=>updateLineItem(item.id,{vendor:e.target.value})} style={{...inputStyle,fontSize:12,padding:"4px 8px"}}>{vendors.map(v=><option key={v.id} value={v.id}>{v.name}{v.discountRate?' ('+((v.discountRate*100).toFixed(0))+'% off)':''}</option>)}</select>:<span>{vendors.find(v=>v.id===item.vendor)?.name}{(()=>{const v=vendors.find(v=>v.id===item.vendor);return v?.discountRate?<span style={{fontSize:10,color:"#059669",marginLeft:4}}>({(v.discountRate*100).toFixed(0)}% disc)</span>:null})()}</span>}</td>
...
function DocumentsPage({jobs,lineItems,vendors,customers,reps,getJobItems,notify,qbConfig,setPage,triggerPrint}){
  ...
  // Unique document number generator
  const genDocNum = (prefix) => { const d=new Date(); return prefix+d.getFullYear().toString().slice(2)+String(d.getMonth()+1).padStart(2,'0')+'-'+String(Math.floor(Math.random()*9000)+1000); };

  const genPOs=job=>{const items=getJobItems(job.id);const groups={};items.forEach(i=>{if(!groups[i.vendor])groups[i.vendor]=[];groups[i.vendor].push(i)});return Object.entries(groups).map(([vid,vitems])=>({vendor:vendors.find(v=>v.id===vid),items:vitems,total:vitems.reduce((s,i)=>s+i.unitCost*i.qtyOrdered,0),job,docNum:genDocNum('PO-')}))};
  const genInvoice=job=>{const items=getJobItems(job.id).filter(i=>i.qtyReceived>i.qtyInvoiced);return {customer:customers.find(c=>c.id===job.customer),items,total:items.reduce((s,i)=>s+i.unitPrice*(i.qtyReceived-i.qtyInvoiced),0),job,docNum:genDocNum('INV-')}};
  const genQuote=job=>{const items=getJobItems(job.id);const customer=customers.find(c=>c.id===job.customer);return {customer,items,total:items.reduce((s,i)=>s+i.unitPrice*i.qtyOrdered,0),job,docNum:genDocNum('QT-')}};
  ...
  const handleExportPDF=(doc)=>{
    ...
    triggerPrint(docTitle+" - "+doc.job.name,
      '<div class="hdr"><div><div class="co">MIDWEST EDUCATIONAL FURNISHINGS</div><div>Designing Spaces | Building Futures</div><div style="margin-top:4px">(847) 847-1865 · info@mwfurnishings.com · www.mwfurnishings.com</div></div><div style="text-align:right"><div class="gold" style="font-weight:bold;font-size:14px">'+docTitle+'</div><div style="font-weight:bold;font-size:13px">'+(doc.data.docNum||'')+'</div><div>Date: '+new Date().toLocaleDateString()+'</div><div>Job: '+doc.job.id+'</div>'+(isQuote?'<div>Valid for 30 days</div>':'')+'</div></div>'
...
function DirectoryPage({vendors,customers,reps,updateVendor,addVendor,deleteVendor,updateCustomer,addCustomer,deleteCustomer,updateRep,addRep,deleteRep,notify}){
  ...
  return <div style={{animation:"fadeUp 0.4s"}}><Header title="Directory" sub="Manage all vendors, customers, and sales reps — edit, add, sort" action={<Btn onClick={startAdd}><I n="plus" s={14}/> Add {tab==="vendors"?"Vendor":tab==="customers"?"Customer":"Sales Rep"}</Btn>}/>
    ...
    {(adding||editId)&&<Card style={{marginBottom:16,border:"1px solid #c8a25c30"}}><div style={{fontSize:14,fontWeight:700,marginBottom:12,color:"#c8a25c"}}>{adding?"Add New":"Edit"} {tab==="vendors"?"Vendor":tab==="customers"?"Customer":"Sales Rep"}</div>
      {tab==="vendors"&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>{field("name","Company Name")}{field("contact","Contact Person")}{field("email","Email")}{field("phone","Phone")}{field("category","Category")}{field("address","Address")}<div><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:4}}>Discount Rate (%)</label><input type="number" step="1" value={((form.discountRate||0)*100).toFixed(0)} onChange={e=>setForm(prev=>({...prev,discountRate:parseFloat(e.target.value)/100||0}))} style={inputStyle}/></div>{field("discountType","Discount Type")}<div style={{gridColumn:"span 1"}}><label style={{fontSize:11,color:"#6b7280",display:"block",marginBottom:4}}>Discount Notes</label><input value={form.discountNotes||""} onChange={e=>setForm(prev=>({...prev,discountNotes:e.target.value}))} placeholder="Volume thresholds, freight terms..." style={inputStyle}/></div></div>}
    ...
    {tab==="vendors"&&<><div style={{marginBottom:8,display:"flex",gap:8,alignItems:"center"}}><span style={{fontSize:12,color:"#6b7280"}}>Sort by:</span>{["name","category","contact"].map(s=><button key={s} onClick={()=>setSort(s)} style={{fontSize:11,padding:"3px 10px",borderRadius:6,border:"none",cursor:"pointer",background:sort===s?"#c8a25c22":"transparent",color:sort===s?"#c8a25c":"#6b7280",fontFamily:"inherit"}}>{s}</button>)}</div>
      <Tbl columns={[{header:"Name",render:r=><span style={{fontWeight:600,color:"#f5f5f4"}}>{r.name}</span>},{header:"Contact",render:r=>r.contact},{header:"Email",render:r=><span style={{color:"#c8a25c"}}>{r.email}</span>},{header:"Phone",render:r=>r.phone},{header:"Category",render:r=>r.category},{header:"Discount",render:r=><span style={{fontFamily:"'DM Mono',monospace",color:"#059669",fontWeight:600}}>{r.discountRate?(r.discountRate*100).toFixed(0)+'%':'--'}</span>},{header:"Discount Notes",render:r=><span style={{fontSize:11,color:"#9ca3af"}}>{r.discountNotes||"--"}</span>},{header:"",render:r=><Btn v="ghost" style={{fontSize:10,padding:"3px 8px"}} onClick={e=>{e.stopPropagation();startEdit(r)}}>Edit</Btn>}]} data={sortedVendors}/></>}
